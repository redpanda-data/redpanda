...
class JiraHelper:
    ...
    def _check_env_val(self, val: str):
        ...
        try:
            assert os.environ[val] is not None, (
                f"Environment variable {val} must be set"
            )
        except KeyError:
    ...
    def _check_env(self):
        ...
        if self.command == Command.ISSUE:
            ...
            raise NotImplementedError(f"Command {self.command} is not implemented")

    def _get_gh_issue_comments(self, url):
        return json.loads(
            self._run_cmd_return_stdout(f"gh issue view {url} --json comments")
        )
    ...
    def _issue_helper(self):
        ...
        if event_name == "opened":
            ...
            if issue_id is not None:
                ...
                self.logger.debug(f"Creating issue for {os.environ['ISSUE_URL']}")
                self._create_issue_and_add_comments()
        elif event_name == "reopened":
            issue_id = self._find_issue()
            self._create_issue_and_add_comments() if issue_id is None else self._reopen_issue(
                issue_id
            )
            ...
            issue_id = self._find_issue()
            self._create_issue_and_add_comments() if issue_id is None else self._update_issue(
                issue_id
            )
    ...
    def execute(self):
        ...
        if self.command == Command.ISSUE:
            ...
            raise NotImplementedError(f"Command {self.command} not yet implemented")
    ...
    def _close_issue(self, issue_id):
        ...
        self._add_comment_to_issue(
            issue_id, f"Closing JIRA issue from GitHub issue {os.environ['ISSUE_URL']}"
        )
    ...
    def _create_issue(self):
        ...
        fields = {
            "description"
            if issue_type == "Task"
            else "customfield_10083": f"{jira_issue_body}",
            "summary": f"{issue_title}",
            "issuetype": {"name": f"{issue_type}"},
            "labels": labels,
            "project": {"key": f"{self._project_key}"},
...
"""
        ...
        self.logger.debug(f"Updating issue text to include link to {jira_issue_url}")
    ...
    def _add_comment_to_issue(self, issue_id, comment):
        ...
        self.logger.debug(f'Updating issue {issue_id} with comment "{comment}"')
        try:
    ...
    def _update_comment(self):
        try:
            ...
            self.logger.warning(f"No issue found for {os.environ['ISSUE_URL']}")
        except MultipleJiraIssuesFound:
            self.logger.warning(f"Multiple issues found for {os.environ['ISSUE_URL']}")
    ...
    def _update_issue(self, issue_id):
        ...
        fields = {}
        jira_issue_summary = self._jira.issue_field_value(issue_id, field="summary")
        ...
        jira_issue_labels = self._jira.issue_field_value(issue_id, field="labels")
...
def parse_args() -> Tuple[Command, bool]:
    ...
    parser.add_argument("-p", "--project", required=True, help="The project to use")
