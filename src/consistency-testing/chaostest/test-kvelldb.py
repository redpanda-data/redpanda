...
async def run(config, n, overrides):
    ...
    for _ in range(0, n):
        chaos_stdout.info("starting a cluster")
        cluster.terminate_wipe_restart()
        cluster_warmup = config["cluster_warmup"]
        await asyncio.sleep(cluster_warmup)
        chaos_stdout.info("cluster started")
        chaos_stdout.info("")

        await inject_recover_scenarios_aio(config, cluster, faults,
