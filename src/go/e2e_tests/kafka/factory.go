...
func (f *sandboxClientFactory) NewConsumer(config *sarama.Config) sarama.Consumer {
	return CreateConsumer(f.s, config)
}

func (f *sandboxClientFactory) NewSyncProducer(config *sarama.Config) sarama.SyncProducer {
	return CreateProducer(f.s, config)
}

func (f *sandboxClientFactory) NewClusterAdmin(config *sarama.Config) sarama.ClusterAdmin {
...
func (f *addressClientFactory) NewConsumer(config *sarama.Config) sarama.Consumer {
	return createConsumer([]string{f.addr}, config)
}

func (f *addressClientFactory) NewSyncProducer(config *sarama.Config) sarama.SyncProducer {
	return createProducer([]string{f.addr}, config)
}

func (f *addressClientFactory) NewClusterAdmin(config *sarama.Config) sarama.ClusterAdmin {
