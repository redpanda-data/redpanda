...
func (r *externalDNS) Ensure(ctx context.Context) error {
	...
	if !reflect.DeepEqual(dnsEndpoint.Spec.Endpoints, r.endpoints) {
		...
		r.logger.V(defaultLogLevel).Info("DNSEndpoint update", "number of endpoints", len(dnsEndpoint.Spec.Endpoints), "NamespacedName", r.Key())
		r.logger.V(verboseLogLevel).Info("DNSEndpoint update", "name", dnsEndpoint.Name, "endpoints", dnsEndpoint.Spec.Endpoints)
