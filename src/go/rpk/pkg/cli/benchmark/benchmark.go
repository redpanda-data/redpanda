...
func newBenchmarkRun(fs afero.Fs, p *config.Params, cmd *cobra.Command, cfg benchmarkConfig) (*benchmarkRun, error) {
	...
	return &benchmarkRun{
		...
		ctx:     ctx,
		cancel:  cancel,
		timing:  newBenchmarkTiming(ctx, cfg),
		adm:     adm,
		topic:   cfg.topic,
	}, nil
}
...
func prepareBenchmarkTopic(ctx context.Context, adm *kadm.Client, cfg benchmarkConfig) error {
	err := createBenchmarkTopic(ctx, adm, cfg.topic, cfg.partitions, cfg.replicas)
	if err != nil {
		return err
	}
	...
	if cfg.waitLeadershipBalanced {
		...
		if err := waitForBalancedLeadership(ctx, adm, cfg.topic, cfg.partitions); err != nil {
			if deleteErr := deleteBenchmarkTopic(ctx, adm, cfg.topic); deleteErr != nil {
				return fmt.Errorf("%w (cleanup warning: unable to delete topic %q: %v)", err, cfg.topic, deleteErr)
			}
			return err
		}
	}
