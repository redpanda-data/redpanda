...
func TestModeCommand(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cmd.SetArgs(tt.args)
			err = executeMode(fs, cmd, tt.args)
			...
			require.Exactly(t, tt.expectedConfig.Redpanda.DeveloperMode, conf.Redpanda.DeveloperMode)
			for _, tuner := range []string{
				"TuneNetwork",
				"TuneDiskScheduler",
				"TuneDiskWriteCache",
				"TuneNomerges",
				...
				e := reflect.ValueOf(tt.expectedConfig.Rpk).FieldByName(tuner)
				v := reflect.ValueOf(conf.Rpk).FieldByName(tuner)
				require.Exactly(t, e.Bool(), v.Bool())
			}
		})
	}
}
