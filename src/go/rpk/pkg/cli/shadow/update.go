...
`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			// The submitted configuration replaces the entire configuration;
			// the editor is pre-seeded with the current one.
			updatedCfg, changed, err := resolveUpdatedConfig(cmd.Context(), fs, cfg, prof, fromCloud, linkName)
			out.MaybeDieErr(err)
			if !changed {
...
// resolveUpdatedConfig returns the replacement config from an editor seeded
// with the current one; changed is false when the editor left it untouched.
func resolveUpdatedConfig(ctx context.Context, fs afero.Fs, cfg *config.Config, prof *config.RpkProfile, fromCloud bool, linkName string) (updated *ShadowLinkConfig, changed bool, err error) {
