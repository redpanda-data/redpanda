...
client::txn::txn(const client_opts &_opts, client_stats *m,
                 ...
                 seastar::shared_ptr<bmtl_api_client> c)
  ...
  p.put = std::make_unique<rp::wal_put_requestT>();
...
client::txn::stage(const char *key, int32_t key_size, const char *value,
  ...
  auto ctype = value_size >= opts.record_compression_value_threshold
                 ...
                 : rp::wal_compression_type::wal_compression_type_none;
  auto record =
    rp::wal_segment_record::coalesce(key, key_size, value, value_size, ctype);
  ...
  if (it == puts.end()) {
    auto ptr = std::make_unique<rp::wal_put_partition_recordsT>();
...
seastar::future<smf::rpc_recv_typed_context<rp::chains::chain_put_reply>>
client::txn::submit() {
  LOG_THROW_IF(submitted_,
...
client::open(seastar::ipv4_addr seed) {
  ...
  rpc_ = seastar::make_shared<bmtl_api_client>(seed);
  ...
  return rpc_->connect()
    .then([this]() {
      smf::rpc_typed_envelope<rp::wal_topic_create_request> x;
...
client::consume_from_partition(int32_t partition) {
  smf::rpc_typed_envelope<rp::chains::chain_get_request> x;
