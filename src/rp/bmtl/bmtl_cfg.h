...
template <>
struct convert<::rp::bmtl_cfg> {
  static Node
  encode(const ::rp::bmtl_cfg &rhs) {
    Node node;
  ...
  static bool
  decode(const Node &node, ::rp::bmtl_cfg &rhs) {
    // Required fields
...
operator<<(ostream &o, const ::rp::bmtl_cfg &c) {
  o << "rp::bmtl_cfg{directory:" << c.directory << ", ip:" << c.ip
    << ", port:" << c.port
