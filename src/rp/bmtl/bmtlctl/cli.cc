...
cli::cli(const boost::program_options::variables_map *cfg)
  ...
  id_ = rand_.next();
  rp::api::client_opts co(options()["namespace"].as<seastar::sstring>(),
  ...
  if (kRegularTopic == options()["topic-type"].as<seastar::sstring>()) {
    co.topic_type = rp::wal_topic_type::wal_topic_type_regular;
  }
  if (kCompactionTopic == options()["topic-type"].as<seastar::sstring>()) {
    co.topic_type = rp::wal_topic_type::wal_topic_type_compaction;
  }
