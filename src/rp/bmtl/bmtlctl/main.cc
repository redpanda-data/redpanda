...
main(int argc, char **argv, char **env) {
  ...
  seastar::distributed<rp::qps_load> runner;
  ...
  return app.run(argc, argv, [&runner, &app] {
    ...
    return runner.start(&cfg)
      .then([&runner] { return runner.invoke_on_all(&rp::qps_load::open); })
      .then([&runner] { return runner.invoke_on_all(&rp::qps_load::drive); })
      .then([&runner] {
        return runner
          .map_reduce(smf::unique_histogram_adder(),
                      [](auto &shard) { return shard.copy_histogram(); })
          .then([](auto h) {
            LOG_INFO("Histogram: bmtlctl.hgrm");
            return smf::histogram_seastar_utils::write("bmtlctl.hgrm",
                                                       std::move(h));
          });
      })
