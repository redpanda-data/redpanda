...
chain_replication_service::put(
  ...
  if (!record) { return smf::futurize_status_for_type<chain_put_reply>(400); }
  ...
  DLOG_TRACE("{}", flatbuffers::FlatBufferToString(
                     (const uint8_t *)record.ctx.value().payload.get(),
                     chain_put_request::MiniReflectTypeTable()));
  auto ptr = record.get();
  return seastar::do_with(
           std::move(record), wal_core_mapping::core_assignment(ptr->put()),
           [this](auto &r, auto &fsputs) mutable {
             ...
             return seastar::map_reduce(
                      ...
                      // Initial State:
                      std::make_unique<wal_write_reply>(r->put()->ns(),
                                                        r->put()->topic()),
                      ...
                      [this](auto acc, auto next) {
                        for (const auto &t : *next) {
                          ...
                          acc->set_reply_partition_tuple(
                            acc->ns, acc->topic, p->partition, p->start_offset,
                            p->end_offset);
                        }
                        return std::move(acc);
                      })
...
chain_replication_service::get(
  ...
  if (!record) { return smf::futurize_status_for_type<chain_get_reply>(400); }
  ...
  return seastar::with_semaphore(
    record.ctx->rpc_server_limits->resources_available, ptr->max_bytes(),
...
chain_replication_service::do_get(
  ...
  return seastar::do_with(
           ...
           [this, offset](auto &record, auto req) {
             LOG_THROW_IF(!wal_read_request::is_valid(req),
             ...
             return seastar::smp::submit_to(
               ...
               .then([offset](std::unique_ptr<wal_read_reply> r) {
                 ...
                 DLOG_THROW_IF(offset > data.data->get->next_offset,
    ...
    .handle_exception([ns, topic, partition, offset,
                       validate_payload](std::exception_ptr eptr) {
...
chain_replication_service::raw_get(smf::rpc_recv_context &&c) {
  using inner_t = rp::chains::chain_get_request;
  ...
  return get(input_t(std::move(c))).then([this](auto x) {
    ...
    const rp::chains::chain_get_replyT &ref = *x.data;
    x.envelope.letter.body = std::move(smf::native_table_as_buffer(ref));
    x.data = nullptr;

    smf::checksum_rpc(x.envelope.letter.header, x.envelope.letter.body.get(),
