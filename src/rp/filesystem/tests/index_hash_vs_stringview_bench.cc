...
bool
operator==(const foo &f, uint64_t x) {
  return f.hash == x;
}
struct foo_comparator_hash {
  bool
  operator()(const foo &f, const uint64_t &hash) const {
    return f.hash < hash;
  };
  bool
  operator()(const uint64_t &hash, const foo &f) const {
    return hash < f.hash;
  };
...
static void
BM_xxhash(benchmark::State &state) {
  ...
  for (auto i = 0; i < state.range(1); ++i) {
    ...
    f.hash = rp::xxhash_64(f.k.data(), f.k.size());
    s.insert(std::move(f));
  }
  auto it = s.begin();
...
static void
BM_string(benchmark::State &state) {
