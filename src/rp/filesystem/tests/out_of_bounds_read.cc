    ...
    return w.start(wal_opts(".", std::chrono::milliseconds(5)))
      ...
      .then([&] {
        return smash.invoke_on(0, [](auto &smasher) {
          ...
          return seastar::smp::submit_to(core, [&smasher,
                                                out_of_bounds_partition] {
            return smasher.write_one(out_of_bounds_partition)
              .then([](auto reply) {
                // ALSO test that the error is propagated to the released ptr
                std::unique_ptr<wal_put_replyT> ptr = reply->release();

                LOG_THROW_IF(
                  ptr->error !=
                    wal_put_errno::wal_put_errno_invalid_ns_topic_partition,
                  "Should have returned correct invalid_ns_topic_partition "
                  "error. got: {}",
                  EnumNamewal_put_errno(ptr->error));

                LOG_INFO(
                  "WRITE: Success. Correct error condition for out of bounds");
              });
          });
        });
      })
            ...
            core, [&smasher, out_of_bounds_partition] {
              return smasher.read_one(out_of_bounds_partition)
                .then([](auto reply) {
                  ...
                  LOG_INFO(
                    "READ: Success. Correct error condition for out of bounds");
                });
            });
        });
      });
