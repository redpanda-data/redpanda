...
TEST(page_cache_result_lease, ref_count_ctor) {
  rp::page_cache_result r(0, gsl::span<char>(),
                          rp::page_cache_result::priority::low);
  {
    rp::page_cache_result_lease l(&r);
    ASSERT_EQ(1, r.locks);
  }
...
TEST(page_cache_result, one_total_page) {
  ...
  rp::page_cache_result r(0, gsl::span<char>(payload.data(), payload.data()),
                          rp::page_cache_result::priority::low);
