...
TEST(wal_opts_validator, invalid_max_log_segment_size) {
  rp::wal_opts o(".", std::chrono::minutes(1), std::chrono::hours(168), -1, 4,
                 1024 * 1024, 1024 * 1024 /*BAD log_segment_size 1MB*/);

  ASSERT_EQ(rp::wal_opts::validate(o),
            rp::wal_opts::validation_status::invalid_log_segment_size);
}
TEST(wal_opts_validator, valid_max_log_segment_size) {
  ...
  ASSERT_EQ(rp::wal_opts::validate(o), rp::wal_opts::validation_status::ok);
}
TEST(wal_opts_validator, invalid_flush_period) {
  rp::wal_opts o(".", std::chrono::milliseconds(1) /*BAD FLUSH PERIOD*/,
                 std::chrono::hours(168), -1, 4, 1024 * 1024,
                 1024 * 1024 * 1024);
  ASSERT_EQ(rp::wal_opts::validate(o),
            rp::wal_opts::validation_status::invalid_writer_flush_period);
}
TEST(wal_opts_validator, invalid_retention_period) {
  ...
  ASSERT_EQ(rp::wal_opts::validate(o),
            rp::wal_opts::validation_status::invalid_retention_period);
}
