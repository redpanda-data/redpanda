...
write_poem_wal_segment(seastar::sstring base_dir, uint32_t max_appends = 100) {
  auto ws = seastar::make_lw_shared<rp::wal_segment>(
    base_dir + "/" + segment_name,
    rp::priority_manager::get().streaming_write_priority(),
  ...
  try {
    ...
    return app.run(args, argv, [&] {
      ...
      return skip_first_page(dir)
        ...
        .then([dir] {
          return rp::readfile(dir + "/" + ostream_name).then([dir](auto obuf) {
            return rp::readfile(dir + "/" + segment_name)
