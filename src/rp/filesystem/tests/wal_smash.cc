...
wal_smash::wal_smash(wal_smash_opts opt,
  ...
  opts_.ns_id =
    rp::xxhash_64(opts_.topic_namespace.c_str(), opts_.topic_namespace.size());
  opts_.topic_id = rp::xxhash_64(opts_.topic.c_str(), opts_.topic.size());
...
wal_smash::write_one(int32_t partition) {
  ...
  for (auto n = 0; n < opts_.write_batch_size; ++n) {
    ...
    auto record = rp::wal_segment_record::coalesce(k.data(), k.size(), v.data(),
                                                   v.size(), ctype);
