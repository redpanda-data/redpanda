...
struct key_hash_functor {
  ...
  operator()(const uint64_t &hash,
             const std::unique_ptr<rp::wal_segment_index_key_entryT> &e) const {
    return hash < e->hash;
  }
  bool
  operator()(const std::unique_ptr<rp::wal_segment_index_key_entryT> &e,
...
wal_segment_indexer::get_or_create(int64_t offset, const wal_binary_record *r) {
  ...
  auto xx = rp::xxhash_64(key_begin, hdr.key_size());
