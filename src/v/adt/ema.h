...
public:
    ...
    static constexpr double one_minus_alpha() {
        return 1 - alpha();
    }
    // -- main api
    inline void record_ema(uint64_t nanos) {
        _ema = alpha() * nanos + (one_minus_alpha() * _ema);
    }
    inline double get() const {
        return _ema;
    }
    std::unique_ptr<time_measure> measure() {
...
inline std::ostream& operator<<(ostream& o, const ema<Period>& e) {
