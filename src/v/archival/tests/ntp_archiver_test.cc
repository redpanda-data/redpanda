...
SEASTAR_THREAD_TEST_CASE(small_segment_run_test) {
    ...
    archival::adjacent_segment_run run;
    for (const auto& s : segments) {
        if (run.maybe_add_segment(s, high_watermark)) {
            break;
        }
    }
    BOOST_REQUIRE(run.base_offset == model::offset{11});
    BOOST_REQUIRE(run.max_offset == model::offset{21});
    BOOST_REQUIRE(run.num_segments == 3);
    BOOST_REQUIRE(run.size_bytes == 100);
}
