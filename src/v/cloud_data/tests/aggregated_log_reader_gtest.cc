    ...
    void produce_placeholders(bool use_cache, int group_by) {
        ...
        auto generate_placeholder =
          [](
            ...
            cloud_data::dl_placeholder p{
              .id = cloud_data::object_id(id),
              .offset = cloud_data::first_byte_offset_t(offset),
              .size_bytes = cloud_data::byte_range_size_t(size),
            };
            ...
            builder.add_raw_kv(
              serde::to_iobuf(cloud_data::dl_placeholder_record_key::payload),
              serde::to_iobuf(p));
            // Match number of records in the batch with the 'source'
            for (auto i = 1; i < source.record_count(); i++) {
                iobuf empty;
                builder.add_raw_kv(
                  serde::to_iobuf(cloud_data::dl_placeholder_record_key::empty),
                  std::move(empty));
            }
...
TEST_F_CORO(aggregated_log_reader_fixture, full_scan_test) {
    ...
    auto reader = cloud_data::make_aggregated_log_reader(
      config,
