...
// The format is `migration/{namespace}/{label}/{topic}`.
constexpr std::string_view topic_mount_manifest_path_fmt = "migration/{}/{}/{}";
...
ss::sstring remote_path_provider::topic_mount_manifest_path(
  const topic_mount_manifest& manifest) const {
    return ss::sstring(cloud_storage::topic_mount_manifest_path(
      manifest.get_source_label().cluster_uuid, manifest.get_tp_ns()));
}
...
topic_mount_manifest_path::operator ss::sstring() const {
    return fmt::format(
      topic_mount_manifest_path_fmt, _cluster_uuid, _tp_ns.ns, _tp_ns.tp);
}
