...
ss::future<> remote_segment::do_hydrate_segment() {
    ...
    if (auto cache_status = co_await _cache.is_cached(_path);
        cache_status == cloud_io::cache_element_status::available) {
        vlog(
          _ctxlog.debug,
          "skipping segment hydration for {} as it is in the cache",
          _path);
        co_return;
    }
...
ss::future<> remote_segment::do_hydrate_index() {
    ...
    if (auto cache_status = co_await _cache.is_cached(_index_path);
        cache_status == cloud_io::cache_element_status::available) {
        vlog(
          _ctxlog.debug,
          "skipping index hydration for {} as it is in the cache",
          _index_path);
        co_return;
    }
...
ss::future<> remote_segment::do_hydrate_txrange() {
    ...
    if (!_compacted) {
        ...
        if (auto cache_status = co_await _cache.is_cached(
              manifest.get_manifest_path());
            cache_status == cloud_io::cache_element_status::available) {
            vlog(
              _ctxlog.debug,
              "skipping tx-manifest hydration for {} as it is in the cache",
              manifest.get_manifest_path());
            co_return;
        }
