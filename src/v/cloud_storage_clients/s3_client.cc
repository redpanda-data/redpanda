...
ss::future<http::client::response_stream_ref> s3_client::do_get_object(
  ...
  bool expect_no_such_key) {
    ...
    if (result != boost::beast::http::status::ok) {
        // Got error response, consume the response body and produce rest api
        // error
        if (
          expect_no_such_key
          && result == boost::beast::http::status::not_found) {
            ...
            vlog(
              s3_log.warn, "S3 replied with error: {:l}", ref->get_headers());
        }
        auto res = co_await util::drain_response_stream(std::move(ref));
        co_return co_await parse_rest_error_response<
          http::client::response_stream_ref>(result, std::move(res));
    }
    co_return ref;
}
