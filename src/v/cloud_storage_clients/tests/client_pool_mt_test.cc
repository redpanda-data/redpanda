...
SEASTAR_THREAD_TEST_CASE(test_client_pool_acquire_after_leasing_all) {
    ...
    ss::smp::submit_to(1, [&pool, &leases, num_connections_per_shard]() {
        auto r = boost::irange(0ul, num_connections_per_shard);
        return ss::do_for_each(r, [&pool, &leases](auto) {
            return pool.local()
              .acquire(leases.local().as)
              .then([&leases](auto c) {
                  leases.local().leases.push_back(std::move(c));
              });
        });
    }).get();
    ...
    ss::smp::submit_to(
      1,
      [&pool_no_bg_ops, &leases, num_connections_per_shard]() {
          auto r = boost::irange(0ul, num_connections_per_shard);
          return ss::do_for_each(r, [&pool_no_bg_ops, &leases](auto) {
              leases.local().leases.pop_back();
              return pool_no_bg_ops();
          });
      })
      .get();
