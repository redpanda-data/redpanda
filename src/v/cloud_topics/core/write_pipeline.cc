...
write_pipeline<Clock>::write_pipeline() {
    ...
    for (size_t i = 0; i < max_pipeline_stages; i++) {
        _stages.push_back(pipeline_stage_id(i));
    }
}
...
template<class Clock>
pipeline_stage write_pipeline<Clock>::next_stage(pipeline_stage old) const {
    if (old == unassigned_pipeline_stage) {
        return first_stage();
    }
    auto next_ix = old()->get_numeric_id() + 1;
    vassert(
...
template<class Clock>
pipeline_stage write_pipeline<Clock>::first_stage() const {
...
pipeline_stage write_pipeline<Clock>::register_pipeline_stage() noexcept {
    auto ix = _next_stage_id_to_alloc++;
    auto ptr = &_stages.at(ix);
    return pipeline_stage(ptr);
}
