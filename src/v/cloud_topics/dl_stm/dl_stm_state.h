...
struct overlay_collection {
    bool operator==(const overlay_collection&) const noexcept = default;

    void append(const dl_overlay&) {}

    std::optional<dl_overlay> lower_bound(kafka::offset) const {
        return std::nullopt;
    }
};
...
public:
    ...
    auto serde_fields() {
        return std::tie(_insync_offset, _last_reconciled_offset, _term_map);
    }
