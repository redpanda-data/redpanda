...
struct maintenance_cluster_state {
    model::node_id self;
...
public:
    ...
    // `maintenance_scheduler` will no longer receive updates on which CTPs
...
protected:
    ...
    // registration/deregistration of logs with the `maintenance_scheduler`. For
    // example, setting up a callback with a cluster object that pushes newly
    // managed CTPs to the `maintenance_scheduler`.
    virtual ss::future<> start_collecting_logs() = 0;

    // Called during tear-down. Stops the `log_collector` from making further
    // registrations/deregistrations of logs with the `maintenance_scheduler`
...
// on this broker (irrespective of shard) to the `maintenance_scheduler`.
class partition_leader_log_collector : public log_collector {
public:
    // Typedefs for necessary functions to call into the
    // `maintenance_scheduler`.
...
private:
    // Registers/unregisters `ntp`s with the `maintenance_scheduler` using
    ...
    // `maintenance_scheduler::unmanage_partition()`).
    ...
    // Callback to register a partition with the `maintenance_scheduler`.
    manage_cb_t _manage_cb;

    // Callback to deregister a partition with the `maintenance_scheduler`.
