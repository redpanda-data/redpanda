...
public:
    ...
    ss::future<> start_scheduler() {
        ...
        // not `std::make_unique` because private `maintenance_scheduler` c-tor.
        scheduler = std::unique_ptr<l1::maintenance_scheduler>(
          new l1::maintenance_scheduler(std::move(info_collector)));
...
protected:
    std::unique_ptr<l1::maintenance_scheduler> scheduler{nullptr};
};
