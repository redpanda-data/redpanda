...
public:
    ss::future<> start_workers(l1::worker_manager& manager) {
        co_await manager._workers.start(
          ...
          nullptr);
        co_await manager._workers.invoke_on_all(&l1::maintenance_worker::start);
    }

    ss::future<l1::maintenance_worker::worker_state>
    get_worker_state(l1::worker_manager& manager, ss::shard_id shard) {
        return manager._workers.invoke_on(
          shard,
          [](l1::maintenance_worker& worker) { return worker._worker_state; });
    }
...
TEST_F(WorkerManagerTestFixture, AcquireWork) {
    auto cmp_func = [](
                      ...
                      const l1::log_maintenance_meta_ptr& b) {
        return a->ntp < b->ntp;
    };
    ...
    auto meta = ss::make_lw_shared<l1::log_maintenance_meta>(
      test_tidp, test_ntp);
    list.push_back(*meta);
