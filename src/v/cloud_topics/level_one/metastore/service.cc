...
ss::future<get_leveling_info_reply> service::get_leveling_info(
  get_leveling_info_request request, ::rpc::streaming_context&) {
    return _leader_router->local().get_leveling_info(
      std::move(request), leader_router::local_only::yes);
}
