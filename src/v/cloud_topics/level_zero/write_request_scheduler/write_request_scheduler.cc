...
write_request_scheduler::write_request_scheduler(write_pipeline<>::stage s)
  : _stage(s)
  , _max_buffer_size(
      config::shard_local_cfg().cloud_topics_l0_data_threshold.bind())
  , _max_cardinality(
      config::shard_local_cfg().cloud_topics_l0_cardinality_threshold.bind())
  , _scheduling_interval(
      config::shard_local_cfg().cloud_topics_l0_upload_interval.bind()) {}
