...
ss::future<> reconciler::commit_object(const object_range_info& range) {
    /*
     * build and replicate a overlay batch to the underlying partition that
     * records the range of data in the newly uploaded L1 object.
     */
    auto reader = [] {
        /*
         * TODO: as you can see we are replicating a version_fence batch. this
         * is a temporary replacement for the real overlay batch type.
         */
        storage::record_batch_builder builder(
          model::record_batch_type::version_fence, model::offset(0));
        builder.add_raw_kv(
          serde::to_iobuf(
            ss::sstring(features::feature_table::version_fence_batch_key)),
          serde::to_iobuf(features::feature_table::version_fence()));
        auto batch = std::move(builder).build();
        return model::make_memory_record_batch_reader(std::move(batch));
    }();

    auto result = co_await range.partition->partition->replicate(
      std::move(reader),
    ...
    if (result.has_error()) {
        vlog(
          lg.warn,
          "Failed to replicate overlay batch to {}: {}",
