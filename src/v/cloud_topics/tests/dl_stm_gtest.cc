...
TEST(overlay_collection_test, test_append1) {
    ...
    ct::detail::dl_stm_state overlays;
...
TEST(overlay_collection_test, sorted_run_serialization_roundtrip) {
    ct::detail::sorted_run_t original;
    auto gen = generate_monotonic_range(
      kafka::offset(0),
      kafka::offset(1000),
      10,
    ...
    auto restored = serde::from_iobuf<ct::detail::sorted_run_t>(std::move(buf));

    ASSERT_TRUE(original == restored);
}

TEST(overlay_collection_test, overlay_collection_serialization_roundtrip) {
    ct::detail::overlay_collection original;
    auto gen = generate_monotonic_range(
      kafka::offset(0),
      kafka::offset(1000),
      10,
    ...
    auto buf = serde::to_iobuf(original);
    auto restored = serde::from_iobuf<ct::detail::overlay_collection>(
      std::move(buf));
...
TEST(overlay_collection_test, dl_stm_state_serialization_roundtrip) {
    ct::detail::dl_stm_state original;
    auto gen = generate_monotonic_range(
      kafka::offset(0),
      kafka::offset(1000),
      10,
    ...
    auto buf = serde::to_iobuf(original);
    auto restored = serde::from_iobuf<ct::detail::dl_stm_state>(std::move(buf));
...
struct dl_stm_node {
    ...
    ss::shared_ptr<cloud_topics::dl_stm> dl_stm;
};
...
public:
    ...
    cloud_topics::dl_stm& get_leader_stm() {
