...
ss::future<std::error_code> ntp_archiver::process_anomalies(
  ...
  cloud_storage::anomalies detected) {
    auto runid = random_generators::get_int(
      std::numeric_limits<int64_t>::min(), std::numeric_limits<int64_t>::max());
    ssx::watchdog wd(100s, [this, runid] {
        vlog(_rtclog.error, "NEEDLE {} process_anomalies _mutex", runid);
    });
...
ss::future<> ntp_archiver::upload_until_term_change_legacy() {
    ...
    while (may_begin_uploads()) {
        ...
        auto wd = std::make_optional<ssx::watchdog>(100s, [this, runid] {
            vlog(
              _rtclog.error,
              "NEEDLE {} upload_until_term_change timed out waiting for "
              "_uploads_active(2)",
...
ss::future<cloud_storage::upload_result> ntp_archiver::do_upload_segment(
  ...
  int64_t runid) {
    ...
    auto reset_func = [candidate, &stream_state, runid, &ctxlog] {
        ...
        if (stream_state.has_value()) {
            vlog(ctxlog.debug, "First 'reset_func' call {}", runid);
            ...
            vlog(ctxlog.debug, "Second+ 'reset_func' call {}", runid);
...
ss::future<ntp_archiver_upload_result> ntp_archiver::upload_segment(
  ...
  std::optional<std::reference_wrapper<retry_chain_node>> source_rtc) {
    ...
    auto upload_fut = do_upload_segment(
      path, candidate, std::move(stream_upload), source_rtc, runid);
    ...
    ssx::watchdog w(100s, [this, runid] {
        vlog(
          _rtclog.error,
          "NEEDLE {} stuck uploading segment or generating index {}",
          runid,
          _parent.ntp());
    });
...
ntp_archiver::make_segment_index(
  ...
  int64_t runid) {
    ...
    vlog(
      ctxlog.debug,
      "creating remote segment index: {}, run {}",
      index_path,
      runid);
    ...
    auto res = co_await builder->consume().finally([&ctxlog, &builder, runid] {
        vlog(ctxlog.debug, "closing index builder, run {}", runid);
...
ntp_archiver::replicate_archival_metadata(
  ...
  std::vector<wait_uploads_complete_result> finished_uploads) {
    ...
    vlog(
      _rtclog.debug,
      "{} Replicating {} finished uploads",
      runid,
      finished_uploads.size());
    ...
    if (total.num_succeeded != 0) {
        ...
        ssx::watchdog wd(60s, [this, runid] {
            vlog(
              _rtclog.error,
...
ss::future<ntp_archiver::batch_result> ntp_archiver::upload_next_candidates(
  ...
  std::optional<model::offset> unsafe_max_offset_override_exclusive) {
    ...
    try {
        ssx::watchdog wd_upload_segments(100s, [this, runid] {
...
ss::future<> ntp_archiver::housekeeping() {
    ...
    try {
        if (may_begin_uploads()) {
            ...
            // and retention/GC
            ssx::watchdog wd_housekeeping(150s, [this, runid] {
                vlog(
                  _rtclog.error,
...
ss::future<> ntp_archiver::garbage_collect_archive() {
    ...
    retry_chain_node fib(
      ...
      _conf->cloud_storage_initial_backoff(),
      &_rtcnode);
    ssx::watchdog w(150s, [this, runid] {
        vlog(
          _rtclog.error,
...
ss::future<bool> ntp_archiver::do_upload_local(
  ...
  std::optional<std::reference_wrapper<retry_chain_node>> source_rtc) {
    ...
    auto runid = random_generators::get_int(
      std::numeric_limits<int64_t>::min(), std::numeric_limits<int64_t>::max());
    vlog(_rtclog.debug, "{} Uploading segment {}", runid, upload.exposed_name);
    ssx::watchdog wd_upload_segments(100s, [this, runid] {
