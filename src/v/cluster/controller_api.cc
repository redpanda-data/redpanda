...
controller_api::get_node_decommission_progress(
  model::node_id id, model::timeout_clock::time_point timeout) {
    ...
    auto node = _members.local().get_broker(id);
    ...
    if ((*node)->get_membership_state() == model::membership_state::removed) {
        ret.finished = true;
        co_return ret;
    }

    if ((*node)->get_membership_state() != model::membership_state::draining) {
