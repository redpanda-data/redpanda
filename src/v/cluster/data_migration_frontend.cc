...
data_migration_frontend::data_migration_frontend(
  model::node_id self,
...
auto data_migration_frontend::process_or_dispatch(
  Request req,
...
bool data_migration_frontend::data_migrations_active() const {
    return _features.local().is_active(features::feature::data_migrations);
}

ss::future<result<data_migration_id>> data_migration_frontend::create_migration(
  data_migration migration, can_dispatch_to_leader can_dispatch) {
    if (!data_migrations_active()) {
        return ssx::now<result<data_migration_id>>(errc::feature_disabled);
    }
    ...
    return process_or_dispatch<
      ...
      create_data_migration_request{.migration = std::move(migration)},
      can_dispatch,
      [timeout = _operation_timeout](
        create_data_migration_request req,
        data_migrations_client_protocol& client) {
          return client.create_data_migration(
            std::move(req), rpc::client_opts(timeout));
      },
      [this](create_data_migration_request req) {
          return container().invoke_on(
            data_migrations_shard,
            [req = std::move(req)](data_migration_frontend& local) mutable {
                return local.do_create_migration(std::move(req.migration));
            });
      },
      [](result<create_data_migration_reply> reply)
        -> result<data_migration_id> {
          if (reply.has_error()) {
              return reply.error();
          }
...
ss::future<std::error_code> data_migration_frontend::update_migration_state(
  ...
  can_dispatch_to_leader can_dispatch) {
    ...
    return process_or_dispatch<
      ...
      update_data_migration_state_request{.id = id, .state = state},
      can_dispatch,
      [timeout = _operation_timeout](
        update_data_migration_state_request req,
        data_migrations_client_protocol& client) {
          return client.update_data_migration_state(
            std::move(req), rpc::client_opts(timeout));
      },
      [this](update_data_migration_state_request req) {
          return container().invoke_on(
            data_migrations_shard,
            [req](data_migration_frontend& local) mutable {
                return local.do_update_migration_state(req.id, req.state);
            });
      },
      [](result<update_data_migration_state_reply> reply) -> std::error_code {
          if (reply.has_error()) {
              return reply.error();
          }
          if (reply.assume_value().ec != errc::success) {
...
ss::future<std::error_code> data_migration_frontend::remove_migration(
  data_migration_id id, can_dispatch_to_leader can_dispatch) {
    if (!data_migrations_active()) {
        return ssx::now<std::error_code>(errc::feature_disabled);
    }
    ...
    return process_or_dispatch<
      ...
      remove_data_migration_request{.id = id},
      can_dispatch,
      [timeout = _operation_timeout](
        remove_data_migration_request req,
        data_migrations_client_protocol& client) {
          return client.remove_data_migration(
            std::move(req), rpc::client_opts(timeout));
      },
      [this](remove_data_migration_request req) {
          return container().invoke_on(
            data_migrations_shard,
            [req](data_migration_frontend& local) mutable {
      ...
      [](result<remove_data_migration_reply> reply) -> std::error_code {
          if (reply.has_error()) {
              return reply.error();
          }
          if (reply.assume_value().ec != errc::success) {
...
data_migration_frontend::do_create_migration(data_migration migration) {
     ...
     */
    if (data_migration_table::is_empty_migration(migration)) {
    ...
    ec = co_await replicate_and_wait(
      ...
      create_data_migration_cmd(
        ...
        create_data_migration_cmd_data{
          .id = id, .migration = std::move(migration)}),
      _operation_timeout + model::timeout_clock::now());
    if (ec) {
        co_return ec;
    }
    co_return result<data_migration_id>(id);
}

ss::future<chunked_vector<data_migration_metadata>>
data_migration_frontend::list_migrations() {
    return container().invoke_on(
      data_migrations_shard, [](data_migration_frontend& local) {
          return local._table.list_migrations();
      });
}

ss::future<std::error_code> data_migration_frontend::insert_barrier() {
...
ss::future<std::error_code> data_migration_frontend::do_update_migration_state(
  data_migration_id id, data_migration_state state) {
    validate_migration_shard();
    auto ec = co_await insert_barrier();
    if (ec) {
        co_return ec;
    ...
    if (!data_migration_table::is_valid_state_transition(
...
data_migration_frontend::do_remove_migration(data_migration_id id) {
    validate_migration_shard();
    auto ec = co_await insert_barrier();
    if (ec) {
        co_return ec;
    ...
    ec = co_await replicate_and_wait(
      _controller,
      _as,
      remove_data_migration_cmd(0, remove_data_migration_cmd_data{.id = id}),
      _operation_timeout + model::timeout_clock::now());
