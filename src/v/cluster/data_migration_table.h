...
public:
    ...
    using notification_callback
      = ss::noncopyable_function<void(data_migration_id)>;
    ...
    std::optional<std::reference_wrapper<const data_migration_metadata>>
      get_migration(data_migration_id) const;
     ...
     */
    chunked_vector<data_migration_metadata> list_migrations() const {
        chunked_vector<data_migration_metadata> ret;
...
private:
    friend class data_migration_frontend;
    ...
    data_migration_id get_next_id() { return _next_id++; }
    void notify_callbacks(data_migration_id id);

    std::optional<validation_error>
    validate_migrated_resources(const inbound_data_migration&) const;
    std::optional<validation_error>
    validate_migrated_resources(const outbound_data_migration&) const;

    static bool is_valid_state_transition(
      data_migration_state current_state, data_migration_state target_state);
...
private:
    ...
    absl::node_hash_map<data_migration_id, data_migration_metadata> _migrations;
    ss::sharded<data_migrated_resources>& _resources;
