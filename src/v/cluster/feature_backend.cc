...
feature_backend::apply_update(model::record_batch b) {
    ...
    // Special case for systems pre-dating the original_version field: they may
    // have loaded a snapshot that doesn't contain an original version, so must
    // populate it from updates.
    if (
      ...
      == cluster::invalid_version) {
        co_await ss::visit(
          ...
          [this](feature_update_cmd update) {
              vlog(
                clusterlog.info,
                "No original version detected, using logical version {}",
                update.key.logical_version);
              return _feature_table.invoke_on_all(
                [v = update.key.logical_version](features::feature_table& ft) {
