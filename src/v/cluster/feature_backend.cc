...
feature_backend::apply_update(model::record_batch b) {
    ...
    co_await ss::visit(
      ...
      [this](feature_update_license_update_cmd update) {
          const auto& existing_license = _feature_table.local().get_license();
          auto& new_license = update.key.redpanda_license;
          if (
            existing_license
            && existing_license->source != security::source_type::autogenerated
            && new_license.source == security::source_type::autogenerated) {
              // Ignore. An autogenerated license should never overwrite a
              // non-autogenerated license
              return ss::now();
          }
          return _feature_table.invoke_on_all(
            [license = std::move(new_license)](
