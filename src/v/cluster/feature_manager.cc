        ...
        return ss::do_until(
          [this] {
              return _as.local().abort_requested()
                     || _feature_table.local().get_license().has_value();
          },
          [this] {
              return try_insert_trial_license().then(
                []() { return ss::sleep(1s); });
          });
    });
...
make_builtin_trial_license(security::license::clock::time_point start_time) {
    ...
    return security::license{
      ...
      .expiry = expiry,
      .checksum = "",
      .source = security::source_type::autogenerated,
    };
}

} // namespace

ss::future<> feature_manager::try_insert_trial_license() {
    vlog(clusterlog.info, "Trying to install a trial license");
    if (!_am_controller_leader) {
        co_return;
    }

    // The barrier is not required for correctness, but it reduces the chance of
    // creating trial licenses that are going to be ignored in the backend
    constexpr auto operation_timeout = 10s;
    auto deadline = model::timeout_clock::now() + operation_timeout;

    auto barrier_result = co_await _stm.local().insert_linearizable_barrier(
      deadline);
    if (!barrier_result) {
        vlog(
          clusterlog.debug,
          "Failed to insert linearizable barrier: {}",
    ...
    if (_feature_table.local().get_license().has_value()) {
        co_return;
    }

    const auto& cluster_info = _stm.local().get_metrics_reporter_cluster_info();
    if (!cluster_info.is_initialized()) {
        vlog(
          clusterlog.debug,
          "Waiting for the cluster info to be initialized before installing "
          "the evaluation period license");
        co_return;
    }

    // Note that there is a small chance here of racing with a concurrent
    // `update_license` which has already been issued but not yet observed
    // in the backend. Breaking this race is handled in the feature_backend by
    // preventing an autogenerated license from overwriting a user-configured
    // license. This assumes the cluster is created with all brokers having this
    // change (which is true if we assume that all brokers have the same version
    // on bootstrap).
    auto errc = co_await update_license(make_builtin_trial_license(
      model::to_time_point(cluster_info.creation_timestamp)));
    if (errc) {
        vlog(clusterlog.debug, "Failed to update trial license: {}", errc);
        co_return;
    }

    vlog(clusterlog.info, "Successfully installed a trial license");
}
