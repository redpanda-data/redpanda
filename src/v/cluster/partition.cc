...
bool partition::should_construct_archiver() {
    ...
    // Cloud topic: keep the archiver alive only while there are pre-migration
    // TS segments that still need GC via apply_retention() + garbage_collect().
    // Uploads are suppressed in the upload loop (see ntp_archiver), so the
    // archiver does GC-only work here. Once the archival manifest is drained,
    // this returns false and the archiver is not rebuilt on the next
    // restart_archiver() call -- which is also when the migration seal is
    // completed and the partition becomes a native cloud topic.
    return _archival_meta_stm != nullptr
           && !_archival_meta_stm->manifest().empty();
}
