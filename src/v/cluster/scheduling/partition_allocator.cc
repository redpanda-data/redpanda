...
bool guess_is_memory_group_aware_memory_limit() {
    ...
    // MiB and later reduced to 4MiB. Both of those weren't really factual and
    // rather set such that they about align with `topic_partitions_per_shard`.
    //
    ...
    // Previous default was 4MiB (and even higher before that). New one is more
...
std::error_code partition_allocator::check_memory_limits(
  ...
  uint64_t effective_cluster_memory) const {
    ...
    if (is_memory_aware) {
        memory_limit = effective_cluster_memory
