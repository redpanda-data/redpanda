...
namespace cluster::sloth_mail::tests::kinds {
/*
 * Mail kinds for testing.
 *
...
struct test_kind_0 {
    ...
    static void merge(const key_t&, value_t& value, const value_t& new_value) {
        value.value += " and " + new_value.value;
    }

    constexpr static size_t entry_size = 100;
};

struct test_kind_1 {
    static constexpr kind_id id{1};
...
namespace std {
template<>
struct hash<cluster::sloth_mail::tests::kinds::test_kind_0::key_t> {
    size_t operator()(
      const cluster::sloth_mail::tests::kinds::test_kind_0::key_t& k) const {
        return std::hash<int>()(k.key);
    }
};
template<>
struct hash<cluster::sloth_mail::tests::kinds::test_kind_1::key_t> {
    size_t operator()(
      const cluster::sloth_mail::tests::kinds::test_kind_1::key_t& k) const {
        return std::hash<ss::sstring>()(k.key);
    }
};
...
using supported_kinds = std::variant<kinds::test_kind_0, kinds::test_kind_1>;
} // namespace cluster::sloth_mail::tests
