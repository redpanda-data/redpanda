...
namespace cluster::testing_details {
class data_migration_table_test_accessor {
public:
    explicit data_migration_table_test_accessor(
      cluster::data_migration_table& t)
      : _t(t) {}

    data_migration_id get_next_id() { return _t.get_next_id(); }

private:
    cluster::data_migration_table& _t;
};
} // namespace cluster::testing_details
...
chunked_vector<cluster::inbound_topic>
create_inbound_topics(std::vector<std::string_view> topics) {
    chunked_vector<cluster::inbound_topic> ret;
    ret.reserve(topics.size());
    for (auto& t : topics) {
        ret.push_back(cluster::inbound_topic{
...
static chunked_vector<cluster::consumer_group>
create_groups(std::vector<std::string_view> strings) {
    chunked_vector<cluster::consumer_group> ret;
    std::ranges::transform(
      strings, std::back_inserter(ret), [](std::string_view s) {
          return cluster::consumer_group(s);
      });
    return ret;
}
...
struct data_migration_table_fixture : public seastar_test {
    ss::future<> SetUpAsync() override {
        ...
        table = std::make_unique<cluster::data_migration_table>(
          resources, topics);
        table->register_notification([this](cluster::data_migration_id id) {
    ...
    data_migration_id get_next_id() {
        return testing_details::data_migration_table_test_accessor(*table)
          .get_next_id();
    }
    ...
    ss::future<std::error_code> apply_update_state(
      cluster::data_migration_id id, cluster::data_migration_state state) {
        return table->apply_update(
          create_cmd<cluster::update_data_migration_state_cmd>(
            cluster::update_data_migration_state_cmd_data{
              .id = id, .requested_state = state}));
    }

    ss::future<std::error_code>
    apply_remove_migration(cluster::data_migration_id id) {
        return table->apply_update(
          create_cmd<cluster::remove_data_migration_cmd>(
            remove_data_migration_cmd_data{.id = id}));
    }

    ss::future<std::error_code> progress_through_states(
      cluster::data_migration_id id,
      std::vector<cluster::data_migration_state> states) {
    ...
    ss::future<result<data_migration_id>>
    try_create_migration(data_migration m) {
        auto id = get_next_id();
        auto ec = co_await table->apply_update(
          create_cmd<cluster::create_data_migration_cmd>(
            cluster::create_data_migration_cmd_data{
              .id = id,
    ...
    ss::future<cluster::data_migration_id> create_migration(data_migration m) {
        auto r = co_await try_create_migration(std::move(m));
        EXPECT_TRUE(r.has_value());
        co_return r.value();
    }

    ss::future<cluster::data_migration_id> create_simple_migration() {
        auto topics = create_topic_vector({"t-1", "t-2", "t-3"});
        co_await populate_topic_table_with_topics(topics);
        co_return co_await create_migration(cluster::outbound_data_migration{
          .topics = std::move(topics),
          .groups = create_groups({"g-1", "g-2"})});
    }

    void validate_group_resource_state(
      const std::vector<std::pair<ss::sstring, migrated_resource_state>>&
        expected_states) {
        for (auto& expected : expected_states) {
            EXPECT_EQ(
              resources.local().get_group_state(consumer_group{expected.first}),
              expected.second);
        }
    }
    ...
    cluster::data_migration_table::notification_id notification_id;
    ss::sharded<cluster::topic_table> topics;
    ss::sharded<cluster::data_migrated_resources> resources;
    std::unique_ptr<cluster::data_migration_table> table;

    std::vector<cluster::data_migration_id> notifications;
...
TEST_F_CORO(data_migration_table_fixture, test_crud_operations) {
    ...
    validate_group_resource_state(
      {{"g-1", migrated_resource_state::non_restricted},
       {"g-2", migrated_resource_state::non_restricted}});

    validate_topic_resource_state(
      {{"t-1", migrated_resource_state::non_restricted},
       {"t-2", migrated_resource_state::non_restricted},
       {"t-3", migrated_resource_state::non_restricted}});

    auto r = co_await table->apply_update(
      create_cmd<cluster::create_data_migration_cmd>(
        cluster::create_data_migration_cmd_data{
          .id = id_1,
          .migration = cluster::outbound_data_migration{
            .topics = topics.copy(),
            .groups = create_groups({"g-1", "g-2"})}}));
    ...
    validate_group_resource_state(
      {{"g-1", migrated_resource_state::restricted},
       {"g-2", migrated_resource_state::restricted}});

    validate_topic_resource_state(
      {{"t-1", migrated_resource_state::restricted},
       {"t-2", migrated_resource_state::restricted},
       {"t-3", migrated_resource_state::restricted}});
    ...
    r = co_await table->apply_update(
      create_cmd<cluster::create_data_migration_cmd>(
        cluster::create_data_migration_cmd_data{
          .id = id_1,
          .migration = cluster::inbound_data_migration{
    ...
    validate_group_resource_state(
      {{"g-3", migrated_resource_state::blocked},
       {"g-4", migrated_resource_state::blocked}});

    validate_topic_resource_state(
      {{"in-t-1", migrated_resource_state::blocked}});
    ...
    r = co_await apply_remove_migration(cluster::data_migration_id(10));
    ...
    r = co_await table->apply_update(
      create_cmd<cluster::update_data_migration_state_cmd>(
        cluster::update_data_migration_state_cmd_data{
          .id = id_1,
          .requested_state = cluster::data_migration_state::preparing}));
    EXPECT_EQ(r, cluster::errc::data_migration_not_exists);
    EXPECT_EQ(notifications.size(), 3);
    ...
    EXPECT_EQ(
      ...
      cluster::data_migration_state::preparing);
}
...
TEST_F_CORO(data_migration_table_fixture, test_migration_stm_happy_path) {
    ...
    r = co_await progress_through_states(
      ...
      {
        cluster::data_migration_state::preparing,
        cluster::data_migration_state::prepared,
        cluster::data_migration_state::executing,
        cluster::data_migration_state::executed,
        cluster::data_migration_state::finished,
      });
...
TEST_F_CORO(data_migration_table_fixture, test_invalid_transition) {
    ...
    r = co_await progress_through_states(
      id_1,
      {cluster::data_migration_state::canceling,
       cluster::data_migration_state::cancelled});
...
TEST_F_CORO(data_migration_table_fixture, test_resource_validation) {
    ...
    inbound_data_migration idm_with_alias{
    ...
    validate_group_resource_state({
      {"gr-1", migrated_resource_state::restricted},
      {"gr-2", migrated_resource_state::restricted},
      {"gr-3", migrated_resource_state::restricted},
    });

    validate_topic_resource_state({
      {"topic-1", migrated_resource_state::restricted},
      {"topic-2", migrated_resource_state::restricted},
      {"topic-3", migrated_resource_state::restricted},
    });
    ...
    EXPECT_TRUE(r.has_value());
    validate_topic_resource_state({
      {"alias-of-topic-1", migrated_resource_state::blocked},
    });
}
