...
public:
    ...
    std::optional<application*> get_leader_node_application() {
        auto c = *apps.begin();
        auto leader
          = c.second->controller->get_partition_leaders().local().get_leader(
    ...
    ss::future<> wait_for_node_decommissioned(int node) {
        ...
        return tests::cooperative_spin_wait_with_timeout(20s, [this, id] {
            ...
            return (*leader)
              ...
              .local()
              .decommission_nodes({id})
              .then([id](result<cluster::decommissioning_status> result) {
                  if (!result) {
                      test_logger.info(
                        "waiting for node {}", result.error().message());
                      return false;
                  }
                  test_logger.info(
                    "waiting for node {}", result.value().nodes[0].second);
                  auto it = std::find_if(
                    result.value().nodes.begin(),
                    result.value().nodes.end(),
                    [id](std::pair<
                         model::node_id,
                         cluster::decommissioning_status::state> p) {
                        return id == p.first;
                    });
                  if (it == result.value().nodes.end()) {
                      return false;
                  }

                  return it->second
                         == cluster::decommissioning_status::state::
...
FIXTURE_TEST(
  test_single_node_decomissioning, nodes_decomissioning_test_fixture) {
    ...
    auto res = (*get_leader_node_application())
                 ->controller->get_members_manager()
                 .local()
                 .decommission_nodes({model::node_id(0)})
                 .get0();
    // we expect that decommissioning will be in progress since we haven't
    // yet added any new nodes
    BOOST_REQUIRE(res.has_value());
    auto v = res.value();
    BOOST_REQUIRE_EQUAL(v.nodes.size(), 1);
    BOOST_TEST(
      v.nodes[0].second
      == cluster::decommissioning_status::state::decommission_in_progress);
    add_node(10);
...
FIXTURE_TEST(test_two_nodes_decomissioning, nodes_decomissioning_test_fixture) {
    ...
    auto res = (*get_leader_node_application())
                 ->controller->get_members_manager()
                 .local()
                 .decommission_nodes({model::node_id(0), model::node_id(1)})
                 .get0();
    // we expect that decommissioning will be in progress since we haven't
    // yet added any new nodes
    BOOST_REQUIRE(res.has_value());
    auto v = res.value();
    BOOST_REQUIRE_EQUAL(v.nodes.size(), 2);
    BOOST_TEST(
      v.nodes[0].second
      == cluster::decommissioning_status::state::decommission_in_progress);
    BOOST_TEST(
