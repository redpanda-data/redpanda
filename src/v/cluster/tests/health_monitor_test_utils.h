...
make_partition_statues(size_t num_partitions) {
    ...
    for (size_t i = 0; i < num_partitions; ++i) {
        ...
        part_status.leader_id = model::node_id(
          random_generators::get_int<int32_t>(10));
        part_status.reclaimable_size_bytes
          = random_generators::get_int<size_t>();
        part_status.revision_id = tests::random_named_int<model::revision_id>();
        part_status.term = tests::random_named_int<model::term_id>();
        part_status.size_bytes = random_generators::get_int<size_t>(20000000);
        part_status.under_replicated_replicas
          = random_generators::get_int<uint8_t>(5);
