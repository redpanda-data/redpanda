...
model::topic_namespace create_topic_namespace(ss::sstring tp) {
...
SEASTAR_THREAD_TEST_CASE(test_getting_not_existing_topic_metatadata) {
    cluster::metadata_cache cache;
    auto md = cache.get_topic_metadata(create_topic_namespace("test_topic"));
...
SEASTAR_THREAD_TEST_CASE(test_getting_topic_metadata) {
    ...
    BOOST_REQUIRE_EQUAL(md->tp_ns.tp(), "test_topic");
}
...
SEASTAR_THREAD_TEST_CASE(test_getting_topics_list) {
    ...
    auto find_topic = [&all_topics](const model::topic& tp) {
        return std::find_if(
          all_topics.begin(),
          all_topics.end(),
          [&tp](const model::topic_namespace& tp_ns) {
              return tp_ns.tp == tp;
          });
    };
...
SEASTAR_THREAD_TEST_CASE(test_updating_partition_leader) {
    ...
    cache.update_partition_leader(
      model::ntp(test_ns, model::topic("test_topic"), model::partition_id(0)),
      model::term_id{1},
      model::node_id(1));
...
SEASTAR_THREAD_TEST_CASE(print) {
    auto tp_ns = create_topic_namespace("test_topic");
    std::cout << tp_ns << std::endl;
}
