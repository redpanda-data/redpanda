...
public:
    explicit partition_balancer_sim_fixture(bool rack_awareness_enabled = true)
      : _workers(rack_awareness_enabled) {}
    ...
    const cluster::partition_balancer_state& balancer_state() const {
        return _workers.state.local();
    }

    cluster::partition_balancer_state& balancer_state() {
        return _workers.state.local();
    }

    /// Violation count observed by the planner during the most recent tick.
    /// Populated by run_balancer() / do_run_balancer(). Zero before any
    /// tick has run.
    size_t last_pinning_violations_count() const {
        return _last_pinning_violations_count;
    }
    ...
    /// Set a node's disk usage ratio. Used by tests to simulate a disk-full
    /// node that the allocator excludes from reallocation destinations.
    void set_node_disk_fill(model::node_id id, double ratio) {
        auto it = _nodes.find(id);
        vassert(it != _nodes.end(), "unknown node id: {}", id);
        it->second.used = static_cast<size_t>(double(it->second.total) * ratio);
    }
    ...
    cluster::partition_balancer_planner::plan_data do_run_balancer() {
        ...
        _last_pinning_violations_count
          = plan_data.last_pinning_violations_count;
...
private:
    ...
    size_t _last_pinning_violations_count = 0;
