...
bool topic_properties::has_overrides() const {
    return cleanup_policy_bitflags || compaction_strategy || segment_size
           ...
           || experimental_datalake_topic.has_value();
}
...
topic_properties::get_ntp_cfg_overrides() const {
    ...
    ret.experimental_datalake_topic = experimental_datalake_topic;
    return ret;
}
...
adl<cluster::incremental_topic_updates>::from(iobuf_parser& in) {
    ...
    if (version <= cluster::incremental_topic_updates::version_with_datalake) {
        updates.experimental_datalake_topic
          = adl<cluster::property_update<std::optional<bool>>>{}.from(in);
    }
