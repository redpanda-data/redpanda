...
bool link_status_reconciler::per_link_reconciler::topic_needs_promotion(
  const ::model::topic& topic) const {
    ...
    // A topic is cloud only if it was created within the override scope
    // (storage.mode is create-only), so keying off the actual current mode
    // here honors the per-topic scope without re-evaluating the (mutable)
