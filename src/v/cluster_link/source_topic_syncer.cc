...
void source_topic_syncer::enqueue_update_mirror_topic_commands(
  ...
  const chunked_vector<kafka::describe_configs_result>& describe_results) {
    ...
    for (const auto& describe_result : describe_results) {
        ...
        if (configs.has_value()) {
            // For links with a storage-mode override the override manages
            // storage.mode at create time, so it must not be reconciled from
            // the source on update: a cloud topic cannot legally transition
            // toward a non-cloud source value, and editing the scope filters
            // must not retroactively churn already-created topics. Links
            // WITHOUT an override follow the source's storage mode as usual.
            if (_config.storage_mode_override.has_value()) {
                configs->erase(
                  ss::sstring(kafka::topic_property_redpanda_storage_mode));
                configs->erase(
