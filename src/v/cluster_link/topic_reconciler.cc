    ...
    _reconciler_timer.set_callback([this] {
        ssx::spawn_with_gate(_gate, [this] {
            return execute()
              ...
              .finally([this] { _reconciler_timer.arm(_run_interval); });
        });
    });
...
ss::future<> topic_reconciler::stop() {
...
ss::future<> topic_reconciler::execute(std::optional<model::id_t> link_id) {
    ...
    try {
        ...
        co_return co_await ss::parallel_for_each(
          link_ids, [this](model::id_t id) { return do_reconcile_topic(id); });
    } catch (const std::exception& e) {
        auto log_level = ssx::is_shutdown_exception(std::current_exception())
                           ? ss::log_level::trace
                           : ss::log_level::warn;
        vlogl(cllog, log_level, "Failure during topic reconciliation: {}", e);
    }
}
...
ss::future<> topic_reconciler::do_reconcile_topic(model::id_t link_id) {
    ...
    auto mirror_topics = link->get().copy().state.mirror_topics;
...
ss::future<> topic_reconciler::maybe_create_mirror_topic(
  ::model::topic topic, model::mirror_topic_metadata mirror_topic_config) {
    ...
    for (const auto& [config_name, config_value] :
         mirror_topic_config.topic_configs) {
        topic_configs.emplace(config_name, config_value);
    }
