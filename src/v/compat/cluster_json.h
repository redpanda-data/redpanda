...
inline void rjson_serialize(
  json::Writer<json::StringBuffer>& w, const cluster::topic_properties& tps) {
    ...
    write_member(w, "total_retention_bytes", tps.total_retention_bytes);
    write_member(w, "total_retention_ms", tps.total_retention_ms);
    w.EndObject();
}
...
inline void read_value(json::Value const& rd, cluster::topic_properties& obj) {
    ...
    read_member(rd, "total_retention_bytes", obj.total_retention_bytes);
    read_member(rd, "total_retention_ms", obj.total_retention_ms);
}
