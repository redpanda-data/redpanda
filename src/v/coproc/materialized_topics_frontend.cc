...
void materialized_topics_frontend::topic_creation_resolved(
  ...
  model::timeout_clock::time_point timeout) {
    ...
    for (auto& topic : topics) {
        auto [itr, success] = _topics.insert(
          {topic.name, ss::make_lw_shared<ss::shared_promise<>>()});
        if (!success) {
    ...
    if (!todos.empty()) {
        auto f = _topics_frontend.local()
                   .create_non_replicable_topics(std::move(todos), timeout)
                   .then([this](std::vector<cluster::topic_result> result) {
                       topic_creation_resolved(std::move(result));
                   });
