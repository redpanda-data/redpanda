...
#include "coproc/tests/fixtures/script_dispatcher_fixture.h"
...
FIXTURE_TEST(test_register, script_dispatcher_fixture) {
    ...
    enable_scripts({{foo_subber, {{foo, tp_stored}}},
                    {bar_subber, {{bar, tp_stored}}},
                    {all_subbed, {{foo, tp_stored}, {bar, tp_earliest}}}})
      .get();
    ...
    /// cannot just assume the solution is the sum of shards_for_topic over
    /// each topic, since topics may reside on the same shard (common case).
    /// The solution is the union of unique shards
...
FIXTURE_TEST(test_deregister, script_dispatcher_fixture) {
    ...
    enable_scripts({{a, {{foo, tp_stored}, {bar, tp_earliest}}},
                    {b, {{bar, tp_stored}}},
                    {c, {{foo, tp_stored}, {bar, tp_stored}, {baz, tp_stored}}},
                    {d, {{foo, tp_stored}, {boom, tp_stored}}}})
      .get();
