...
static const ss::sstring crash_report_suffix{".crash"};
...
ss::future<> service::start() {
    ...
    if (maybe_crash_md) {
        ...
        if (!ok_to_proceed) {
            auto crashes_msg = co_await describe_recent_crashes();
            vlog(
              ctlog.error,
    ...
    // TODO: wrap this ss::sstring to ensure that no allocation is possible
    prepared_cd._crash_reason.resize(4096, '\0');

    // TODO: wrap this std::vector to ensure that no allocation is possible
    prepared_cd._stacktrace.reserve(30);

    // TODO: wrap this iobuf to ensure that no allocation is possible
...
ss::future<ss::sstring> service::describe_recent_crashes() const {
    auto crash_report_dir = config::node().crash_report_dir_path();
    ...
    if (crash_descriptions.empty()) {
        co_return "No recent crashes recorded.";
    }

    std::stringstream ss;
    ss << "The following recent crashes have been recorded:\n";
    for (size_t i = 0; i < crash_descriptions.size(); ++i) {
        ...
        fmt::print(ss, " {}. {}\n", i + 1, crash_descriptions[i]._crash_reason);
    }
...
void service::record_crash(crash_recorder_fn recorder) {
    if (!initialized.load()) {
        constexpr static std::string_view skipping
          = "Skipping recording crash reason to startup_log\n";
    ...
    if (update_crash_md(prepared_cd)) {
        constexpr static std::string_view success
          = "Successfully written crash reason to startup_log\n";
        ss::print_safe(success.data(), success.size());
    } else {
        constexpr static std::string_view failure
          = "Failed to write crash reason to startup_log\n";
...
ss::future<> service::stop() {
    ...
    if (initialized.load()) {
        ::close(fd);
        [[maybe_unused]] auto r = ::remove(crash_report_file_name.c_str());
    }
