...
public:
    explicit impl(digest_type type)
      : _type(type)
      , _md_ctx(internal::EVP_MD_CTX_ptr(EVP_MD_CTX_new())) {
        if (!_md_ctx) {
    ...
    size_t length() const { return length(EVP_MD_CTX_get0_md(_md_ctx.get())); }

    static size_t length(const EVP_MD* md) { return EVP_MD_get_size(md); }
    ...
    bytes finish() {
        bytes digest(bytes::initialized_later(), length());
        finish(digest);
        return digest;
    }
...
size_t digest_ctx::len() const { return _impl->length(); }
size_t digest_ctx::len(digest_type type) {
    return impl::length(internal::get_md(type));
}
