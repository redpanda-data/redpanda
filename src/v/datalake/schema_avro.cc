...
struct state {
    std::deque<avro::NodePtr> type_hierarchy;
...
inner_field_type_from_avro(const avro::NodePtr& node, state& state) {
    ...
    if (state.type_hierarchy.size() >= max_schema_depth) {
        return schema_conversion_exception("Max nesting depth of 100 reached");
    }
    ...
    case avro::AVRO_MAP: {
        // Avro Map is represented as an Iceberg List with key value pairs as we
        // can not guarantee the uniqueness of keys in AVRO_MAP
        if (node->leaves() != 2) {
            return schema_conversion_exception(fmt::format(
        ...
        auto map_id = state.next_field_id();
        ...
        iceberg::struct_type kv_pair_type;
        kv_pair_type.fields.reserve(2);
        kv_pair_type.fields.push_back(iceberg::nested_field::create(
          state.next_field_id(),
          "key",
          iceberg::field_required::yes,
          iceberg::string_type{}));
        ...
        return iceberg::list_type::create(
          map_id, iceberg::field_required::yes, std::move(kv_pair_type));
    }
        ...
        for (size_t i = 0; i < node->leaves(); ++i) {
            ...
            if (field.has_value()) {
                ret.fields.push_back(iceberg::nested_field::create(
                  ...
                  fmt::format(
                    "{}_{}",
                    node->hasName() ? node->name().fullname() : "union",
                    i),
