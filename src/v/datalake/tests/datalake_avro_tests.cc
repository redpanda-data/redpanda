...
iceberg::struct_type
make_key_value(int k_id, int v_id, iceberg::field_type value_type) {
    iceberg::struct_type ret;
    ret.fields.push_back(iceberg::nested_field::create(
      k_id, "key", iceberg::field_required::yes, iceberg::string_type{}));
    ret.fields.push_back(iceberg::nested_field::create(
      v_id, "value", iceberg::field_required::yes, std::move(value_type)));
    return ret;
}
...
TEST(AvroSchema, TestNonRecordSchema) {
    ...
    tl_type_expectations.emplace_back(
      ...
      iceberg::list_type::create(
        ...
        make_key_value(2, 3, iceberg::int_type{})));

    iceberg::struct_type union_st;
    union_st.fields.push_back(iceberg::nested_field::create(
      1, "union_0", iceberg::field_required::no, iceberg::int_type{}));
    union_st.fields.push_back(iceberg::nested_field::create(
      2, "union_1", iceberg::field_required::no, iceberg::long_type{}));
    union_st.fields.push_back(iceberg::nested_field::create(
      3, "union_2", iceberg::field_required::no, iceberg::float_type{}));
...
TEST(AvroSchema, TestRecordType) {
    ...
    ASSERT_TRUE(field_matches(
      ...
      iceberg::list_type::create(
        5,
        iceberg::field_required::yes,
        make_key_value(6, 7, iceberg::int_type{}))));
    ...
    ASSERT_TRUE(field_matches(
      ...
      iceberg::list_type::create(
        9,
        iceberg::field_required::yes,
        make_key_value(13, 14, nested_record(10)))));
    ...
    ASSERT_TRUE(field_matches(
      ...
      "myarray",
      iceberg::list_type::create(
        16, iceberg::field_required::yes, iceberg::double_type{})));
    ASSERT_TRUE(
      field_matches(struct_t.fields[5], 18, "myenum", iceberg::int_type{}));
    ...
    union_struct.fields.push_back(iceberg::nested_field::create(
      ...
      iceberg::list_type::create(
        19,
        iceberg::field_required::yes,
        make_key_value(20, 21, iceberg::int_type{}))));
    union_struct.fields.push_back(iceberg::nested_field::create(
      23, "union_2", iceberg::field_required::no, iceberg::float_type{}));

    ASSERT_TRUE(field_matches(
      struct_t.fields[6], 24, "myunion", std::move(union_struct)));
    iceberg::struct_type another_union_struct;

    another_union_struct.fields.push_back(iceberg::nested_field::create(
      25, "union_0", iceberg::field_required::no, iceberg::binary_type{}));

    ASSERT_TRUE(field_matches(
      struct_t.fields[7], 26, "anotherunion", std::move(another_union_struct)));

    ASSERT_TRUE(
      field_matches(struct_t.fields[8], 27, "mybool", iceberg::boolean_type{}));
    ASSERT_TRUE(field_matches(
      struct_t.fields[9], 31, "anothernested", nested_record(28)));
    ASSERT_TRUE(field_matches(
      struct_t.fields[10], 32, "myfixed", iceberg::fixed_type{.length = 16}));
    ASSERT_TRUE(field_matches(
      struct_t.fields[11], 33, "anotherint", iceberg::int_type{}));
    ASSERT_TRUE(
      field_matches(struct_t.fields[12], 34, "bytes", iceberg::binary_type{}));
