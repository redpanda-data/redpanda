  ...
  , _reclaimer(
    ...
    _metrics.add_group(
      ...
      {sm::make_derive(
       ...
       sm::make_gauge(
         "disk_latency_ema",
         [this] { return _stats.disk_latency.get(); },
...
page_cache_file_idx* page_cache::index(uint32_t file) {
    auto it = _files.find(file);
    if (it == _files.end()) {
        it = _files.emplace(file, std::make_unique<page_cache_file_idx>(file))
               .first;
    }
...
page_cache_result* page_cache::try_get(page_cache_request r) {
    ...
    if (auto ret = ptr->range(r.begin_pageno); ret) {
        if (ret->pending_reads > 0) {
            ...
            //
            // as is, this is just a soft number, still useful but needs
            // improvement
            ret->pending_reads--;
        }
        return ret;
...
void page_cache::cache(uint32_t fileid, page_range_ptr ptr) {
    ...
    return with_semaphore(_mngr.lock(r.file_id, clamp), 1, [=] {
        return _mngr.allocate(clamp.first, prio)
          .then([this, r, clamp](page_range_ptr range_ptr) {
              ...
              return r.fptr
                ->dma_read(
                  page_offset, ptr->data.data(), ptr->data.size(), r.pc)
                .then([this,
                       r,
...
// evictable
void page_cache::prefetch_next(page_cache_request r) {
    ...
    for (int i = 0; i < kStridesOf256KB; ++i) {
        ...
        if (i != 0) {
            prio = page_cache_result::priority::low;
        }
        if (r.begin_pageno >= r.hint_file_last_pageno) {
            break;
        }
