...
constexpr std::array<std::pair<int32_t, int32_t>, 8192> generate_clamp_table() {
  ...
  page_cache_table_clamp_page(int32_t pageno) {
    ...
    struct clamp_comparator final {
        constexpr bool operator()(
          const int32_t& pageno, const std::pair<int32_t, int32_t>& p) const {
            return pageno < (p.second - 1);
        }
        constexpr bool operator()(
          const std::pair<int32_t, int32_t>& p, const int32_t& pageno) const {
            return (p.second - 1) < pageno;
        }
    };

    return *std::lower_bound(
      kPageCacheClampTable.begin(),
      kPageCacheClampTable.end(),
      pageno,
      clamp_comparator{});
}
