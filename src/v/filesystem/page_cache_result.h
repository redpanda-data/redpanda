...
struct page_cache_result {
    ...
    explicit page_cache_result(
      int32_t first_page, gsl::span<char> payload, priority p)
      : begin_pageno(first_page)
      , data(payload)
      , prio(p) {
    }

    page_cache_result(page_cache_result&& o) noexcept
      : begin_pageno(o.begin_pageno)
      , data(std::move(o.data))
      , prio(std::move(o.prio))
      , marked_for_eviction(o.marked_for_eviction)
      , locks(o.locks)
      , pending_reads(o.pending_reads) {
    }

    page_cache_result& operator=(page_cache_result&& o) noexcept {
        if (this != &o) {
    ...
    ~page_cache_result() {
        DLOG_ERROR_IF(
          !is_evictable(), "did not finish releasing locks. {}", *this);
    }
    ...
    int32_t end_pageno() const {
        // x >> 12 == x/4096
        static constexpr const int32_t kTrailingZeroes = __builtin_ctz(4096);
        return begin_pageno
               + std::max<int32_t>(1, data.size() >> kTrailingZeroes);
    }

    bool is_page_in_range(int32_t pageno) const {
        return pageno >= begin_pageno && pageno < end_pageno();
    }
    const char* get_page_data(int32_t pageno) const {
        DLOG_THROW_IF(
          !is_page_in_range(pageno),
...
struct page_cache_result_lease {
    ...
    explicit page_cache_result_lease(page_cache_result* p)
      : result(p) {
        if (result != nullptr)
            result->locks++;
    }
    ...
    page_cache_result_lease& operator=(const page_cache_result_lease& o) {
        if (this != &o) {
            if (result != nullptr)
                result->locks--;
            result = o.result;
...
inline ostream& operator<<(ostream& o, const page_cache_result& r) {
