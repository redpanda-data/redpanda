...
namespace std {
inline ostream& operator<<(ostream& o, const page_cache_stats& s) {
    double allreqs = static_cast<double>(s.cache_miss + s.cache_hit + s.stalls);
    double missrate = s.cache_miss > 0
                        ? (static_cast<double>(s.cache_miss) / allreqs)
                        : 0;
    double hitrate = s.cache_hit > 0
                       ? (static_cast<double>(s.cache_hit) / allreqs)
                       : 0;
