...
static uint64_t fixed_hash(const std::string& topic, uint32_t partition) {
...
static uint64_t stack_hash(const std::string& topic, uint32_t partition) {
...
static uint64_t incremental_hash(const std::string& topic, uint32_t partition) {
...
static void BM_fixed_hash(benchmark::State& state) {
    for (auto _ : state) {
        state.PauseTiming();
        std::string x;
...
static void BM_stackhash(benchmark::State& state) {
    for (auto _ : state) {
        state.PauseTiming();
        std::string x;
        x.reserve(state.range(0));
...
static void BM_incremental_hash(benchmark::State& state) {
    for (auto _ : state) {
        state.PauseTiming();
        std::string x;
        x.reserve(state.range(0));
