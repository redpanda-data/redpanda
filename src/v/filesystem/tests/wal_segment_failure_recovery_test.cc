...
public:
    put() {
        ...
        DLOG_INFO(
          "Encoded request is of size: {}", smf::human_bytes(body.size()));
        _orig = std::make_unique<smf::fbs_typed_buf<wal_put_request>>(
          std::move(body));
    }
          ...
          return f->allocate(0, sz).then([f] {
              ...
              return f->dma_write(0, ptr, f->disk_write_dma_alignment())
                .then([b = std::move(buf), f](auto written_bytes) {
                    LOG_THROW_IF(
                      written_bytes != f->disk_write_dma_alignment(),
