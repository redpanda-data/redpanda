...
void print_record(int64_t offset, const wal_binary_record* i) {
...
void print_reply(std::vector<uint8_t>& vec) {
    static auto lz4 = smf::codec::make_unique(
    ...
    LOG_INFO(
      "Largest offset stored in index: {}, lens_bytes: {}, total keys: {}",
      reply->largest_offset(),
      reply->lens_bytes(),
      reply->keys()->size());
}
                        ...
                        return seastar::do_with(
                          ...
                          .then([ret, writer_size] {
                              int64_t sz = std::accumulate(
                                ret->reply().gets.begin(),
                                ret->reply().gets.end(),
                                int64_t(0),
                                [](int64_t acc, auto& n) {
                                    return acc + n->data.size();
                                });
