...
wal_smash::wal_smash(
  ...
  , _wal(THROW_IFNULL(w)) {
    _opts.ns_id = xxhash_64(
      _opts.topic_namespace.c_str(), _opts.topic_namespace.size());
...
wal_smash::create(std::vector<int32_t> partitions) {
    ...
    auto tbuf = smf::fbs_typed_buf<wal_topic_create_request>(
      gen_create_topic_buf(
        _opts.topic_namespace,
        _opts.topic,
        _opts.topic_partitions,
        _opts.topic_type,
        _opts.topic_props));
...
wal_smash::read_one(int32_t opartition) {
    ...
    const int32_t partition = opartition <= -1 ? jump_consistent_hash(
