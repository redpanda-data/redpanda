...
seastar::future<> wal_cold_boot::visit_namespace(seastar::directory_entry de) {
    if (!de.type) {
...
seastar::future<> wal_cold_boot::visit_partition(
  seastar::sstring ns, seastar::sstring topic, seastar::directory_entry de) {
    if (!de.type) {
        return seastar::make_ready_future<>();
    }
    if (de.type != seastar::directory_entry_type::directory) {
