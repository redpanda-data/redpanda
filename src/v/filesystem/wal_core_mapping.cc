...
wal_core_mapping::core_assignment(const wal_topic_create_request* p) {
    ...
    for (auto i = 0u; i < assign.size(); ++i) {
        if (assign[i].empty()) {
            DLOG_TRACE(
              "Core `{}` has no partitions. ns: {}, topic: `{}`",
              i,
              p->ns(),
              p->topic());
            continue;
...
wal_read_request wal_core_mapping::core_assignment(const wal_get_request* p) {
...
wal_core_mapping::core_assignment(const wal_put_request* p) {
    ...
    retval.reserve(std::accumulate(
      view.begin(), view.end(), int32_t(0), [](int32_t acc, auto& next) {
    ...
    for (auto& v : view) {
        for (auto& tup : v) {
            ...
            retval.push_back(wal_write_request(
              p,
              tup.second.runner_core,
              partition,
              idx,
              std::move(tup.second.records)));
        }
    }
    return retval;
}
