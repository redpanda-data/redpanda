...
struct wal_name_extractor_utils {
    ...
    wal_segment_extract_epoch_term(const sstring& filename);

    /// \brief determines if is a wal_file or not
    static bool is_wal_segment(const sstring& filename);
    ...
    /// \brief determines if is a wal_file or not
    static bool is_valid_partition_name(const sstring& filename);

    /// \brief extracts the partition out of the directory name
    static int32_t wal_partition_dir_extract(const sstring& filename);
};
