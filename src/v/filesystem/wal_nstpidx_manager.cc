...
struct offset_comparator {
    ...
    bool operator()(
      const std::unique_ptr<wal_reader_node>& yptr,
    ...
    while (opts.max_retention_size > 0 && nodes_size-- > 0) {
        ...
        if (max_bytes <= 0) {
            ...
            for (auto i = 0; i <= nodes_size; ++i) {
                ...
                _nodes.pop_front();
                LOG_INFO(
                  "Marking node {} as deleted. Size retention policy",
    ...
    return wal_nstpidx_repair::repair(work_dir)
      .then([this](auto repaired) {
          ...
          for (const auto& i : repaired) {
              ...
              auto n = std::make_unique<wal_reader_node>(
                i.epoch, i.term, i.size, i.modified, full_path_file);
              _nodes.push_back(std::move(n));
          }
          LOG_INFO(
            "{} total nodes: {}, {}-{} ({})",
...
std::unique_ptr<wal_partition_statsT> wal_nstpidx_manager::stats() const {
