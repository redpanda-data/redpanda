...
//
inline int64_t
round_up(int64_t file_size) {
  int64_t ret = file_size / kReadPageSize;
  if (file_size % kReadPageSize != 0) { ret += 1; }
  return ret;
}

inline wal_read_errno
validate_header(const wal_header &hdr) {
...
validate_payload(const wal_header &hdr, const uint8_t *buf, int32_t buf_size) {
  ...
  if (wal_segment_record::checksum((const char *)buf, buf_size) !=
      hdr.checksum()) {
...
static int64_t
copy_page_data(wal_reader_node::read_exactly_req &req,
               const page_cache_result *result, int32_t page_no) {
  const int64_t page_offset = req.consume_offset() % kReadPageSize;

  DLOG_THROW_IF(result == nullptr, "bad range");
  DLOG_TRACE("relative_offset: {}, consume_offset: {}, page begin: {}, page: "
             "{}, disk_page bytes: {}, page_offset: {}",
             req.relative_offset, req.consume_offset(), page_no * kReadPageSize,
             page_no, result->data.size(), page_offset);
  ...
  const int64_t step_size =
    std::min<int64_t>(result->data.size() - ptdf, req.size_left_to_consume());

  DLOG_TRACE("page_offset: {}, step_size: {}, space_left_to_consume(): {}, "
             "disk_page.size: {}",
...
void
wal_reader_node::update_file_size(int64_t newsize) {
  // size must be monotonically increasing
  if (newsize <= file_size_) return;
  ...
  if (file_) {
    ...
    if (!pages_to_evict.empty()) {
      page_cache::get().evict_pages(global_file_id_, std::move(pages_to_evict));
    }
  }
}
seastar::future<>
wal_reader_node::open() {
  return open_node().then([this] {
    ...
    return file_->stat().then([this](auto stat) {
      ...
      global_file_id_ =
        xxhash_32(std::experimental::make_array(stat.st_ino, stat.st_dev));
      DLOG_TRACE("{}: inode: {}, device: {}, file_id: {}", filename,
                 stat.st_ino, stat.st_dev, global_file_id_);
    });
  });
}
...
wal_reader_node::get(wal_read_reply *retval, wal_read_request r) {
  ...
  LOG_ERROR_IF(retval->on_disk_size() != 0,
               ...
               "size:{} offset {} is >= {} or < {}",
               filename, file_size_, retval->on_disk_size(), offset,
               ending_epoch(), starting_epoch);
  retval->set_error(wal_read_errno::wal_read_errno_invalid_offset);
  return seastar::make_ready_future<>();
}

seastar::future<seastar::stop_iteration>
wal_reader_node::copy_one_record(wal_read_reply *retval,
  ...
  return copy_exactly(
    ...
    .then([this, retval, ptr, pager, roffset](auto iter) {
      ...
      return copy_exactly(read_exactly_req(ptr->data.data() + kWalHeaderSize,
    ...
    .then([ptr, retval, record = std::move(record)](auto iter) mutable {
      ...
      if (retval->validate_checksum) {
        ...
        // 3. validate payload
        auto err =
          validate_payload(hdr, ptr->data.data() + kWalHeaderSize, bodysz);
        if (err != wal_read_errno::wal_read_errno_none) {
          retval->set_error(err);
...
seastar::future<seastar::stop_iteration>
wal_reader_node::copy_exactly(wal_reader_node::read_exactly_req req,
                              std::reference_wrapper<wal_disk_pager> pager) {
  using stop_t = seastar::stop_iteration;
  ...
  return seastar::do_with(std::move(req), [pager](auto &r) {
    return seastar::repeat([&r, pager]() mutable {
             ...
             auto pageno = offset_to_page(r.consume_offset(), kReadPageSize);
             return pager.get().fetch(pageno).then([&r, pager, pageno](
                                                     auto range) mutable {
               DLOG_THROW_IF(
                 !range, "This means that the caller asked for out of bounds");
               DLOG_THROW_IF(!range->is_page_in_range(pageno),
                             "Page({}) not in range: {}. req:{}", pageno,
                             *range, pager.get().request());
      ...
      .then([&r] {
        DLOG_TRACE("ReadExactly: left to consume: {}. Copied bytes: {}",
                   r.size_left_to_consume(), r.consumed);
        // Here we have to invert the logic above.
        // We do not want our parents to stop reading unless
        // we had encountered an error and we couldn't copy what we promised
        if (r.size_left_to_consume() <= 0) {
          return seastar::make_ready_future<stop_t>(stop_t::no);
        }
        return seastar::make_ready_future<stop_t>(stop_t::yes);
...
wal_reader_node::do_read(wal_read_reply *retval, wal_read_request r) {
  auto roffset = relative_offset(retval);
  if (roffset < 0) {
    LOG_ERROR("offset: {} is below bounds ({}-{}]", retval->next_epoch(),
    ...
    [this, retval, remaining_req_size, max_bytes = r.req->max_bytes(),
     initial_size = retval->on_disk_size()](wal_disk_pager &real_pager) {
      ...
      return seastar::repeat([=] {
        ...
        if (retval->next_epoch() < starting_epoch ||
