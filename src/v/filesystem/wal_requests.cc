...
bool wal_create_request::is_valid(const wal_create_request& r) {
    try {
        ...
        LOG_ERROR(
          "Validating wal_create_request: caught exception: {}", e.what());
    }
    return false;
}

bool wal_read_request::is_valid(const wal_read_request& r) {
    try {
        ...
        LOG_ERROR(
          "Validating wal_read_request: caught exception: {}", e.what());
        return false;
    }
    return true;
}

wal_read_reply::wal_read_reply(
  int64_t ns,
  int64_t topic,
  int32_t partition,
...
void wal_write_reply::set_reply_partition_tuple(
  int64_t ns, int64_t topic, int32_t partition, int64_t begin, int64_t end) {
...
std::unique_ptr<wal_put_replyT> wal_write_reply::release() {
