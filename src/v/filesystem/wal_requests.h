...
struct wal_create_request final {
    ...
    wal_create_request(
      const wal_topic_create_request* ptr,
      uint32_t core,
      std::vector<int32_t> assignments)
      : req(THROW_IFNULL(ptr))
      , runner_core(core)
    ...
    wal_create_request(wal_create_request&& o) noexcept
      : req(std::move(o.req))
      , runner_core(o.runner_core)
      , partition_assignments(std::move(o.partition_assignments)) {
    }

    wal_create_request& operator=(wal_create_request&& o) noexcept {
        if (this != &o) {
...
struct wal_create_reply final {
    ...
    wal_create_reply(wal_create_reply&& o) noexcept
...
struct wal_read_request final {
    ...
    wal_read_request(wal_read_request&& o) noexcept
      : req(std::move(o.req))
      , runner_core(std::move(o.runner_core))
      , idx(std::move(o.idx)) {
    }
...
public:
    ...
    wal_read_reply(wal_read_reply&& o) noexcept
      : validate_checksum(o.validate_checksum)
      , _data(std::move(o._data))
      , data_size_(o.data_size_) {
    }

    wal_read_reply& operator=(wal_read_reply&& o) noexcept {
        if (this != &o) {
            this->~wal_read_reply();
    ...
    inline void add_record(std::unique_ptr<wal_binary_recordT> r) {
    ...
    inline const wal_get_replyT& reply() const {
        return _data;
    }
    inline int64_t next_epoch() const {
        return _data.next_offset;
    }
    inline void set_next_offset(int64_t n) {
        _data.next_offset = n;
    }

    inline int64_t on_disk_size() const {
        return data_size_;
    }

    inline void set_error(wal_read_errno e) {
        _data.error = e;
    }
    ...
    ///
    inline std::unique_ptr<wal_get_replyT> release() {
...
struct wal_write_request final {
    ...
    wal_write_request(
      const wal_put_request* ptr,
      const uint32_t core,
      int32_t _partition,
      const wal_nstpidx& id,
    ...
    wal_write_request& operator=(wal_write_request&& o) noexcept {
        if (this != &o) {
    ...
    inline static bool is_valid(const wal_write_request& r) {
        return !r.data.empty();
    }
};
...
public:
    using underlying = ska::bytell_hash_map<
      wal_nstpidx,
    ...
    wal_write_reply(int64_t _ns, int64_t _topic)
      : ns(_ns)
      , topic(_topic) {
    }
    ...
    void set_reply_partition_tuple(
      int64_t ns,
      int64_t topic,
      int32_t partition,
...
struct wal_stats_reply {
    ...
    wal_stats_reply(wal_stats_reply&& o) noexcept
      : stats(std::move(o.stats)) {
    }
    wal_stats_reply& operator=(wal_stats_reply&& o) noexcept {
        if (this != &o) {
