...
struct key_hash_functor {
    ...
    bool operator()(
      const std::unique_ptr<wal_segment_index_key_entryT>& e,
      const uint64_t& hash) const {
        return e->hash < hash;
    }
};

static std::unique_ptr<wal_binary_recordT> binary_index(
  uint64_t xorkey,
  ...
  wal_segment_indexer::wal_segment_index_key_entry_set& set) {
    ...
    while (!set.empty()) {
        std::unique_ptr<wal_segment_index_key_entryT> tmp = std::move(
          set.extract(set.begin()).value());
...
wal_segment_indexer::get_or_create(int64_t offset, const wal_binary_record* r) {
    ...
    if (auto it = std::lower_bound(
        ...
        if (__builtin_expect(
            ...
            // 2) This one will then disambiguate via strncmp *only* in the case
            // the
            //    hashes match
            // We accept this complexity because we don't want to allocate key
            // space for every record
            //
...
void wal_segment_indexer::reset() {
    // fully reset
    _size = 0;
    xorwalkey = 0;
    lens_bytes_ = 0;
