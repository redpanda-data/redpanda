...
int32_t wal_segment_record::checksum(const char* p, std::size_t s) {
    return std::numeric_limits<int32_t>::max() & xxhash_64(p, s);
}

wal_header wal_segment_record::header_for(
  const char* key,
  int32_t key_size,
  const char* val,
  int32_t val_size,
...
std::unique_ptr<wal_binary_recordT> wal_segment_record::coalesce(
  const char* key,
  int32_t key_size,
  const char* val,
  int32_t val_size,
  wal_compression_type ctype) {
    ...
    // copy the value
    std::memcpy(
      retval->data.data() + kWalHeaderSize + key_size, real_val, val_size);

    auto hdr = wal_segment_record::header_for(
      key, key_size, real_val, val_size, ctype);
...
wal_segment_record::extract_from_bin(const char* begin, int32_t sz) {
    ...
    if (hdr.compression() == wal_compression_type::wal_compression_type_lz4) {
      ...
      hdr.compression() == wal_compression_type::wal_compression_type_zstd) {
        valbuf = zstd->uncompress(
          begin + kWalHeaderSize + keybuf.size(), hdr.value_size());
    } else {
        // copy the val
        std::memcpy(
          valbuf.get_write(),
          begin + kWalHeaderSize + keybuf.size(),
          valbuf.size());
    }
