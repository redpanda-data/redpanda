  ...
  seastar::sstring name) {
    ...
    return n->open().then([n] {
        return n->heal().then([n] {
            ...
            if (!n->is_recovered()) {
                LOG_ERROR(
                  "Failed to recover file: {}, artificially returning size 0",
                  n->filename);
                should_rename = true;
                real_size = 0;
            } else {
            ...
            return n->close().then([n /*kept to ommit finally()*/,
                                    name = n->filename,
                                    should_rename,
                                    real_size] {
                if (should_rename) {
