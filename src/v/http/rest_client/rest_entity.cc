...
request_builder rest_entity::create(
  optional_headers headers, optional_query_params query_params) {
    apply_optionals(headers, query_params);
    return _rb.target(resource_url()).method(create_verb());
}

request_builder rest_entity::list(
  optional_headers headers, optional_query_params query_params) {
    apply_optionals(headers, query_params);
    return _rb.target(resource_url()).method(retrieve_verb());
}
...
request_builder rest_entity::get(
  ...
  optional_query_params query_params) {
    ...
    return _rb.target(target).method(retrieve_verb());
}
...
request_builder rest_entity::update(
  ...
  optional_query_params query_params) {
    apply_optionals(headers, query_params);
    return _rb.target(fmt::format("{}/{}", resource_url(), identifier))
      .method(update_verb());
}
...
rest_entity& rest_entity::with_host(std::string_view host) {
    _rb.host(host);
    return *this;
}

void rest_entity::apply_optionals(
  optional_headers headers, optional_query_params query_params) {
    if (headers.has_value()) {
        _rb.headers(headers.value());
    }

    if (query_params.has_value()) {
        _rb.query_params_kv(query_params.value());
    }
}
