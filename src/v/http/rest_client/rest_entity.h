...
public:
    ...
    using optional_headers
      = std::optional<absl::flat_hash_map<std::string_view, std::string_view>>;
    ...
    virtual rest_entity& with_host(std::string_view host);
    ss::sstring resource_url() const;

private:
    virtual void apply_optionals(
      optional_headers headers, optional_query_params query_params);

    http::request_builder _rb;
