...
struct fetch_request final {
    ...
    std::optional<std::chrono::milliseconds> debounce_delay() const {
        if (data.max_wait <= std::chrono::milliseconds::zero()) {
            return std::nullopt;
        } else {
            return data.max_wait;
        }
    }
    ...
    bool is_full_fetch_request() const {
        return data.epoch == initial_fetch_session_epoch
               || data.epoch == final_fetch_session_epoch;
    }
