...
// template<typename Request, typename FwdFunc>
// auto group_router::route_stages_co(Request r, FwdFunc func) {
//     (void)func;
//     // using return_type = std::invoke_result_t<
//     //   FwdFunc,
//     //   decltype(std::declval<group_manager>()),
//     //   Request&&>;
//     using return_type = group::offset_commit_stages;
//     using resp_type = typename return_type::value_type;
//     auto m = shard_for(r.data.group_id);
//     if (!m) {
//         return group::stages(resp_type(r, error_code::not_coordinator));
...
//           //   tracker->record("in_sg");
//           return get_group_manager()
//             .invoke_on(
//               coordinator_shard,
//               _ssg,
//               [r = std::move(r)](group_manager& mgr) mutable {
//                   //   tracker->record("on_co_shard");
//                   group::stages<resp_type> stages = mgr.offset_commit(
//                     std::move(r));
//                   // group::stages<resp_type> stages = std::invoke(
//                   //   func, mgr, std::move(r));
//                   /**
...
//                         return std::make_tuple(
//                           e,
//                           std::make_unique<decltype(rfut)>(std::move(rfut)));
//                     });
...
//                    coordinator_shard = coordinator_shard,
//                    d = std::move(dispatched)](auto f) {
//                 // In the immediately preceding continuation we did not
//                 return
//                 // the stages.result future directly, rather we wrapped it
//                 with
//                 // a unique_ptr. This means that the future doesn't receive
//                 the
//                 // cross-shard treatment and is only safe to use on the
//                 // coordinator shard, not this one. So next we pass this
//                 future
//                 // back to the coordinator to unwrap it and return it
//                 unwrapped,
//                 // so it can be used by this shard (and, as a side effect the
//                 // dispatched future is also set).
//                 //
...
//                 // the unwrapped future above?
//                 //
//                 // Well, this second stage may take a very long time resolve
//                 and
//                 // may depend on coordination with clients: e.g., the
//                 JoinGroup
//                 // request won't be responded to until all other current
//                 members
//                 // have also sent their JoinGroup requests: all the joiners
//                 have
//                 // their responses held until that moment. Above, the
//                 invoke_on
//                 // is under the influence of the kafka smp_service_group
//                 (_ssg),
//                 // with a limit of 5,000 outstanding requests on any code to
//                 the
//                 // coordinator: this means that a group with more than 5,000
//                 // members cannot successfully be joined: at most join 5,000
//                 // requests can be outstanding at once, any more will block
//                 on
//                 // the ssg, and the existing ones won't complete because they
//                 // are waiting for the blocked ones: a type of "soft"
//                 deadlock
//                 // (soft because the server eventually) times out the join
//                 // process an the process repeats.
//                 //
...
//                 // freeing up the service group slot for another request to
//                 // dispatch. We extract the stage.result below in a new
//                 // invoke_on call *without any ssg*: this uses the default
//                 ssg
//                 // which does not have any limit, avoiding the deadlock.
