...
std::optional<error_code_and_msg> validate_timestamp(
  ...
  std::optional<std::chrono::milliseconds> message_timestamp_alert_before_ms,
  std::optional<std::chrono::milliseconds> message_timestamp_alert_after_ms,
  kafka::kafka_probe& probe) {
    ...
    if (message_timestamp_alert_before_ms.has_value()) {
        is_invalid |= delta > model::timestamp(
                        message_timestamp_alert_before_ms->count());
    }
    if (message_timestamp_alert_after_ms.has_value()) {
        is_invalid |= model::timestamp(-1 * delta()) > model::timestamp(
                        message_timestamp_alert_after_ms->count());
    }
...
std::optional<error_code_and_msg> validate_batch(const validation_args& args) {
    ...
    if (batch.compressed()) {
        if (should_decompress(batch, validation_mode)) {
            maybe_decompressed_batch
              = storage::internal::maybe_decompress_batch_sync(batch);
    ...
    return validate_batch(
      ...
      validation_mode,
      args.timestamp_type,
      args.message_timestamp_alert_before_ms,
      args.message_timestamp_alert_after_ms,
      args.probe);
}
