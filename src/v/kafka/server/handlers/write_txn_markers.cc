...
ss::future<std::error_code> expire_tx_for_ntp(
    ...
    for (const auto& marker : request.data.markers) {
        ...
        for (const auto& topic : marker.topics) {
            ...
            for (const auto& partition : topic.partition_indexes) {
                ...
                auto res = co_await expire_tx_for_ntp(ctx, ntp, pid);
                writable_txn_marker_partition_result partition_res;
                partition_res.partition_index = partition;
                switch (res.value()) {
                case static_cast<int>(cluster::tx_errc::none):
                    break;
                case static_cast<int>(cluster::tx_errc::shard_not_found):
                case static_cast<int>(cluster::tx_errc::partition_not_found): {
