...
static ktp as_ktp(ktp_with_hash ktph) {
    return {ktph.get_topic(), ktph.get_partition()};
}

BOOST_AUTO_TEST_CASE(test_ktp_with_hash_move_from) {
    ktp_with_hash ktph{"topic1", 0};
    auto ktp0 = as_ktp(ktph);

    BOOST_CHECK_EQUAL(ktp0, ktph);
    BOOST_CHECK_EQUAL(hash_of(ktp0), hash_of(ktph));

    // move out of the ktph and check that the hash of the moved-from
    // object is stil correct
    auto temp{std::move(ktph)};
    // NOLINTNEXTLINE(bugprone-use-after-move)
    auto ktp1 = as_ktp(ktph);

    BOOST_CHECK_EQUAL(ktp1, ktph);
    BOOST_CHECK_EQUAL(hash_of(ktp1), hash_of(ktph));
    BOOST_CHECK_EQUAL(hash_of(temp), hash_of(as_ktp(temp)));

    ktph = std::move(temp); // move back
    BOOST_CHECK_EQUAL(ktp0, ktph);
    BOOST_CHECK_EQUAL(hash_of(ktp0), hash_of(ktph));
    // NOLINTNEXTLINE(bugprone-use-after-move)
    BOOST_CHECK_EQUAL(hash_of(temp), hash_of(as_ktp(temp)));

    // now again with move-assignment
    temp = std::move(ktph);
    // NOLINTNEXTLINE(bugprone-use-after-move)
    ktp1 = as_ktp(ktph);
...
BOOST_AUTO_TEST_CASE(test_ktp_with_hash_move_from2) {
    ktp_with_hash ktp0{"topic1", 10};

    ktp_with_hash ktp1{std::move(ktp0)};

    BOOST_CHECK_EQUAL(ktp1.get_topic(), "topic1");
    BOOST_CHECK_EQUAL(ktp1.get_partition(), 10);
    BOOST_CHECK_EQUAL(hash_of(ktp1), hash_of(as_ktp(ktp1)));
}

BOOST_AUTO_TEST_CASE(test_sstring_move_from) {
    // check that move-from leaves sstring empty even in the
    // SSO range, since other tests rely on this
    ss::sstring x("from");
    ss::sstring y = std::move(x);

    BOOST_CHECK_EQUAL(x, ""); // NOLINT(bugprone-use-after-move)
}

BOOST_AUTO_TEST_CASE(test_ktp_move_leaves_empty) {
    // validate the impl is doing what we think
    ktp_with_hash ktp0{"topic1", 10};
    ktp_with_hash ktp1{std::move(ktp0)};

    // NOLINTNEXTLINE(bugprone-use-after-move)
    BOOST_CHECK_EQUAL(ktp0.get_topic(), ""); // topic name was moved-from
    BOOST_CHECK_EQUAL(
      ktp1.get_partition(), 10); // partition not moved: stays the same
}

BOOST_AUTO_TEST_CASE(test_ktp_with_hash_move_leaves_empty) {
    // validate the impl is doing what we think
    ktp_with_hash ktp0{"topic1", 10};
    ktp_with_hash ktp1{std::move(ktp0)};
...
BOOST_AUTO_TEST_CASE(test_ktp_with_hash_move_assign_leaves_empty) {
    // validate the impl is doing what we think
    ktp_with_hash ktp0{"topic1", 10};
    ktp_with_hash ktp1;
    ktp1 = std::move(ktp0);
