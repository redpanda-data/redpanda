...
create_consumer(server::request_t rq, server::reply_t rp) {
    ...
    validate_no_control(
      req_data.name(),
      parse::pp_parsing_error{req_data.name(), "create_consumer"});
...
subscribe_consumer(server::request_t rq, server::reply_t rp) {
    ...
    std::for_each(
      ...
      [](const auto& topic_name) {
          validate_no_control(
            topic_name(),
            parse::pp_parsing_error{topic_name(), "subscribe_consumer"});
      });
...
get_consumer_offsets(server::request_t rq, server::reply_t rp) {
    ...
    std::for_each(req_data.begin(), req_data.end(), [](const auto& r) {
        validate_no_control(
          r.name(), parse::pp_parsing_error{r.name(), "Get_consumer_offsets"});
    });
...
post_consumer_offsets(server::request_t rq, server::reply_t rp) {
    ...
    std::for_each(req_data.begin(), req_data.end(), [](const auto& r) {
        validate_no_control(
          r.name(), parse::pp_parsing_error{r.name(), "post_consumer_offsets"});
    });
