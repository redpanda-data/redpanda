...
///\brief An in-memory store, backed by a partition.
class backed_store {
public:
    using insert_result = sharded_store::insert_result;

    explicit backed_store(
      sharded_store& store, ss::sharded<kafka::client::client>& client)
      : _store{store}
      , _client{client} {}
    ...
    ///\brief Insert or update a schema for the subject version.
    ...
    ///\brief Signal that an offset has been reached.
    ss::future<> signal(model::offset offset, offset_conflict conflict);
    ///\brief Wait for an offset to be reached.
