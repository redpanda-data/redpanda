...
template<typename T>
void log_response(const ss::http::request& req, const T& resp) {
    if (srlog.is_enabled(ss::log_level::trace)) {
        using Buffer = ::json::chunked_buffer;
        Buffer buf;
        ::json::iobuf_writer<Buffer> wrt{buf};
        rjson_serialize(wrt, resp);
        iobuf_parser parser{std::move(buf).as_iobuf()};
...
get_config(server::request_t rq, server::reply_t rp) {
    ...
    auto resp = get_config_req_rep{.compat = res};
    log_response(*rq.req, resp);
    rp.rep->write_body("json", ppj::rjson_serialize(resp));
    co_return rp;
}
...
delete_config_subject(server::request_t rq, server::reply_t rp) {
    ...
    auto resp = get_config_req_rep{.compat = lvl};
    log_response(*rq.req, resp);
    rp.rep->write_body("json", ppj::rjson_serialize(resp));
    co_return rp;
}
...
ss::future<server::reply_t> get_mode(server::request_t rq, server::reply_t rp) {
    ...
    auto resp = mode_req_rep{.mode = res};
    log_response(*rq.req, resp);
    rp.rep->write_body("json", ppj::rjson_serialize(resp));
    co_return rp;
}
...
get_schemas_ids_id(server::request_t rq, server::reply_t rp) {
    ...
    auto resp = get_schemas_ids_id_response{.definition{std::move(def)}};
    log_response(*rq.req, resp);
    rp.rep->write_body("json", ppj::rjson_serialize(std::move(resp)));
    co_return rp;
}
...
get_schemas_ids_id_versions(server::request_t rq, server::reply_t rp) {
    ...
    auto resp = get_schemas_ids_id_versions_response{
...
ss::future<ctx_server<service>::reply_t> get_schemas_ids_id_subjects(
  ctx_server<service>::request_t rq, ctx_server<service>::reply_t rp) {
    ...
    auto resp = co_await rq.service().schema_store().get_schema_subjects(
      id, incl_del);
    log_response(*rq.req, resp);
    rp.rep->write_body("json", ppj::rjson_serialize(std::move(resp)));
    co_return rp;
...
get_subjects(server::request_t rq, server::reply_t rp) {
    ...
    auto resp = co_await rq.service().schema_store().get_subjects(
      inc_del, subject_prefix);
    log_response(*rq.req, resp);
    rp.rep->write_body("json", json::rjson_serialize(std::move(resp)));
    co_return rp;
}
...
post_subject(server::request_t rq, server::reply_t rp) {
    ...
    auto resp = post_subject_versions_version_response{
...
post_subject_versions(server::request_t rq, server::reply_t rp) {
    ...
    auto resp = post_subject_versions_response{.id{schema_id}};
    log_response(*rq.req, resp);
    rp.rep->write_body("json", ppj::rjson_serialize(resp));
    co_return rp;
}
...
compatibility_subject_version(server::request_t rq, server::reply_t rp) {
    ...
    try {
        ...
        if (is_verbose && reportable(e.code())) {
            auto resp = post_compatibility_res{
              .is_compat = false,
              .messages = {e.message()},
