...
// Regression: partition_proxy::replicate() returns last_offset, so for
// multi-record batches (e.g. CONTEXT + SCHEMA on first write to a new
// context) we expect the rpc service to convert to base_offset.
...
TEST_P(SchemaRegistryRpcTransportTest, GetHighWatermark) {
    // The in_memory_proxy hardcodes high_watermark to 102
    auto hwm = transport().get_high_watermark().get();
    EXPECT_EQ(hwm, model::offset(102));
}
...
TEST_P(SchemaRegistryRpcTransportTest, ConsumeRangeProducedBatches) {
    // Produce a few batches
    transport().produce(make_batch("k1", "v1")).get();
    transport().produce(make_batch("k2", "v2")).get();
    transport().produce(make_batch("k3", "v3")).get();

    // Consume from offset 0 to offset 3 (exclusive upper bound)
    int consumed_count = 0;
    transport()
      .consume_range(
        model::offset(0),
        model::offset(3),
        [&consumed_count](model::record_batch) -> ss::future<> {
            ++consumed_count;
            return ss::now();
        })
      .get();
    EXPECT_EQ(consumed_count, 3);
}
...
TEST_P(SchemaRegistryRpcTransportTest, ConsumeRangeEmpty) {
    // Consume an empty range — should return without invoking the callback
    int consumed_count = 0;
    transport()
      .consume_range(
        model::offset(0),
