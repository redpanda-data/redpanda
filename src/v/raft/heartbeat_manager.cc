...
heartbeat_manager::requests_for_range_v2() {
    ...
    for (auto& r : _consensus_groups) {
        ...
        for (auto& [id, follower_metadata] : r->_fstats) {
            ...
            hb_request_envelope envelope{
              .group = r->group(),
            };
            const auto raft_metadata = r->meta();
            if (!needs_full_heartbeat(follower_metadata, raft_metadata)) {
            ...
            envelope.data = heartbeat_request_state{
    ...
    for (auto& p : pending_beats) {
        ...
        reqs.emplace_back(
          p.first,
          heartbeat_request_v2(_self, p.first, std::move(requests)),
          std::move(meta_map));
    }
...
bool heartbeat_manager::needs_full_heartbeat(
  ...
  const protocol_metadata& p_meta) const {
...
void heartbeat_manager::process_reply(
  ...
  result<heartbeat_reply_v2> r) {
    ...
    auto& reply = r.value();
    for (auto& m : reply.group_replies) {
        ...
        if (unlikely(m.result == reply_status::group_unavailable)) {
        ...
        if (unlikely(m.result == reply_status::timeout)) {
            vlog(
              hbeatlog.debug,
        ...
        if (unlikely(reply.target_node != consensus->self().id())) {
            vlog(
              hbeatlog.warn,
        ...
        consensus->process_append_entries_reply(
          ...
          result<append_entries_reply>(append_entries_reply{
            .target_node_id = raft::vnode(
              reply.target_node, m.data->target_revision),
            .node_id = raft::vnode(reply.source_node, m.data->source_revision),
            .group = m.group,
            .term = m.data->term,
