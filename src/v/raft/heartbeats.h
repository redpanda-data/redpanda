...
public:
        ...
        explicit cursor(const deltafor_column<ValueT, DeltaT, FieldT>& column)
          ...
          , _decoder(
            ...
            if (_column.size() <= _column.values_per_row) {
                _buffer = _column._reminder_buffer;
                _in_reminder = true;
            } else {
    ...
    void add(ValueT value) {
        ...
        if constexpr (values_per_field == 1) {
            _reminder_buffer[idx] = value;
        } else {
            _reminder_buffer[idx] |= (static_cast<field_t>(value) << shift);
        }

        if (
          idx == (row_sz - 1) && shift == (values_per_field - 1) * value_bits) {
            _data.add(_reminder_buffer);
            _reminder_buffer = {};
        }
    }
    ...
    deltafor_column<ValueT, DeltaT, FieldT> copy() const {
        ...
        ret._data = encoder_t(
          ...
          _data.copy());
        ret._cnt = _cnt;
        ret._reminder_buffer = _reminder_buffer;
        return ret;
    }
    ...
    friend bool operator==(
      ...
      const deltafor_column<ValueT, DeltaT, FieldT>& rhs) {
        return lhs._data == rhs._data && lhs._cnt == rhs._cnt
               && lhs._reminder_buffer == rhs._reminder_buffer;
    }
    ...
    friend inline void read_nested(
      ...
      size_t const bytes_left_limit) {
        ...
        c._reminder_buffer = read_nested<row_t>(in, bytes_left_limit);
    ...
    write(iobuf& out, deltafor_column<ValueT, DeltaT, FieldT> state) {
        using serde::write;
        write(out, state._reminder_buffer);
...
struct heartbeat_request_data {
    ...
    friend inline void read_nested(
      iobuf_parser& in,
      heartbeat_request_data& state,
      size_t const bytes_left_limit) {
        using serde::read_nested;
        read_nested(in, state.source_revision, bytes_left_limit);
        read_nested(in, state.target_revision, bytes_left_limit);
        read_nested(in, state.commit_index, bytes_left_limit);
        read_nested(in, state.term, bytes_left_limit);
        read_nested(in, state.prev_log_index, bytes_left_limit);
        read_nested(in, state.prev_log_term, bytes_left_limit);
        read_nested(in, state.last_visible_index, bytes_left_limit);
    }
    friend inline void write(iobuf& out, heartbeat_request_data state) {
        using serde::write;
        write(out, state.source_revision);
        write(out, state.target_revision);
        write(out, state.commit_index);
        write(out, state.term);
        write(out, state.prev_log_index);
        write(out, state.prev_log_term);
        write(out, state.last_visible_index);
    }
...
struct heartbeat_reply_data {
    ...
    friend inline void read_nested(
      iobuf_parser& in,
      heartbeat_reply_data& state,
      size_t const bytes_left_limit) {
        using serde::read_nested;
        read_nested(in, state.source_revision, bytes_left_limit);
        read_nested(in, state.target_revision, bytes_left_limit);
        read_nested(in, state.term, bytes_left_limit);
        read_nested(in, state.last_flushed_log_index, bytes_left_limit);
        read_nested(in, state.last_dirty_log_index, bytes_left_limit);
        read_nested(in, state.last_term_base_offset, bytes_left_limit);
    }
    friend inline void write(iobuf& out, heartbeat_reply_data state) {
        using serde::write;
        write(out, state.source_revision);
        write(out, state.target_revision);
        write(out, state.term);
        write(out, state.last_flushed_log_index);
        write(out, state.last_dirty_log_index);
        write(out, state.last_term_base_offset);
    }
...
static constexpr size_t lw_decoder_row_size = 16;
