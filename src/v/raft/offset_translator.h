...
/// only batches with type not in the `filtered_types` set.
///
...
private:
    ...
    void do_add(model::offset offset);

    bytes offsets_map_key() const;
    bytes highest_known_offset_key() const;

    iobuf serialize_offsets_map(
      const absl::btree_map<model::offset, int64_t>& offset2delta);
    absl::btree_map<model::offset, int64_t> deserialize_offsets_map(iobuf buf);
...
private:
    ...
    // Map from log offset of non-data batches to corresponding delta with data
    // offset. Delta is active in the interval (log offset; next log offset]
    // (left end exclusive, right end inclusive).
    absl::btree_map<model::offset, int64_t> _offset2delta;
