...
public:
    ...
    void vote_request_term_older() {
        ++_vote_request_older_term;
    }

    void vote_request_term_newer() {
        ++_vote_request_newer_term;
    }

    void append_requested() {
        ++_append_requests;
    }

    void entries_appended(uint32_t entries) {
        _appended_entries += entries;
    }

    void append_request_term_older() {
        ++_append_request_older_term;
    }

    void append_request_term_newer() {
        ++_append_request_newer_term;
    }
    ...
    void append_request_log_term_older() {
        ++_append_request_older_log_term;
    }

    void append_request_log_truncate() {
        ++_append_request_log_truncated;
    }

    void leader_commit_index_mismatch() {
        ++_leader_commit_index_mismatched;
    }

    void append_request_heartbeat() {
        ++_append_request_heartbeats;
    }
