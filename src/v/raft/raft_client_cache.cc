...
verify_backoff(raft_client_cache::backoff t) {
    DLOG_THROW_IF(
      t < raft_client_cache::backoff::none
...
verify_bitflags(raft_client_cache::bitflags t) {
    DLOG_THROW_IF(
      t < raft_client_cache::bitflags::none
...
static inline int32_t to_secs(raft_client_cache::backoff b) {
...
static inline int32_t to_ms(smf::random& rng, raft_client_cache::backoff b) {
    ...
    return with_gate(
             ...
             [this, node, b] {
                 ...
                 return sleep(ms).then([this, node, b, ms] {
                     ...
                     LOG_INFO(
                       "Attempting ({}) re-connection after: {} millis",
                       ptr->server_addr,
                       ms.count());
                     constexpr uint8_t max_retries = static_cast<uint8_t>(
                       backoff::max);
  ...
  raft::raft_api_client* ptr, raft_client_cache::backoff b) {
    ...
    return ptr->reconnect()
      ...
      .handle_exception([this, ptr, b](auto eptr) {
          LOG_INFO(
            "Recovering from '{}'. Staging reconnect to: {}",
            eptr,
            ptr->server_addr);
