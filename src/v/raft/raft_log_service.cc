...
raft_log_service::raft_cfg_log_process_one(std::unique_ptr<wal_read_reply> r) {
    auto data = r->release();
    LOG_THROW_IF(
      data->error != wal_read_errno::wal_read_errno_none,
    ...
    for (auto& get : data->gets) {
        ...
        if (auto it = _omap.find(idx); it != _omap.end()) {
            DLOG_INFO(
              "raft cfg recovery ns/topic/partition: {}/{}/{}",
              k.ns(),
              k.topic(),
              k.partition());
            ...
            case kt::log_index_topic_key_type_configuration: {
                ...
                auto tbuf = smf::fbs_typed_buf<raft::configuration_request>(
                  std::move(vbuf));
                auto ct = std::unique_ptr<raft::configuration_requestT>(
                  tbuf->UnPack());
                cfg = *ct;
                break;
            }
            ...
            default:
                DLOG_THROW(
                  "Cannot understand key: {}, during meta log recovery",
                  k.key());
...
future<> raft_log_service::recover_existing_logs() {
    ...
    for (auto& e : data_stats->stats) {
        ...
        raft_nstpidx_metadata m(
          ...
          segments.back().term(),
          segments.front().start_offset(),
          segments.back().end_offset());
    ...
    return read_all_raft_configuration_topic(
      _data,
      skip_ns,
      raft_config_topic_no,
      raft_config_begin_offset,
      raft_config_end_offset,
      [this](auto reply) {
          return raft_cfg_log_process_one(std::move(reply));
      });
}
