...
public:
    ...
    explicit raft_nstpidx_metadata(
      wal_nstpidx i,
      int64_t n,
      int64_t t,
      int32_t p,
    ...
    void initialize_leader() {
        initialize_leader_look_aside_table();
    }

private:
    void initialize_leader_look_aside_table() {
        leader_look_aside_table_.clear();
        leader_look_aside_table_.reserve(
          config.nodes.size() + config.learners.size());
    }

private:
    // must be re-initialized, cleared every time we become leader
    // key is the server id in the config
    ska::bytell_hash_map<int64_t, raft_nstpidx_metadata>
      leader_look_aside_table_;
};
