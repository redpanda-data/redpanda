    ...
    return ss::with_semaphore(
             ...
             [this, r = std::move(r)]() mutable {
                 return do_cache(std::move(r)).then([this](item_ptr i) {
                     if (
                       _batch_sem.waiters() == 0
                       || _pending_bytes >= _max_batch_size) {
                         return flush().then([this, i]() mutable { return i; });
                     }
                     return ss::make_ready_future<item_ptr>(i);
                 });
             })
      .then([this](item_ptr i) { return i->_promise.get_future(); });
}
                    ...
                    notifications = std::move(notifications)]() mutable {
                       ...
                       append_entries_request req{
                         ...
                         .batches = model::make_memory_record_batch_reader(
                           std::move(data))};
                       return do_flush(
                         std::move(notifications), std::move(req));
                   });
             })
