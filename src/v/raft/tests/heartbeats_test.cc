...
SEASTAR_THREAD_TEST_CASE(heartbeat_request_v2_rt) {
    ...
    serde::write_async(target, request.copy()).get();

    iobuf_parser parser(target.copy());

    auto from_serde
      = serde::read_async<raft::heartbeat_request_v2>(parser).get();

    BOOST_REQUIRE(request == from_serde);
}

SEASTAR_THREAD_TEST_CASE(heartbeat_reply_v2_rt) {
    auto reply = random_reply();
    iobuf target;

    serde::write_async(target, reply.copy()).get();
    iobuf_parser parser(target.copy());
    auto from_serde = serde::read_async<raft::heartbeat_reply_v2>(parser).get();

    BOOST_REQUIRE(reply == from_serde);
}
...
SEASTAR_THREAD_TEST_CASE(heartbeat_reply_for_each) {
    ...
    BOOST_TEST_MESSAGE(
      fmt::format("VALIDATING {} == {}", expected.size(), actual.size()));
