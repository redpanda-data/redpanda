...
ss::httpd::migration_json::inbound_migration_state to_admin_type(
  ...
  const cluster::inbound_data_migration& idm,
  cluster::data_migration_state state) {
...
ss::httpd::migration_json::outbound_migration_state to_admin_type(
  cluster::data_migration_id id,
  const cluster::outbound_data_migration& odm,
  cluster::data_migration_state state) {
...
void write_migration_as_json(
  const cluster::data_migration_metadata& meta,
...
cluster::inbound_topic parse_inbound_topic(json::Value& json) {
    cluster::inbound_topic ret;
...
cluster::inbound_data_migration
parse_inbound_data_migration(json::Value& json) {
    cluster::inbound_data_migration ret;
...
cluster::outbound_data_migration
parse_outbound_data_migration(json::Value& json) {
    cluster::outbound_data_migration ret;
    auto topics_array = json["topics"].GetArray();
    ret.topics.reserve(topics_array.Size());
    std::ranges::transform(
...
cluster::data_migration parse_add_migration_request(json::Document& json_doc) {
...
cluster::data_migration_id parse_data_migration_id(ss::http::request& req) {
    auto id_str = req.get_path_param("id");
    try {
        return cluster::data_migration_id(
          boost::lexical_cast<cluster::data_migration_id::type>(id_str));
    } catch (const boost::bad_lexical_cast& e) {
        throw ss::httpd::bad_param_exception(e.what());
    }
}
cluster::data_migration_state parse_migration_action(ss::http::request& req) {
    auto action_str = req.get_query_param("action");
    if (action_str == "prepare") {
        return cluster::data_migration_state::preparing;
    } else if (action_str == "execute") {
        return cluster::data_migration_state::executing;
    } else if (action_str == "cancel") {
        return cluster::data_migration_state::canceling;
    } else if (action_str == "finish") {
        return cluster::data_migration_state::finished;
    }
...
ss::future<std::unique_ptr<ss::http::reply>> admin_server::list_data_migrations(
  std::unique_ptr<ss::http::request>, std::unique_ptr<ss::http::reply> reply) {
    ...
    co_await ssx::async_for_each(
      migrations.begin(),
      migrations.end(),
      [&writer](const cluster::data_migration_metadata& meta) {
