...
security::scram_credential parse_scram_credential(const json::Document& doc) {
    ...
    validate_no_control(
      algorithm,
      admin_server::string_conversion_exception{
        algorithm, "parse_scram_credential (algorithm)"});
    ...
    validate_no_control(
      password,
      admin_server::string_conversion_exception{
        "PASSWORD", "parse_scram_credential (password)"});
...
bool match_scram_credential(
  const json::Document& doc, const security::scram_credential& creds) {
    ...
    validate_no_control(
      algorithm,
      admin_server::string_conversion_exception{
        algorithm, "match_scram_credential"});
...
security::role_name parse_role_definition(const json::Document& doc) {
    ...
    validate_no_control(
      role_name(),
      admin_server::string_conversion_exception{
        role_name(), "parse_role_definition"});
...
admin_server::create_user_handler(std::unique_ptr<ss::http::request> req) {
    ...
    validate_no_control(
      username(),
      string_conversion_exception{username(), "create_user_handler"});
