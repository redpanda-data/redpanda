...
admin_server::mark_transactions_draining_handler(
  std::unique_ptr<ss::http::request> req) {
    ...
    auto res = co_await tx_frontend.local().set_draining_txes(req_data, tx_ntp);
    co_await throw_on_error(*req, res, tx_ntp);
...
void admin_server::register_transaction_routes() {
    ...
    register_route<superuser>(
      ss::httpd::transaction_json::drain_transactions,
      [this](std::unique_ptr<ss::http::request> req) {
          return mark_transactions_draining_handler(std::move(req));
      });

    register_route<superuser>(
      ss::httpd::transaction_json::get_drain_transactions,
      [this](std::unique_ptr<ss::http::request> req) {
