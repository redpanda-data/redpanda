...
ss::logger alogger{"admin_api_server"};
...
ss::future<> admin_server::start() {
    ...
    vlog(alogger.info, "Registered HTTP admin service endpoints");
...
void admin_server::configure_admin_routes() {
    ...
    rb->set_api_doc(_server.local().server()._routes);
    rb->register_api_file(_server.local().server()._routes, "header");
    rb->register_api_file(_server.local().server()._routes, "config");
    rb->register_function(_server.local().server()._routes, insert_comma);
    rb->register_api_file(_server.local().server()._routes, "cluster_config");
    rb->register_function(_server.local().server()._routes, insert_comma);
    rb->register_api_file(_server.local().server()._routes, "raft");
    rb->register_function(_server.local().server()._routes, insert_comma);
    rb->register_api_file(_server.local().server()._routes, "kafka");
    rb->register_function(_server.local().server()._routes, insert_comma);
    rb->register_api_file(_server.local().server()._routes, "partition");
    rb->register_function(_server.local().server()._routes, insert_comma);
    rb->register_api_file(_server.local().server()._routes, "security");
    rb->register_function(_server.local().server()._routes, insert_comma);
    rb->register_api_file(_server.local().server()._routes, "status");
    rb->register_function(_server.local().server()._routes, insert_comma);
    rb->register_api_file(_server.local().server()._routes, "features");
    rb->register_function(_server.local().server()._routes, insert_comma);
    rb->register_api_file(_server.local().server()._routes, "hbadger");
    rb->register_function(_server.local().server()._routes, insert_comma);
    rb->register_api_file(_server.local().server()._routes, "broker");
    rb->register_function(_server.local().server()._routes, insert_comma);
    rb->register_api_file(_server.local().server()._routes, "transaction");
    rb->register_function(_server.local().server()._routes, insert_comma);
    rb->register_api_file(_server.local().server()._routes, "debug");
    rb->register_function(_server.local().server()._routes, insert_comma);
    rb->register_api_file(_server.local().server()._routes, "cluster");
    rb->register_function(_server.local().server()._routes, insert_comma);
    rb->register_api_file(_server.local().server()._routes, "transform");
  ...
  std::exception_ptr eptr) const {
    ...
    try {
        ...
        if (ex.status() == http_status::internal_server_error) {
            vlog(alogger.error, "{}", log_ex(status));
        } else if (status >= 400) {
            vlog(alogger.warn, "{}", log_ex(status));
        }
    } catch (...) {
        vlog(alogger.error, "{}", log_ex());
    }
}
...
ss::future<ss::httpd::redirect_exception> admin_server::redirect_to_leader(
  ss::http::request& req, model::ntp const& ntp) const {
    ...
    if (!leader_id_opt.has_value()) {
        vlog(alogger.info, "Can't redirect, no leader for ntp {}", ntp);
    ...
    vlog(
      alogger.info, "Redirecting admin API call to {} leader at {}", ntp, url);
          ...
          return cfg
            ...
            .then([](auto statuses) {
                ...
                for (const auto& s : statuses) {
                    vlog(alogger.trace, "status: {}", s.second);
...
admin_server::create_user_handler(std::unique_ptr<ss::http::request> req) {
    ...
    vlog(
      alogger.debug, "Creating user '{}' {}:{}", username, err, err.message());
...
admin_server::delete_user_handler(std::unique_ptr<ss::http::request> req) {
    ...
    if (!_controller->get_credential_store().local().contains(user)) {
        vlog(alogger.debug, "User '{}' already gone during deletion", user);
        co_return ss::json::json_return_type(ss::json::json_void());
    }
    ...
    vlog(alogger.debug, "Deleting user '{}' {}:{}", user, err, err.message());
...
admin_server::update_user_handler(std::unique_ptr<ss::http::request> req) {
    ...
    vlog(alogger.debug, "Updating user {}:{}", err, err.message());
    co_await throw_on_error(*req, err, model::controller_ntp);
    co_return ss::json::json_return_type(ss::json::json_void());
}
...
admin_server::mark_transaction_expired_handler(
  std::unique_ptr<ss::http::request> req) {
    ...
    vlog(alogger.info, "Mark transaction expired for pid:{}", pid);
...
admin_server::self_test_start_handler(std::unique_ptr<ss::http::request> req) {
    ...
    if (need_redirect_to_leader(model::controller_ntp, _metadata_cache)) {
        vlog(alogger.debug, "Need to redirect self_test_start request");
        throw co_await redirect_to_leader(*req, model::controller_ntp);
    }
    auto doc = co_await parse_json_body(req.get());
    ...
    try {
        ...
        vlog(alogger.info, "Request to start self test succeeded: {}", tid);
...
admin_server::self_test_stop_handler(std::unique_ptr<ss::http::request> req) {
    if (need_redirect_to_leader(model::controller_ntp, _metadata_cache)) {
        vlog(alogger.info, "Need to redirect self_test_stop request");
        throw co_await redirect_to_leader(*req, model::controller_ntp);
    }
    ...
    vlog(alogger.info, "Request to stop self test succeeded");
    co_return ss::json::json_void();
}
...
void admin_server::register_debug_routes() {
    register_route<user>(
      ...
      [this](std::unique_ptr<ss::http::request>) {
          vlog(alogger.info, "Stopping stress fiber");
    ...
    register_route<user>(
      ...
      [this](std::unique_ptr<ss::http::request> req) {
          vlog(alogger.info, "Requested stress fiber");
          stress_config cfg;
          ...
          return _stress_fiber_manager
            .invoke_on_all([cfg](auto& stress_mgr) {
                ...
                if (ran) {
                    vlog(alogger.info, "Started stress fiber...");
                } else {
                    vlog(alogger.info, "Stress fiber already running...");
                }
            })
            .then(
              [] { return ss::json::json_return_type(ss::json::json_void()); });
    ...
    register_route<user>(
      ...
      [this](std::unique_ptr<ss::http::request>) {
          vlog(alogger.info, "Request to reset leaders info");
    ...
    register_route<user>(
      ...
      [this](std::unique_ptr<ss::http::request>) {
          vlog(alogger.info, "Request to refresh disk health info");
    ...
    register_route<user>(
      ...
      [this](std::unique_ptr<ss::http::request>) {
          vlog(alogger.info, "Request to get leaders info");
...
admin_server::get_partition_balancer_status_handler(
  std::unique_ptr<ss::http::request> req) {
    vlog(alogger.debug, "Requested partition balancer status");
...
void admin_server::register_cluster_routes() {
    register_route<publik>(
      ...
      [this](std::unique_ptr<ss::http::request>) {
          vlog(alogger.debug, "Requested cluster status");
    ...
    register_route_sync<publik>(
      ...
      [this](ss::httpd::const_req) -> ss::json::json_return_type {
          vlog(alogger.debug, "Requested cluster UUID");
...
ss::future<ss::json::json_return_type> admin_server::sync_local_state_handler(
  std::unique_ptr<ss::http::request> request) {
    ...
    vlog(alogger.info, "Requested bucket syncup");
    auto ntp = parse_ntp_from_request(request->param, model::kafka_namespace);
    if (need_redirect_to_leader(ntp, _metadata_cache)) {
        vlog(alogger.info, "Need to redirect bucket syncup request");
        ...
        vlog(alogger.info, "Requested bucket syncup completed");
        if (result) {
            std::stringstream sts;
            result->serialize_json(sts);
            vlog(alogger.info, "Requested bucket syncup result {}", sts.str());
        } else {
            vlog(alogger.info, "Requested bucket syncup result empty");
        }
    }
    co_return ss::json::json_return_type(ss::json::json_void());
}
...
admin_server::unsafe_reset_metadata(
  ...
  std::unique_ptr<ss::http::reply> reply) {
    ...
    auto ntp = parse_ntp_from_request(request->param, model::kafka_namespace);
    if (need_redirect_to_leader(ntp, _metadata_cache)) {
        vlog(alogger.info, "Need to redirect unsafe reset metadata request");
        throw co_await redirect_to_leader(*request, ntp);
    }
...
admin_server::cpu_profile_handler(std::unique_ptr<ss::http::request> req) {
    vlog(alogger.info, "Request to sampled cpu profile");
...
admin_server::restart_service_handler(std::unique_ptr<ss::http::request> req) {
    ...
    vlog(
      alogger.info, "Restart redpanda service: {}", to_string_view(*service));
...
admin_server::sampled_memory_profile_handler(
  std::unique_ptr<ss::http::request> req) {
    vlog(alogger.info, "Request to sampled memory profile");
