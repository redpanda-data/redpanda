...
// in the whole program and that after it is invoked the original handler is
// restored.
...
void install_oneshot_signal_handler() {
    ...
    auto r = ::sigaction(Signal, nullptr, &original_sa);
    if (r == -1) {
    ...
    sa.sa_sigaction = [](int sig, siginfo_t*, void*) {
        ...
        if (!handled) {
            ...
            if (r == -1) {
                // This shouldn't happen, but fall back to reinstalling the
                // default signal handler to be more graceful
                ::signal(sig, SIG_DFL);
            }
            // Trigger the original signal handler by reraising the signal
...
void record_sigabrt_crash() {
    crash_tracker::get_recorder().record_crash_sighandler(SIGABRT);
}
void record_sigill_crash() {
    crash_tracker::get_recorder().record_crash_sighandler(SIGILL);
}

void install_sighandlers() {
    install_oneshot_signal_handler<SIGSEGV, record_sigsegv_crash>();
    install_oneshot_signal_handler<SIGABRT, record_sigabrt_crash>();
    install_oneshot_signal_handler<SIGILL, record_sigill_crash>();
}

void application::init_crashtracker() {
    construct_single_service(_crash_tracker_service);
    _crash_tracker_service->start().get();
    install_sighandlers();
}
...
void application::schedule_crash_tracker_file_cleanup() {
    // Schedule a deletion of the tracker file.
