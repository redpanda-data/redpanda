...
struct redpanda_cfg {
  ...
  raft_cfg
  raft() const {
    return raft_cfg{id, min_version, max_version,
                    seed_server_meta_topic_partitions, seed_servers};
  }
...
struct convert<::v::raft_seed_server> {
  ...
  static bool
  decode(const Node &node, type &rhs) {
    // Required fields
    for (auto s : {"id", "addr"}) {
...
struct convert<::v::redpanda_cfg> {
  ...
  decode(const Node &node, ::v::redpanda_cfg &rhs) {
    ...
    auto parse = [&node](const char *field, auto &v) {
      if (node[field]) { v = node[field].as<std::decay_t<decltype(v)>>(); }
    };
    ...
    parse("seed_server_meta_topic_partitions",
          rhs.seed_server_meta_topic_partitions);
    parse("ip", rhs.ip);
    parse("port", rhs.port);
    parse("id", rhs.id);
