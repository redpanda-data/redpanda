...
size_t cloud_topics_reconciler_memory_reservation::reserved_bytes(
  size_t total_memory) const {
    size_t bytes_limit = static_cast<size_t>(
      total_memory * (max_limit_pct / 100.0));
    return std::min(max_bytes, bytes_limit);
}
...
system_memory_groups::system_memory_groups(
  ...
  , _cloud_topics_reconciler_reserved_memory(
      cloud_topics_reconciler.reserved_bytes(total_available_memory))
...
void system_memory_groups::log_memory_group_allocations(seastar::logger& log) {
    log.info(
      ...
      "cloud topics compaction: {}, cloud topics reconciler: {}, datalake: {}, "
      "partitions: {}",
...
system_memory_groups& memory_groups() {
    ...
    // Reserve memory for reconciling domains in parallel.
    // The number of domains is set in domain_supervisor.cc.
    constexpr size_t num_domains = 3;
    cloud_topics_reconciler_memory_reservation cloud_topics_reconciler{
      .max_bytes = num_domains
                   * cfg.cloud_topics_reconciliation_max_object_size.value(),
      .max_limit_pct = 10.0};
    partitions_memory_reservation partitions{
