...
public:
    ...
    echo_service(scheduling_group sc, smp_service_group ssg)
      : _sc(sc)
      , _ssg(ssg) {}
    ...
    raw_prefix_echo(input_stream<char>& in, rpc::streaming_context& ctx) {
        ...
        return fapply.exec(
          ...
          [this](echo_req&& t, rpc::streaming_context& ctx)
            -> future<echo_resp> { return prefix_echo(std::move(t), ctx); });
    }
    virtual future<echo_resp> prefix_echo(echo_req&&, rpc::streaming_context&) {
        throw std::runtime_error("unimplemented method");
    }
    /// \brief echo_req -> echo_resp
    virtual future<rpc::netbuf>
    raw_suffix_echo(input_stream<char>& in, rpc::streaming_context& ctx) {
        auto fapply = execution_helper<echo_req, echo_resp>();
        return fapply.exec(
          in,
          ctx,
          1296043586,
          [this](echo_req&& t, rpc::streaming_context& ctx)
            -> future<echo_resp> { return suffix_echo(std::move(t), ctx); });
    }
