...
public:
    team_movistar_service(ss::scheduling_group sc, ss::smp_service_group ssg)
      : _sc(sc)
      , _ssg(ssg) {}
    ...
    raw_canyon(ss::input_stream<char>& in, rpc::streaming_context& ctx) {
        ...
        return fapply.exec(
          ...
          [this](ultimate_cf_slx&& t, rpc::streaming_context& ctx)
            -> ss::future<nairo_quintana> {
              return canyon(std::move(t), ctx);
          });
    }
    virtual ss::future<nairo_quintana>
    canyon(ultimate_cf_slx&&, rpc::streaming_context&) {
        throw std::runtime_error("unimplemented method");
    }
    /// \brief san_francisco -> mount_tamalpais
    virtual ss::future<rpc::netbuf>
    raw_ibis_hakka(ss::input_stream<char>& in, rpc::streaming_context& ctx) {
        ...
        return fapply.exec(
          ...
          [this](san_francisco&& t, rpc::streaming_context& ctx)
            -> ss::future<mount_tamalpais> {
              return ibis_hakka(std::move(t), ctx);
          });
    }
    virtual ss::future<mount_tamalpais>
