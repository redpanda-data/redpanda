      ...
      [this, b = std::move(b), opts = std::move(opts), seq]() mutable {
          ...
          return get_units(_memory, sz)
            ...
            .then_unpack(
              [this, f = std::move(f), seq, corr](
                ...
                ss::scattered_message<char> scattered_message) mutable {
                  ...
                  // we still do dispatch_send where it is handled. This is needed
                  // for two reasons:
            ...
            .finally([this, seq] {
                vlog(
                  rpclog.trace,
                  "Request with sequence: {} done, last_seq: {}",
                  seq,
                  _last_seq);
            });
      });
              ...
              return ss::do_until(
                ...
                [this, trigger_seq] {
                    ...
                    if (resp_it == _correlations.end()) {
                        ...
                        // (probably due to timeout or disconnect). We don't
                        // need to do anything.
                        return ss::now();
                    }
                    ...
                    // delivery of the dispatched write calls, which is the
                    // intent of holding on to the units up until this point.
