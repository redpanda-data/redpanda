...
std::string iobuf_to_string(const iobuf& buf) {
    std::string result;
    result.reserve(buf.size_bytes());
    for (const auto& frag : buf) {
        result.append(frag.get(), frag.size());
    }
    return result;
}
...
TEST(ParquetWriter, StatsTruncationUtf8CharBoundary) {
    ...
    EXPECT_EQ(iobuf_to_string(stats->min->value), "hello");
    EXPECT_FALSE(stats->min->is_exact);

    // Max, if present, must be valid UTF-8 and strictly greater than "hello".
    if (stats->max.has_value()) {
        std::string max_str = iobuf_to_string(stats->max->value);
        EXPECT_GT(max_str, std::string("hello"));
...
TEST(ParquetWriter, StatsTruncationUtf8MaxNoInvalidByteIncrement) {
    ...
    EXPECT_EQ(iobuf_to_string(stats->min->value), "a\xC3\xBF");
    // Must be "aĀ" (valid UTF-8), not "a\xC3\xC0" (invalid).
    EXPECT_EQ(iobuf_to_string(stats->max->value), std::string("a\xC4\x80"));
    EXPECT_FALSE(stats->max->is_exact);
}
...
TEST(ParquetWriter, StatsTruncationUtf8InvalidInput) {
    ...
    EXPECT_EQ(iobuf_to_string(stats->min->value), "abc");
    EXPECT_FALSE(stats->min->is_exact);

    // Max is optional (nullopt when no valid upper bound). If present it must
    // be valid UTF-8 and strictly greater than the min.
    if (stats->max.has_value()) {
        std::string max_str = iobuf_to_string(stats->max->value);
        EXPECT_GT(max_str, std::string("abc"));
        EXPECT_FALSE(stats->max->is_exact);
    }
}
