...
template<typename... Args>
struct is_absl_flat_hash_map<absl::flat_hash_map<Args...>> : std::true_type {};
template<typename T>
inline constexpr bool is_absl_flat_hash_map_v = is_absl_flat_hash_map<T>::value;

template<typename T>
struct is_absl_btree_set : std::false_type {};
template<typename... Args>
struct is_absl_btree_set<absl::btree_set<Args...>> : std::true_type {};
template<typename T>
...
void write(iobuf& out, T t) {
    ...
    if constexpr (is_envelope_v<Type>) {
      ...
      is_absl_node_hash_set_v<Type> || is_absl_btree_set_v<Type>) {
        if (unlikely(t.size() > std::numeric_limits<serde_size_t>::max())) {
            throw serde_exception(fmt_with_ctx(
              ssx::sformat,
              "serde: absl::node_(hash|btree)_set size {} exceeds serde_size_t",
              t.size()));
        }
        write(out, static_cast<serde_size_t>(t.size()));
        for (auto& e : t) {
      ...
      is_absl_node_hash_map_v<Type> || is_absl_flat_hash_map_v<Type>) {
        if (unlikely(t.size() > std::numeric_limits<serde_size_t>::max())) {
            throw serde_exception(fmt_with_ctx(
              ssx::sformat,
              "serde: absl::node_(hash|flat)_map size {} exceeds serde_size_t",
              t.size()));
        }
        write(out, static_cast<serde_size_t>(t.size()));
        for (auto& v : t) {
...
void read_nested(iobuf_parser& in, T& t, std::size_t const bytes_left_limit) {
    ...
    } else if constexpr (is_absl_btree_set_v<Type>) {
        const auto size = read_nested<serde_size_t>(in, bytes_left_limit);
        for (auto i = 0U; i < size; ++i) {
            auto elem = read_nested<typename Type::key_type>(
              in, bytes_left_limit);
