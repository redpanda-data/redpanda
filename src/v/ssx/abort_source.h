...
public:
    ...
    auto& local() { return _as.local(); }
    auto const& local() const { return _as.local(); }
    ...
    ss::future<> request_abort_ex(std::exception_ptr ex) {
        return _as.invoke_on_all(
    ...
    auto abort_requested() const { return local().abort_requested(); }
