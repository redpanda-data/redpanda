...
TEST(rwlock_test, rwlock_test) {
    ...
    write_holder.reset();
    EXPECT_TRUE(stream_contains(true, outer_os, "Dropping lock on c-lock (0)"));
}

TEST(rwlock_test, rwlock_test_no_log_after_move) {
    ss::logger captured_log("captured_log");
    seastar::global_logger_registry().set_logger_level(
      "captured_log", ss::log_level::trace);
    auto revert_stream = ss::defer(
