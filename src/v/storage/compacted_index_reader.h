...
public:
    ...
    public:
        ...
        ss::future<>
        for_each(Func f, model::timeout_clock::time_point timeout) {
            while (true) {
    ...
    template<typename Func>
    ss::future<> for_each(Func f, model::timeout_clock::time_point timeout) {
        return _impl->for_each(std::move(f), timeout);
    }
