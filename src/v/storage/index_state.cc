...
void compressed_index_columns::pop_back(int n) {
    ...
    std::for_each_n(_relative_offset_index.begin(), expected_size, [&tmp_offsets] (uint64_t o) {
        tmp_offsets.append(o);
    });
    std::for_each_n(_relative_time_index.begin(), expected_size, [&tmp_timestamps] (uint64_t t) {
        tmp_timestamps.append(t);
    });
    std::for_each_n(_position_index.begin(), expected_size, [&tmp_positions] (uint64_t p) {
...
compressed_index_columns compressed_index_columns::copy() const {
    ...
    for_each_relative_offset_index([&res] (uint32_t n) {
        res._relative_offset_index.append(n);
    });
    for_each_relative_time_index([&res] (uint32_t n) {
        res._relative_time_index.append(n);
    });
    for_each_position_index([&res] (uint64_t n) {
        res._position_index.append(n);
    });
    return res;
}
...
void index_state::pop_back(int n) {
    index.pop_back(n);
    if (empty()) {
        non_data_timestamps = false;
    }
...
iobuf index_state_serde::encode(const index_state& st) {
    ...
    // }
    st.index.for_each_position_index([&out] (uint64_t n){
...
bool index_state::truncate(
  model::offset new_max_offset, model::timestamp new_max_timestamp) {
    ...
    if (res != std::nullopt) {
        ...
        int remove_back_elems = res.value();
