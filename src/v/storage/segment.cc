...
ss::future<append_result> segment::append(const model::record_batch& b) {
    // Common case (non-transactional data): append directly and avoid a
    // coroutine frame on the per-batch write path.
    if (unlikely(b.contains_transactional_data() && has_compaction_index())) {
        return append_marking_compaction_index_incomplete(b);
    }
    return do_append(b);
}

ss::future<append_result> segment::append_marking_compaction_index_incomplete(
