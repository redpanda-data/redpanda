...
/**
 * @brief Defines the external_proces::error_category
 */
...
public:
     ...
     * KEY=VALUE
     * @return A created external process
     * @throw std::system_error If unable to launch the process
     */
    static ss::future<external_process> create_external_process(
    ...
    external_process(external_process&&) noexcept = default;
    external_process& operator=(external_process&&) noexcept = delete;

    ~external_process() {
        // vassert(_completed, "Process has not yet completed");
        vassert(
          !_stdout_consumer.has_value(), "stdout consumer not cleaned up");
        vassert(
          !_stderr_consumer.has_value(), "stderr consumer not cleaned up");
    }
     ...
     *
     * @note Must call this _after_ calling `wait()`
     *
