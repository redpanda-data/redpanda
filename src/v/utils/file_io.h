...
static inline ss::future<ss::temporary_buffer<char>> read_fully_tmpbuf(ss::sstring name) {
    return ss::open_file_dma(std::move(name), ss::open_flags::ro).then([](ss::file f) {
        return f.size()
