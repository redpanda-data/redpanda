...
model::record_batch convert_to_record_batch(
  model::record_batch_header header, std::list<record_wrapper>& records) {
    std::vector<model::record> vector_records;
    iobuf serialize_records; // Need to delete this. Only is used for final size
    ...
    for (auto& r : records) {
        ...
        model::append_record_to_buffer(serialize_records, new_record);
        vector_records.emplace_back(std::move(new_record));
    }

    header.size_bytes = serialize_records.size_bytes()
                        + model::packed_record_batch_header_size;
    header.record_count = records.size();
    header.crc = model::crc_record_batch(header, serialize_records);
    return model::record_batch(header, std::move(vector_records));
}

model::record_batch record_batch_wrapper::get_output_batch() {
    return convert_to_record_batch(header, output_records);
}
