...
func merge(ms ...kafka.ConfigMap) kafka.ConfigMap {
    res := make(kafka.ConfigMap)
    for _, m := range ms {
        for k, v := range m {
            res[k] = v
        }
...
func CreateTestClient(brokers *string,
	...
	if securitySettings != nil {
        producerConfig = merge(producerConfig, securitySettings.GenerateConfig())
	}
	...
	if securitySettings != nil {
        consumerConfig = merge(consumerConfig, securitySettings.GenerateConfig())
	}
