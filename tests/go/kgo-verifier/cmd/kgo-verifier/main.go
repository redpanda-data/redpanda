...
var (
	...
	exactlyOnceConsumption = flag.Bool("consume-exactly-once", true, "If true, the consumer will expect to read each record offset exactly once.")
...
func main() {
	...
	if *pCount > 0 {
		...
		), verifier.NewValidatorStatus(*compacted, *validateLatestValues, *exactlyOnceConsumption, *topic, nPartitions))
		workers.Add(&srw)
		workers.SetReady()
		...
		for i := 0; i < *parallelRead; i++ {
			...
			worker := verifier.NewRandomReadWorker(workerCfg, verifier.NewValidatorStatus(*compacted, *validateLatestValues, *exactlyOnceConsumption, *topic, nPartitions))
		...
		grw := verifier.NewGroupReadWorker(
			verifier.NewGroupReadConfig(
				...
				*seqConsumeCount, (*consumeTputMb)*1024*1024, *cgMaxUncommitted, *cgSessionTimeout, *cgRebalanceTimeout), verifier.NewValidatorStatus(*compacted, *validateLatestValues, *exactlyOnceConsumption, *topic, nPartitions))
		workers.Add(&grw)
		workers.SetReady()
