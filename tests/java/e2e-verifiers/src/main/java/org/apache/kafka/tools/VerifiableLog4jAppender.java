 ...
 * message to Kafka on each "append" request. For example, this helps with
 * end-to-end tests of KafkaLog4jAppender.
 *
 ...
 * used, in which case it appends indefinitely.
 */
...
public class VerifiableLog4jAppender {
  ...
  public static VerifiableLog4jAppender createFromArgs(String[] args) {
    ...
    try {
      ...
      props.setProperty(
          "log4j.appender.KAFKA.BrokerList", res.getString("brokerList"));
      props.setProperty("log4j.appender.KAFKA.Topic", topic);
      props.setProperty(
          "log4j.appender.KAFKA.RequiredNumAcks", res.getString("acks"));
      ...
      if (securityProtocol != null
        ...
        props.setProperty(
            "log4j.appender.KAFKA.SecurityProtocol", securityProtocol);
      }
      if (securityProtocol != null && securityProtocol.contains("SSL")) {
        props.setProperty(
            "log4j.appender.KAFKA.SslTruststoreLocation",
            res.getString("sslTruststoreLocation"));
        props.setProperty(
            "log4j.appender.KAFKA.SslTruststorePassword",
            res.getString("sslTruststorePassword"));
      }
      if (securityProtocol != null && securityProtocol.contains("SASL")) {
        props.setProperty(
            "log4j.appender.KAFKA.SaslKerberosServiceName",
            res.getString("saslKerberosServiceName"));
        props.setProperty(
            "log4j.appender.KAFKA.clientJaasConfPath",
      ...
      // Changing log level from INFO to WARN as a temporary workaround for
      // KAFKA-6415. This is to avoid deadlock in system tests when producer
      // network thread appends to log while updating metadata.
      props.setProperty(
  ...
  public static void main(String[] args) {
    ...
    Exit.addShutdownHook(
        "verifiable-log4j-appender-shutdown-hook",
  ...
  private void append(String msg) { logger.info(msg); }
}
