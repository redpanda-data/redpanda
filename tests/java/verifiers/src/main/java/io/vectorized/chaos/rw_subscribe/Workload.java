...
public class Workload {
    ...
    private synchronized long get_id() {
        return ++this.last_id;
    }
        ...
        while (is_active) {
            ...
            synchronized(this) {
                if (op.status == OpProduceStatus.WRITING) {
                    ...
                    if (op.offset != offset) {
                        violation(pid, "written " + op.oid + "@" + offset + "/" + op.partition + " was already seen by offset:" + op.offset);
                    }
                } else {
                    violation(pid, "unexpected status: " + op.status.name());
                }
    ...
    private void consumingProcess(int rid) throws Exception {
        ...
        while (is_active) {
            ...
            while (it.hasNext()) {
                ...
                log(rid, "read\t" + record.partition() + "\t" + record.offset() + "\t" + record.key() + "\t" + oid);
                ...
                synchronized (this) {
                    if (last_oid.get(record.partition()) == -1L) {
                        ...
                        if (!commits.known_commits.containsKey(record.offset())) {
                            ...
                            if (!match) {
                                ...
                                // but before we should check if any offset commit was already known
                                // before poll started

                                for (var id : commits.passed_commits.keySet()) {
                                    var passed_commit = commits.passed_commits.get(id);
                                    if (passed_commit.ended < poll_ts) {
                                        violation(rid, "jump to offset:" + record.offset() + " while at the time poll started offset:" + passed_commit.offset + " was already known");
                                    }
                                }
                                ...
                                if (record.offset() != first_offset.get(record.partition())) {
                                    violation(rid, "started reading from offset:" + record.offset() + " which wasn't committed and isn't the first offset in the partition");
                                }
                            }
                        }
                    }
                    ...
                    if (record.offset() < first_offset.get(record.partition())) {
                        violation(rid, "detected offset:" + record.offset() + " jumping before first offset:" + first_offset.get(record.partition()));
                    }
                    ...
                    if (last_oid.get(record.partition()) != -1L) {
                        ...
                        if (prev_op.next_oid != oid) {
                            violation(rid, "observed " + prev_oid + "->" + oid + " while " + prev_oid + "->" + prev_op.next_oid + " was already known");
                        }
                        if (op.prev_oid == -1) {
                            op.prev_oid = prev_oid;
                        }
                        if (op.prev_oid != prev_oid) {
                            violation(rid, "observed " + prev_oid + "->" + oid + " while " + op.prev_oid + "->" + oid + " was already known");
                        }
                    }
                ...
                processed_records.put(record.partition(), processed_records.get(record.partition()) + 1);
                ...
                if (processed_records.get(record.partition())==10) {
                    ...
                    commit.ended = -1;
                    offsets.put(new TopicPartition(args.topic, record.partition()), new OffsetAndMetadata(commit.offset));
                    
                    try {
                        log(rid, "commit\t" + record.partition() + "\t" + (last_offset.get(record.partition()) + 1));
                        synchronized (this) {
                            var commits = partition_commits.get(record.partition());
                ...
                synchronized (this) {
                    ...
                    if (op.status == OpProduceStatus.WRITING) {
                        ...
                        if (op.offset != record.offset()) {
                            violation(rid, "" + oid + "@" + record.offset() + " has cached offset:" + op.offset);
                        }
                        op.status = OpProduceStatus.SEEN;
                        succeeded(rid);
                        log(rid, "seen\t" + oid + "\t" + (seen_us - op.started_us));
