...
class CloudCleanup:
    ...
    def __init__(self, log_level=logging.INFO):
        ...
        if "cloud_cluster" not in ducktape_globals:
            self.log.warn("# WARNING: Cloud cluster configuration not present, exiting")
            sys.exit(1)
        ...
        if self.config.provider == "AWS":
            ...
            self.log.error(f"# ERROR: Provider {self.config.provider} not supported")
            sys.exit(1)
        # Initialize clients
        self.provider = make_provider_client(
            self.config.provider, self.log, self.config.region, keyId, secret
        )
        ...
        if cluster_type in ["FMC"]:
            ...
            if cluster["state"] in ["deleting_agent"]:
                ...
                try:
                    ...
                    message += (
                        f"| ERROR: failed to delete agent for cluster '{id}': {e}"
                    )
                    self.log.info(message)
                    return False
                # All good
            ...
            # TODO: Implement deletion on rare non-agent statuses
                ...
                message += (
                    "| WARNING: Cluster deletion with status "
    ...
    def _pooled_delete(self, pool, resource_name, func, queue):
        ...
        if queue_len < 1:
            ...
            self.log.info(f"# Clusters: {ok} deleted, {failed} failed, {unsure} other")
            return
    ...
    def clean_namespaces(self, pattern, uuid_len):
        ...
        self.log.info(
            f"  {len(ns_list)} total namespaces found. Filtering with '{pattern}*'"
        )
        ...
        self.log.info(f"# Searching for resources in {len(ns_list)} namespaces")
        for ns in ns_list:
            ...
            if any(counts):
                ...
                if counts[0]:
                    for cluster in clusters:
                        ...
                        cl_queue += [self.cloudv2.cluster_endpoint(cluster["id"])]
            ...
            if _ns_36h_skip_flag:
                self.log.info(f"  skipped '{ns['name']}', 36h delay not passed")
                continue
            else:
        ...
        if not self.delete_peerings:
            self.log.info(f"# {len(npr_queue)} network peerings could be deleted")
        else:
            self._pooled_delete(
                pool, "Network peerings", self.cloudv2.delete_resource, npr_queue
            )
        ...
        if not self.delete_clusters:
            ...
            self._pooled_delete(pool, "Clusters", self._delete_cluster, cl_queue)
        ...
        if not self.delete_networks:
            ...
            self._pooled_delete(
                pool, "Networks", self.cloudv2.delete_resource, net_queue
            )
        ...
        if not self.delete_namespaces:
            ...
            self._pooled_delete(
                pool, "Namespaces", self.cloudv2.delete_resource, ns_queue
            )
    ...
    def _delete_nat(self, nat):
        ...
        for _address in nat["NatGatewayAddresses"]:
            ...
            try:
                ...
                self.log.error(
                    f"ERROR: Failed to release '{_address['PublicIp']}': {e}"
                )
    ...
    def clean_aws_networks(self):
        ...
        def get_net_id_from_nat(nat):
            for tag in nat["Tags"]:
                if tag["Key"] == "Name" and tag["Value"].startswith("network-"):
                    return tag["Value"].split("-")[1]

        def get_date_from_resource(aws_res):
            if "CreateTime" in aws_res:
                return datetime.strftime(aws_res["CreateTime"], "%Y-%m-%d %H:%M:%S")
            else:
                return "Unknown"
        ...
        def check_cloud_network(aws_res):
            ...
            try:
                ...
                r = self.cloudv2.get_network(net_id, quite=True)  # type: ignore
                return r
            except Exception:
        ...
        def filter_nats(nats):
            ...
            for nat in nats:
                if nat["State"] == "deleted":
                    ...
                    self.log.info(f"# Nat {nat['NatGatewayId']} deleted, IPs: {ips}'")
                    continue
        ...
        if self.config.provider == "GCP":
            self.log.warning("Network resources cleaning not yet supported for GCP")
            return False

        # list AWS networks with specific tag
        uuids = {
            "https://cloud-api.ppd.cloud.redpanda.com": "0a9923e1-8b0f-4110-9fc0-6d3c714cc270",
            "https://cloud-api.ign.cloud.redpanda.com": "a845616f-0484-4506-9638-45fe28f34865",
        }
        # List NAT gateways
        ...
        self.log.info(f"# {len(nat_filtered)} orphaned NATs for {self.config.api_url}")
        self.log.info(f"# {len(vpc_filtered)} orphaned VPCs for {self.config.api_url}")

        # Fancy threading
        # Due to RPS limiter on AWS side, please, do not use >5
        pool = ThreadPoolExecutor(max_workers=5)
        self.log.info("\n\n# Cleaning up")
        self._pooled_delete(pool, "NAT Gateways", self._delete_nat, nat_filtered)
    ...
    def clean_buckets(self, mask=None, eight_hours=False):
        ...
        if not mask:
            self.log.info(
                "# Bucket mask is empty. Will not delete all buckets for account"
            )
            return
        else:
            ...
            for bucket in buckets:
                ...
                if last_modified.timestamp() > ts.timestamp():  # type: ignore
                    created_date = last_modified.strftime(ns_name_date_fmt)  # type: ignore
                ...
                if bucket_count >= 3000:
                    ...
                    # instead of old one
                    self.provider.set_bucket_lifecycle_configuration(bucket["Name"])
                    self.log.info(msg)
                else:
                    # Delete objects
                    if bucket_count > 0:
                    ...
                    iters = 10
                    while iters > 0:
                        count = self.provider.list_bucket_objects(bucket["Name"])
                        count = len(count)
                        if count == 0:
                            break
                        time.sleep(10)
