...
def build_connection_string(
    proto: str, endpoint: Optional[str], storage_account: str, key: str
) -> str:
...
class ABSClient:
    def __init__(
        ...
        self.conn_str = build_connection_string(
            proto, endpoint, storage_account, shared_key
        )
    ...
    def _wait_no_key(self, blob_client: BlobClient, timeout_sec: float = 10):
        ...
        try:
            while blob_client.exists():
                ...
                if now > deadline:
                    raise TimeoutError(f"Blob was not deleted within {timeout_sec}s")
                time.sleep(2)
    ...
    def delete_object(self, bucket: str, key: str, verify=False):
        blob_client = BlobClient.from_connection_string(
            self.conn_str, container_name=bucket, blob_name=key
        )
    ...
    def list_objects(
        self, bucket: str, topic: Optional[str] = None, prefix: Optional[str] = None
    ...
    def create_expiration_policy(self, bucket: str, days: int):
        raise Exception("create_expiration_policy not implemented for ABSClient")
