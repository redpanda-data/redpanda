...
class RpkRemoteTool:
    ...
    def cluster_config_force_reset(self, property_name):
        return self._execute(
            [self._rpk_binary(), "cluster", "config", "force-reset", property_name]
        )
    ...
    def create_profile_redpanda(self, name, cfg_location):
        return self._run_profile(["create", name, "--from-redpanda", cfg_location])
    ...
    def read_timestamps(self, topic: str, partition: int, count: int, timeout_sec: int):
        """
        ...
        for line in self._node.account.ssh_capture(
            " ".join(cmd), timeout_sec=timeout_sec
        ):
            try:
