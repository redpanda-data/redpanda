...
class ConfigProfileVerifyTest(Test):
    ...
    def __init__(self, test_context):
        super(ConfigProfileVerifyTest, self).__init__(test_context=test_context)
        self.redpanda = make_redpanda_service(test_context, 3)
        self._ctx = test_context
        self._ipClient = InstallPackClient(self.redpanda._cloud_cluster.config.install_pack_url_template,
    ...
    def setUp(self):
        ...
        self._configProfile = self._ip['config_profiles'][self.redpanda._cloud_cluster.config.config_profile_name]
    ...
    def _check_rp_config(self):
        confRes = self.redpanda._kubectl.exec("rpk redpanda admin config print --host 0")
        clusterConfig = json.loads(confRes)
        self.logger.debug("asserting we got the config for the right cluster: expected rp-{}, actual: {}".format(self._clusterId, clusterConfig["cluster_id"]))
        assert "rp-{}".format(self._clusterId) == clusterConfig['cluster_id']

        for k, v in self._configProfile["cluster_config"].items():
            self.logger.debug("asserting cluster config key {} has expected value: {}  actual: {}".format(k, v, clusterConfig[k]))
    ...
    def _check_gcp_nodes(self):
        cmd = self.redpanda._kubectl._ssh_prefix() + [
            ...
            '"tags.items=redpanda-node tags.items=redpanda-{}"'.format(self._clusterId),
            '--format="json(name,machineType,disks)"'
        ]
        res = subprocess.check_output(cmd)
        resd = json.loads(res)

        self.logger.debug("asserting machineType: expected: {}, actual: {}".format(self._configProfile['machine_type'], resd[0]['machineType']))
        assert resd[0]['machineType'].endswith(self._configProfile['machine_type'])

        self.logger.debug("asserting nodes_count: expected: {}, actual: {}".format(self._configProfile['nodes_count'], len(resd)))
        ...
        for n in resd:
            ...
            self.logger.debug("asserting storage for redpanda node {}: expected: {}, actual: {}".format(n["name"], self._configProfile["storage_size_bytes"], total))
            assert total == self._configProfile['storage_size_bytes']

    def _check_aws_nodes(self):
            cmd = self.redpanda._kubectl._ssh_prefix() + [
                    'aws', 'ec2', 'describe-instances',
                    '--filters="Name=tag:Name, Values=redpanda-{}-rp"'.format(self._clusterId),
            ...
            self.logger.debug("asserting machineType: expected: {}, actual: {}".format(self._configProfile['machine_type'], resd[0]))
