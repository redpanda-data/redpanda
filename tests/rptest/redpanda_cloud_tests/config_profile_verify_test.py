...
class ConfigProfileVerifyTest(RedpandaCloudTest):
    ...
    def _check_rp_config(self):
        ...
        self.logger.debug(
            "asserting we got the config for the right cluster: expected rp-{}, actual: {}".format(
                self._clusterId, clusterConfig["cluster_id"]
            )
        )
        assert clusterConfig["cluster_id"] in (self._clusterId, f"rp-{self._clusterId}")
    ...
    def _check_azure_nodes(self):
        ...
        assert len(resd) == nc, f"expected nodes_count: {nc}, actual: {len(resd)}"
