...
class HighThroughputTest(PreallocNodesMixin, RedpandaCloudTest):
    ...
    def stage_decommission_and_add(self):
        ...
        agent_services = ["redpanda-agent.service", "redpanda-agent-boot.service"]
        if self.redpanda._cloud_cluster.config.provider == PROVIDER_AWS:
        ...
        deployment_args = deployment_args.replace(
        ...
        wait_until(
            lambda: deployment_ready_replicas() == 1, timeout_sec=600, backoff_sec=1
        )
        ...
        pvc_name = (
            f"datadir-rp-{self.redpanda._cloud_cluster.cluster_id}-{new_replicas}"
        )
        ...
        patch = [{"op": "replace", "path": "/spec/replicas", "value": orig_replicas}]
        patch_str = json.dumps(patch)
    ...
    def _patch_cluster_replicas(self, cluster_name, replicas):
        ...
        wait_until(
            ...
            backoff_sec=1,
            retry_on_exc=True,
            err_msg=f"number of ready replicas for {cluster_name} did not arrive at {replicas}",
        )
