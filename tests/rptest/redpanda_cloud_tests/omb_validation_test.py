...
class OMBValidationTest(RedpandaCloudTest):
    ...
    def test_max_connections(self):
        ...
        # Before the test even starts, assert that there are fewer than 100 connections
        # this number is not zero because various things maintain or make occasional connections
        # to cloud clusters like kminion, so I observer typically ~45 connections even on an
        # idle cluster. If this fails it is probably because some other test left services running
        # against the cluster.
        count_before = self._connection_count()
        assert count_before <= 100, f"Wanted less than 100 connections to start but got {count_before}"
