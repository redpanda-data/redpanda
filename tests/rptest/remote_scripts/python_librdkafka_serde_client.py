...
JsonSchemas = {
    "com.redpanda.Payload": json.dumps(
        {
    ...
    "com.redpanda.A.B.C.D.NestedPayload": json.dumps(
        {
    ...
    "com.redpanda.CompressiblePayload": json.dumps(
        {
            "title": "com.redpanda.CompressiblePayload",
            "type": "object",
            "properties": {"val": {"type": "integer"}, "message": {"type": "string"}},
        }
    ),
}
...
subject_name_strategies = {
    "io.confluent.kafka.serializers.subject.TopicNameStrategy": topic_subject_name_strategy,
    "io.confluent.kafka.serializers.subject.RecordNameStrategy": record_subject_name_strategy,
    "io.confluent.kafka.serializers.subject.TopicRecordNameStrategy": topic_record_subject_name_strategy,
}
...
class SerdeClient:
    ...
    def _make_serializer(self):
        return {
            SchemaType.AVRO: AvroSerializer(
                self.sr_client, self.avro_schema, conf=self.avro_serde_config
            ),
            SchemaType.PROTOBUF: ProtobufSerializer(
                self.protobuf_payload_class, self.sr_client, self.proto_serde_config
            ),
            SchemaType.JSON: JSONSerializer(
                self.json_schema,
    ...
    def _make_deserializer(self):
        return {
            SchemaType.AVRO: AvroDeserializer(
                self.sr_client,
                self.avro_schema,
                from_dict=lambda d, _: AvroPayload(d["val"]),
            ),
            SchemaType.PROTOBUF: ProtobufDeserializer(
                self.protobuf_payload_class, {"use.deprecated.format": False}
            ),
            SchemaType.JSON: JSONDeserializer(
                self.json_schema,
                schema_registry_client=self.sr_client,
    ...
    def _make_payload(self, val: int):
        return {
            ...
            SchemaType.PROTOBUF: make_protobuf_payload(
                self.protobuf_payload_class, val
            ),
            SchemaType.JSON: JsonPayload(val),
        }[self.schema_type]
    ...
    def produce(self, count: int):
        ...
        def get_producer_options():
            self.logger.debug("self.compression_type: %s", self.compression_type)
        ...
        self.logger.info(
            "Producing %d %s records to topic %s",
            count,
    ...
    def consume(self, count: int):
        ...
        self.logger.info(
            "Consuming %d %s records from topic %s with group %s",
            count,
            self.schema_type.name,
            self.topic,
            self.group,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", "--topic", dest="topic", default=str(uuid4()), help="Topic name"
    )
    parser.add_argument(
        "-g", "--consumer-group", dest="group", default=str(uuid4()), help="Topic name"
    )
    parser.add_argument(
        "-c",
        "--count",
    ...
    parser.add_argument(
        "--security", dest="security", help="JSON formatted security string"
    )
    parser.add_argument(
        "--skip-known-types",
