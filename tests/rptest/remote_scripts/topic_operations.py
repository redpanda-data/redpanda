...
class TopicSwarm:
    ...
    def generate_topic_name(prefix, length) -> str:
        ...
        if gen_size < 2:
            raise RuntimeError(
                f"Topic name length is too low ({length}) "
    ...
    def create_many_topics(
        ...
        def _create_single_topic(topic_item):
            ...
            r = kclient.create_topics(
                [newTopic], timeout_ms=self.create_topic_timeout_ms
            )
            # Save timing
        ...
        def _create_topic_batch(topic_item_list):
            ...
            r = kclient.create_topics(specs, timeout_ms=self.create_topic_timeout_ms)
        ...
        if self.skip_topic_names_randomization:
            topic_names = [f"{topic_name_prefix}-{i}" for i in range(target_count)]
        else:
            # topic name len
            ...
            self.logger.info(
                "Generating topic names using length of "
            ...
            while len(topic_names) < target_count:
                ...
                retries = 19
                while new_name in topic_names or _avoidable_char_sequences(new_name):
                    new_name = self.generate_topic_name(
                        topic_name_prefix, topic_name_length
                    )
                    retries -= 1
        ...
        # Topic creation
        self.logger.info(
            f"Creating {target_count} topics: "
        ...
        while count_created < target_count:
            ...
            next_batch_size = (
                remaining_count if remaining_count < batch_size else batch_size
            )
            ...
            if use_kafka_batch:
                # Topic creation
                self.logger.info(
                    f"Creating {target_count} topics using batch size of {batch_size}"
                )
                ...
                with ThreadPoolExecutor(max_workers=workers) as executor:
                    ...
                    for result in executor.map(_create_single_topic, topic_batch):
    ...
    def delete_many_topics(self, topic_name_prefix):
        ...
        def _delete_topic_batch(topic_item_list):
            # Send whole batch to kafka client
            r = kclient.delete_topics(
                topic_item_list, timeout_ms=self.create_topic_timeout_ms
            )
            # Filter topic errors if any
            _errors = [e for e in getattr(r, "topic_error_codes", []) if e[1] != 0]
...
def main(args):
    ...
    try:
        if args.command == COMMAND_CREATE:
            ...
            data = {
                "error": f"topic swarm command '{args.command}' not yet implemented"
            }
...
if __name__ == "__main__":
    ...
    parser_create.add_argument(
        ...
        action="store_true",
        default=True,
        help="Put whole batch in kafka client instead of per-topic operation",
    )
    parser_create.add_argument(
        "-l",
        "--topic-name-length",
