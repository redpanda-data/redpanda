...
class CloudStorageCompactionTest(EndToEndTest):
    ...
    def __init__(self, test_context, extra_rp_conf=None, environment=None):
        super(CloudStorageCompactionTest, self).__init__(test_context=test_context)
    ...
    def _init_redpanda(self, test_context, extra_rp_conf, environment):
        self.si_settings = SISettings(
            ...
            cloud_storage_segment_max_upload_interval_sec=self.configuration[
        ...
        extra_rp_conf.update(
            {
                ...
                "compacted_log_segment_size": self.configuration[
                    "compacted_log_segment_size"
                ],
                "max_compacted_log_segment_size": self.configuration[
    ...
    def _setup_read_replica(self):
        ...
        wait_until(
            self._create_read_repica_topic_success, timeout_sec=30, backoff_sec=5
        )
    ...
    def test_read_from_replica(self, cloud_storage_type):
        self.start_workload()
        self.start_consumer(
            num_nodes=2, redpanda_cluster=self.rr_cluster, verify_offsets=False
        )
        while (
            sum(self.producer.last_acked_offsets.copy().values())
            ...
            self.consumer.start()
        self.producer.stop()
        self.run_consumer_validation(enable_compaction=True, consumer_timeout_sec=600)
        ...
        assert (rr_upload_successes + rr_upload_failures) == 0, (
            f"Read replica cluster upload successes {rr_upload_successes} failures {rr_upload_failures}"
        )
