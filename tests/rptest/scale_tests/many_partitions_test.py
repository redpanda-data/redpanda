...
class ManyPartitionsTest(PreallocNodesTest):
    ...
    def _all_elections_done(self, topic_names: list[str], p_per_topic: int):
        ...
        for tn in topic_names:
            ...
            for p in partitions:
                if p.leader == -1:
                    self.logger.info(f"partition {tn}/{p.id} has no leader yet")
                    any_incomplete = True

        return not any_incomplete

    def _node_leadership_evacuated(
        self, topic_names: list[str], p_per_topic: int, node_id: int
    ):
        any_incomplete = False
        for tn in topic_names:
            try:
    ...
    def _node_leadership_balanced(self, topic_names: list[str], p_per_topic: int):
        ...
        error = stddev / ((len(topic_names) * p_per_topic) / len(self.redpanda.nodes))
    ...
    def _consume_all(
        self, topic_names: list[str], msg_count_per_topic: int, timeout_per_topic: int
    ):
        """
    ...
    def _single_node_restart(
        ...
        wait_until(
            lambda: self._node_leadership_evacuated(topic_names, n_partitions, node_id),
            30,
            5,
        )
        ...
        self.logger.info(f"Leaderships balanced in {time.time() - t1:.2f} seconds")
    ...
    def _restart_stress(
        ...
        for node_name, file_count in self._get_fd_counts():
            self.logger.info(f"Open files before restarts on {node_name}: {file_count}")
    ...
    def _tiered_storage_warmup(self, scale, topic_name):
        ...
        expect_runtime_bandwidth = (2 * warmup_total_size) / scale.expect_bandwidth
        expect_runtime_upload = (2 * target_cloud_segments) / expect_upload_rate
        ...
        try:
            ...
            self.rpk.alter_topic_config(
                topic_name, "segment.bytes", str(scale.segment_size_after_warmup)
            )
            self.rpk.alter_topic_config(
                topic_name,
    ...
    def _write_and_random_read(self, scale: ScaleParameters, topic_names):
        ...
        write_bytes_per_topic = (
            min(
                int(
                    (self.redpanda.get_node_memory_mb() * 1024 * 1024)
                    / len(topic_names)
                ),
                fetch_bytes_per_partition * n_partitions,
            )
            * 2
        )
        ...
        expect_transmit_time = int(
            write_bytes_per_topic / scale.expect_single_bandwidth
        )
        ...
        if scale.tiered_storage_enabled:
            ...
            expect_transmit_time = max(
                60,
                int(1.5 * max_msgs * stress_msg_size / scale.expect_single_bandwidth),
            )
    ...
    def _run_omb(self, scale: ScaleParameters):
        ...
        producer_bw = ((len(self.redpanda.nodes) * scale.node_cpus) / 144.0) * 1.25e9
        ...
        workload = {
            ...
            "consumer_per_subscription": producer_count
            if self.redpanda.dedicated_nodes
            else 2,
            "producers_per_topic": consumer_count
            if self.redpanda.dedicated_nodes
            else 2,
            "producer_rate": producer_rate if self.redpanda.dedicated_nodes else 1000,
    ...
    def test_omb(self):
        ...
        self.redpanda.add_extra_rp_conf(
            {
                "topic_partitions_per_shard": DEFAULT_PARTITIONS_PER_SHARD,
                "topic_memory_per_partition": DEFAULT_MIB_PER_PARTITION * 1024 * 1024,
    ...
    def _test_many_partitions(
        ...
        * Run a general produce+consume workload to check that the system remains in
        ...
        self.redpanda.add_extra_rp_conf(
            {
                "topic_partitions_per_shard": topic_partitions_per_shard,
                "topic_memory_per_partition": mib_per_partition * 1024 * 1024,
                "topic_partitions_memory_allocation_percent": DEFAULT_PARTITIONS_MEMORY_ALLOCATION_PERCENT,
            }
        )
        ...
        with repeater_traffic(
            ...
            **repeater_kwargs,
            ...
            try:
                repeater.await_progress(
                    soak_await_msgs, soak_timeout, err_msg="Waiting for traffic soak"
                )
            except TimeoutError:
                t2 = time.time()
