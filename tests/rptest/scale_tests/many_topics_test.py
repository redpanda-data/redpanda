...
class ManyTopicsTest(RedpandaTest):
    ...
    def _wait_for_leadership_balanced(self):
        ...
        wait_until(is_balanced, timeout_sec=self.HEALTHY_WAIT_SECONDS, backoff_sec=30)
    ...
    def test_decommission_safely(self):
        ...
        self.logger.warn(f"Time it took to replace node {unsafe_end - unsafe_start}")
    ...
    def _run_producers_with_constant_rate(self, profile, node_topic_count, topics):
        ...
        for topic in topics:
            swarm_producer = ProducerSwarm(
                ...
                unique_topics=True,
                messages_per_second_per_producer=profile.messages_per_second_per_producer,
            )
    ...
    def _run_consumers_with_constant_rate(
        self, profile, node_topic_count, topics, group
    ):
        swarm_node_consumers = []
        node_message_count = int(0.95 * (profile.message_count * node_topic_count))
        for topic in topics:
    ...
    def _adjust_topic_count(self, node_topic_count, nodes_available, max_topics):
        ...
        if new_node_topic_count > self.MAX_SWARM_NODE_TOPIC_COUNT:
            ...
            self.logger.warning(
                "Setting swarm topic count to MAX of "
    ...
    def _lifecycle_test_impl(self, test):
        ...
        tsm = TopicScaleProfileManager()
        profile = tsm.get_custom_profile(
            "topic_profile_t40k_p1", {"message_count": 25 * 60}
        )
    ...
    def test_many_topics_throughput(self):
        ...
        for topic_prefix in topic_prefixes:
            # messages per node
            _topic_names = [f"{topic_prefix}-{idx}" for idx in range(pnode_topic_count)]
