...
from rptest.utils import scale_parameters
...
class ManyTopicsTest(RedpandaTest):
    ...
    # Max client count supported by single client-swarm node
    # (assuming a m6id.xlarge or equiv node).
    MAX_SWARM_NODE_CLIENT_COUNT = 1_000
    ...
    PARTITIONS_MEMORY_ALLOCATION_PERCENT = int(
        2 * scale_parameters.ScaleParameters.
    ...
    def _adjust_client_count(self, node_client_count, nodes_available):
        new_node_client_count = node_client_count
        if new_node_client_count > self.MAX_SWARM_NODE_CLIENT_COUNT:
            self.logger.warning(
                f"Nodes available for swarm {nodes_available}, clients per swarm "
                f"node would be {node_client_count}, which is more than one "
                f"node can handle ({self.MAX_SWARM_NODE_CLIENT_COUNT})")
            self.logger.warning(
                f"Reducing clients per swarm node from {node_client_count} "
                f" to {self.MAX_SWARM_NODE_CLIENT_COUNT}")
            new_node_client_count = self.MAX_SWARM_NODE_CLIENT_COUNT
        else:
            self.logger.warning("Using swarm client count of "
                                f"{node_client_count} per swarm node "
                                f"({nodes_available} swarm nodes * "
                                f"{node_client_count} = "
                                f"{node_client_count * nodes_available})")

        return new_node_client_count
    ...
    def _stage_create_topics_adjusted(self, profile, producer_node_only=False):
        ...
        if produce_node_client_count > 0:
            self.logger.warning("Checking produce swarm nodes client count")
            produce_node_client_count = self._adjust_client_count(
                produce_node_client_count, produce_nodes)
        if consume_node_client_count > 0:
            self.logger.warning("Checking consume swarm nodes client count")
            consume_node_client_count = self._adjust_client_count(
                consume_node_client_count, consume_nodes)
