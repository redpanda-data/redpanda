...
class TieredStorageSinglePartitionTest(RedpandaTest):
    ...
    def throughput_test(self, restart_stress):
        ...
        self.redpanda.set_cluster_config(
            {
                "cloud_storage_spillover_manifest_size": None,
                "cloud_storage_spillover_manifest_max_segments": spillover_segments,
            }
        )
        ...
        rpk.alter_topic_config(
            self.topic, "retention.local.target.bytes", str(write_bytes // 2)
        )
        ...
        if restart_stress:
            ...
            def metadata_readable():
                return (
                    next(rpk.describe_topic(self.topic, tolerant=True)).high_watermark
                    is not None
                )
        ...
        self.logger.info(f"Produced {write_bytes} in {produce_duration}s, {mbps}MiB/s")
        ...
        partition_describe = next(rpk.describe_topic(self.topic, tolerant=True))
        hwm = partition_describe.high_watermark
        assert hwm >= msg_count
        consume_out = rpk.consume(
            topic=self.topic, n=1, offset=hwm - 1, partition=0, format="%d\\n"
        )
        ...
        uploaded_ts = max(s["max_timestamp"] for s in manifest["segments"].values())
        ...
        assert lag_seconds < (
            self.manifest_upload_interval + (self.log_segment_size / actual_byte_rate)
        )
        ...
        quiesce_uploads(
            self.redpanda,
            [self.topic],
            timeout_sec=self.manifest_upload_interval + self.segment_upload_interval,
        )
        ...
        upload_intervals_elapsed = (
            int(produce_duration) // self.manifest_upload_interval
        )
        expect_manifest_uploads = len(spill_manifests) + upload_intervals_elapsed + 3
        self.logger.info(
        ...
        assert manifest_uploads <= expect_manifest_uploads, (
            f"Too many manifest uploads: {manifest_uploads} > {expect_manifest_uploads}"
        )
