...
class ActionConfig:
    ...
    def time_between_actions(self) -> float:
        ...
        return random.uniform(
            self.min_time_between_actions_sec, self.max_time_between_actions_sec
        )
...
class DisruptiveAction:
    ...
    def target_node(self) -> Optional[ClusterNode]:
        ...
        if not self.nodes_for_action:
            self.nodes_for_action, self.recovered_nodes = (
                self.recovered_nodes,
                self.nodes_for_action,
            )
            self.node_cycle_complete = False
...
class ProcessKill(DisruptiveAction):
    ...
    def _start_rp(self, node):
        ...
        wait_until(
            ...
            err_msg=f"Failed to start redpanda process on {node.account.hostname}",
        )
...
class ActionInjectorThread(Thread):
    ...
    def _run(self):
        ...
        wait_until(
            lambda: all_nodes_started(self.redpanda.nodes),
        ...
        wait_until(
            lambda: all_nodes_started(self.redpanda.started_nodes()),
            timeout_sec=self.config.cluster_start_lead_time_sec,
            backoff_sec=0.5,
...
class ActionCtx:
    ...
    def assert_actions_triggered(self):
        ...
        """
        assert self.thread.action_triggered and self.thread.reverse_action_triggered


def create_context_with_defaults(
    redpanda: RedpandaService,
    op_type,
    ...
    **kwargs,
    ...
    return ActionCtx(
        config, redpanda, op_type(redpanda, config, admin, *args, **kwargs)
    )


def random_process_kills(
    redpanda: RedpandaService, config: ActionConfig | None = None
) -> ActionCtx:
