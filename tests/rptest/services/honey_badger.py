...
class HoneyBadger:
    @staticmethod
    def is_enabled(node):
        return requests.get(Admin._url(node, "failure-probes")).json()["enabled"]
    ...
    def set_delay(node, module, probe):
        requests.post(Admin._url(node, f"failure-probes/{module}/{probe}/delay"))

    @staticmethod
    def set_terminate(node, module, probe):
        requests.post(Admin._url(node, f"failure-probes/{module}/{probe}/terminate"))

    @staticmethod
    def set_exception(node, module, probe):
        requests.post(Admin._url(node, f"failure-probes/{module}/{probe}/exception"))
