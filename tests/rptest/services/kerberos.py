...
def render_krb5_config(kdc_node, realm: str, permitted_enctypes: str):
    return KRB5_CONF_TMPL.format(
        node=kdc_node, realm=realm, permitted_enctypes=permitted_enctypes
    )


def render_remote_kadmin_command(
    command, realm, principal, password, krb5_conf_path=KRB5_CONF_PATH
):
    return REMOTE_KADMIN_TMPL.format(
        realm=realm,
        principal=principal,
        password=password,
...
def render_ktadd_command(principal: str, keytab_file: str = DEFAULT_KEYTAB_FILE):
    return KTADD_TMPL.format(keytab_file=keytab_file, principal=principal)


def render_kinit_command(principal: str, keytab_file: str = DEFAULT_KEYTAB_FILE):
    return KINIT_TMPL.format(principal=principal, keytab_file=keytab_file)


class KrbKdc(Service):
    logs = {
        "kdc_log": {"path": "/var/log/kdc.log", "collect_default": True},
        "kadmin_log": {"path": "/var/log/kadmin.log", "collect_default": True},
    }
    """
    ...
    def _render_cfg(self, node):
        tmpl = KRB5_CONF_TMPL.format(
            node=node, realm=self.realm, permitted_enctypes=self.permitted_enctypes
        )
        ...
        tmpl = KADM5_ACL_TMPL.format(
            realm=self.realm, kadmin_principal=self.kadmin_principal
        )
    ...
    def _create_or_update_principal(self, principal: str, password: str):
        if self._principal_exists(principal):
            ...
            self.logger.debug(f"Principal {principal} does not exist. Creating")
...
"""
    ...
    def pids(self, node):
        def pid(path: str):
            try:
                it = node.account.ssh_capture(
                    f"cat {path}", allow_fail=False, callback=int
                )
                p = next(it)
    ...
    def start_node(self, node, **kwargs):
        ...
        self._create_or_update_principal(
            f"noPermissions@{self.realm}", self.kadmin_password
        )
    ...
    def change_principal_password(self, principal: str, password: str):
        self.nodes[0].account.ssh(
            self._form_kadmin_cmd(f"cpw -pw {password} {principal}"), allow_fail=False
        )
    ...
    def _configure_principal(
        ...
        self.logger.debug(f"Generating keytab file {dest} for {dest_node.name}")
        cmd = render_remote_kadmin_command(
            command=render_ktadd_command(principal=principal, keytab_file=dest),
            realm=self.realm,
            principal=self.kadmin_principal,
            password=self.kadmin_password,
            krb5_conf_path=krb5_conf_path,
...
class KrbClient(Service):
    ...
    def _form_kadmin_command(self, command, realm=None, principal=None, password=None):
        KADMIN_CMD_TMPL = (
            'kadmin -r {realm} -p {principal} -w {password} -q "{command}"'
        )
        ...
        return KADMIN_CMD_TMPL.format(
            realm=realm, principal=principal, password=password, command=command
        )
    ...
    def list_principals(self, node=None):
        ...
        if node is None:
            node = self.nodes[0]
        princs = node.account.ssh_capture(
            cmd=cmd, allow_fail=False, callback=lambda l: l.strip()
        )
        # Drop the first line, which is login details
        return list(princs)[1:]
    ...
    def add_primary(self, primary: str):
        ...
        self.kdc.configure_user_principal(
    ...
    def metadata(self, principal: str):
        ...
        try:
            res = self.nodes[0].account.ssh_output(
                cmd=f"KRB5_TRACE=/dev/stderr KRB5CCNAME={client_cache} kcat -L -J -b {self.redpanda.brokers(listener='kerberoslistener')} {sasl_conf}",
                allow_fail=False,
    ...
    def produce(
        self, principal: str, topic: str, num: int = 100, interval_s: float = 0.1
    ):
        self.logger.debug(f"Produce for {num * interval_s}s")
        producer = PRODUCER_SCRIPT.format(
            bootstrap_servers=self.redpanda.brokers(listener="kerberoslistener"),
        ...
        self.nodes[0].account.ssh(
            cmd=f'echo "{producer}" > /tmp/produce.py ', allow_fail=False
        )
        try:
            res = self.nodes[0].account.ssh_output(
    ...
    def metadata_java(self, principal: str):
        ...
        tmpl = CLIENT_PROPERTIES_TPL.format(
            keytab_file=self.keytab_file, principal=principal
        )
        ...
        try:
            ...
            brokers = self.nodes[0].account.ssh_output(
                cmd=f"/opt/kafka-3.0.0/bin/kafka-broker-api-versions.sh {cmd_args} | awk '/^[a-z]/ {{print $1}}'",
                allow_fail=False,
                combine_stderr=False,
            )
            ...
            metadata = {
                "brokers": [{"name": l} for l in sanitize(brokers)],
                "topics": [{"topic": l} for l in sanitize(topics)],
            }
...
class RedpandaKerberosNode(RedpandaService):
    ...
    def start_node(self, node, **kwargs):
        ...
        self.logger.debug(f"KRB5 config to {self.krb5_conf_path}: {krb5_config}")
        ...
        self.kdc.configure_service_principal(
            self.krb5_conf_path, principal, self.keytab_file, node
        )
...
class ActiveDirectoryKdc:
    def __init__(
        ...
        self.nodes = [
            namedtuple("node", "name account")(remote_account.hostname, remote_account)
        ]

    def ssh_input_output(
        self,
        cmd,
        allow_fail=False,
        combine_stderr=True,
        ...
        try:
            ...
            if exit_status != 0:
                if not allow_fail:
                    raise RemoteCommandError(self, cmd, exit_status, stderr.readlines())
                self.logger.debug(
    ...
    def _configure_principal(
        ...
        self.logger.debug(f"copying keytab: from {src} to {dest_node.name} at {dest}")
        dest_node.account.ssh(f"mkdir -p {os.path.dirname(dest)}")
    ...
    def configure_service_principal(
        ...
        self._configure_principal(
            krb5_conf_path,
            self.spn_user,
            principal,
            self.keytab_password,
            dest,
            dest_node,
        )
