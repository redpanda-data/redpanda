...
class EC2Client:
    ...
    def __init__(self, region, key, secret, logger, endpoint=None, disable_ssl=True):
        self._region = region
        ...
        self._log.debug(
            f"Created EC2 Client: {self._region}, {endpoint},"
            f" key is set = {self._access_key is not None}"
        )

    def _make_client(self):
        cfg = Config(
            region_name=self._region, retries={"max_attempts": 10, "mode": "adaptive"}
        )
        return boto3.client(
            "ec2",
            config=cfg,
    ...
    def get_vpc_by_network_id(self, network_id, prefix=None):
        ...
        if len(_vpcs) < 1:
            self._log.error(f"No VPC connection found for 'network-{network_id}'")
            return None
        elif len(_vpcs) > 1:
            self._log.warning(f"Found multiple VPC for 'network-{network_id}'")
        return _vpcs[0]
    ...
    def get_vpc_peering_connection(self, _id):
        ...
        if len(_vpcs) < 0:
            ...
            self._log.warning(f"Found multiple VPC Peerings for '{_id}'")
        return _vpcs[0]
    ...
    def find_vpc_peering_connection(self, state, params):
        ...
        # Validate
        if len(_ids) > 1:
            self._log.warning(f"Found multiple peering connections (cleaning?): {_ids}")
        elif len(_ids) < 1:
            self._log.error(f"No peering connections found for given info: {_filters}")
            return None
    ...
    def accept_vpc_peering(self, peering_id, dry_run=False):
        ...
        if _r[RESPONCE_META_LABEL]["HTTPStatusCode"] != 200:
            self._log.warning(
                "Unexpected return code while accepting peering"
    ...
    def get_vpc_peering_status(self, vpc_peering_id):
        ...
        """
        _filters = [{"Name": "vpc-peering-connection-id", "Values": [vpc_peering_id]}]
    ...
    def get_route_tables_for_cluster(self, network_id):
        _filters = [
            {"Name": "tag:purpose", "Values": ["private"]},
            {"Name": "tag:Name", "Values": [f"network-{network_id}"]},
        ]
    ...
    def get_route_tables_for_vpc(self, vpc_id):
        _r = self.get_route_tables(filters=[{"Name": "vpc-id", "Values": [vpc_id]}])
        # TODO: Handle errors
        return _r

    def get_route_tables(self, ids=[], filters=[]):
        return self._cli.describe_route_tables(Filters=filters, RouteTableIds=ids)

    def create_route(self, rtb_id, destCidrBlock, vpc_peering_id):
        try:
            _r = self._cli.create_route(
                RouteTableId=rtb_id,
            ...
            if "RouteAlreadyExists" in e.response["Error"]["Code"]:
                # get existing route
                self._log.warning(
                    f"Route already exists in table '{rtb_id}':"
        ...
        if _r and not _r["Return"]:
            self._log.warning(
                f"Failed to create route in '{rtb_id}' for '{destCidrBlock}'"
            )
        return _r
        ...
        if enis:
            for eni in list(enis):
                try:
                    ...
                    eni.delete()
                    self._log.debug(f"\t{vpc_id}: removed interface '{eni.id}'")
                except boto3.exceptions.Boto3Error as e:
                    self._log.error(
                        f"\t{vpc_id}: Failed to Detach/Delete "
    ...
    def delete_igw_by_vpc_id(self, vpc_id):
        ...
        if internet_gateways:
            for gateway in list(internet_gateways):
                try:
                    network_name = self._get_tag_for_resource(vpc_resource, "Name")
                    ...
                    self._log.debug(
                        f"\t{vpc_id}: Detached internet gateway '{gateway.id}'"
                    )
                    gateway.delete()
                    self._log.debug(
                        f"\t{vpc_id}: Deleted internet gateway '{gateway.id}'"
                    )
                except boto3.exceptions.Boto3Error as e:
                    self._log.error(
                        f"\t{vpc_id}: Failed to Detach/Delete "
        ...
        if subnets:
            try:
                for subnet in subnets:
                    subnet.delete()
                    self._log.debug(f"\t{vpc_id}: removed subnet '{subnet.id}'")
            except boto3.exceptions.Boto3Error as e:
                self._log.error(
                    f"\t{vpc_id}: Failed to remove subnet '{subnet.id}': {e}"
                )
        return
    ...
    def delete_route_table_by_vpc_id(self, vpc_id):
        ...
        if route_tables:
            try:
                ...
                self._log.error(
                    f"\t{vpc_id}: Failed to remove routing table '{rtable.id}': {e}"
                )
        return
    ...
    def delete_acl_by_vpc_id(self, vpc_id):
        ...
        if acls:
            try:
                ...
                self._log.error(
                    f"\t{vpc_id}: Failed to remove network ACL '{acl.id}': {e}"
                )
        return
            ...
            try:
                for sg in sgroups:
                    if sg.group_name == "default":
                        ...
                        continue
                    sg.delete()
                    self._log.debug(f"\t{vpc_id}: removed security group '{sg.id}'")
            except boto3.exceptions.Boto3Error as e:
                self._log.error(
                    f"\t{vpc_id}: Failed to remove security group '{sg.id}': {e}"
                )
        return
