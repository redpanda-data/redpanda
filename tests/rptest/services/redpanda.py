    ...
    def wait_until(
        ...
        def wrapped():
            ...
            if not r and time.time() > t_initial + grace_period:
                ...
                assert self.all_up() or getattr(self, "_tolerate_crashes", False)
            return r
...
class RedpandaServiceBase(RedpandaServiceABC, Service):
    ...
    def metric_sum(
        ...
        return self._metric_sum(metric_name, nodes, metrics_endpoint, namespace, topic)
...
class RedpandaServiceCloud(KubeServiceMixin, RedpandaServiceABC):
    ...
    def raise_on_crash(self, log_allow_list: list[str | re.Pattern] | None = None):
...
class RedpandaService(RedpandaServiceBase):
    ...
    def __init__(
        ...
        *,
        ...
        if (
            hasattr(self._context, "injected_args")
            and self._context.injected_args is not None
        ):
            if (
                "use_serde_parquet" in self._context.injected_args
                and self._context.injected_args["use_serde_parquet"]
            ):
                self._environment.update({"__REDPANDA_USE_SERDE_PARQUET_WRITER": "1"})
    ...
    def start_si(self):
        if self.si_settings.cloud_storage_type == CloudStorageType.S3:
            ...
            self.logger.warning(
                "Turning off use_bucket_cleanup_policy "
    ...
    def node_storage(
        self, node, sizes: bool = False, scan_cache: bool = True
    ) -> NodeStorage:
        """
