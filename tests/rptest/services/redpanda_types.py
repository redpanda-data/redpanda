...
class KafkaClientSecurity:
    ...
    def username(self) -> str | None:
        ...
        return (
            self._sasl_credentials.username
            if isinstance(self._sasl_credentials, SaslCredentials)
            else None
        )
    ...
    def password(self) -> str | None:
        ...
        return (
            self._sasl_credentials.password
            if isinstance(self._sasl_credentials, SaslCredentials)
            else None
        )
    ...
    def mechanism(self) -> str | None:
        ...
        return (
            None
            if isinstance(self._sasl_credentials, NonSASL)
            else self._sasl_credentials.mechanism
        )
    ...
    def override(
        ...
        if sasl_mechanism and not username:
            ...
            assert sasl_mechanism in COMPLEX_SASL_MECHANISMS, (
                f"unknown SASL mechanism: {sasl_mechanism}"
            )
        ...
        if username and password:
            ...
            ret._sasl_credentials = SaslCredentials(username, password, sasl_mechanism)
    ...
    def to_dict(self) -> dict[str, str | int]:
        ...
        if c := self.simple_credentials():
            return dict(
                security_protocol=self.security_protocol.name,
            ...
            return (
                dict(security_protocol=self.security_protocol.name)
                if self.tls_enabled
                else {}
            )
