...
class ConsumerEventHandler(object):
    ...
    def handle_records_consumed(self, event, logger):
        ...
        for record_batch in event["partitions"]:
            tp = TopicPartition(
                topic=record_batch["topic"], partition=record_batch["partition"]
            )
...
class VerifiableConsumer(BackgroundThreadService):
    ...
    logs = {
        ...
        "verifiable_consumer_log": {"path": LOG_FILE, "collect_default": True},
    }
    ...
    class WorkerState:
        ...
        def record_position(
            ...
            if tp in self.committed and self.committed[tp] > min_offset:
                msg = (
                    "%s: Consumed position %d is behind the current "
            ...
            if tp in self.position_first:
                ...
                self.position_first[tp] = min(self.position_first[tp], min_offset)
            else:
                self.position_first[tp] = min_offset

        def record_committed(self, tp: TopicPartition, offset, verify_position: bool):
            if verify_position:
                assert self.position[tp] >= offset, (
    ...
    def _worker(self, idx, node):
        ...
        node.account.ssh(
            "mkdir -p %s" % VerifiableConsumer.PERSISTENT_ROOT, allow_fail=False
        )
    ...
    def _update_global_position(self, consumed_event, state: WorkerState):
        for consumed_partition in consumed_event["partitions"]:
            tp = TopicPartition(
                consumed_partition["topic"], consumed_partition["partition"]
            )
            state.record_position(
                tp,
                consumed_partition["minOffset"],
                consumed_partition["maxOffset"],
                self.logger,
                self.verify_offsets,
            )
        ...
        if commit_event["success"]:
            for offset_commit in commit_event["offsets"]:
                ...
                state.record_committed(
                    tp, offset_commit["offset"], verify_position=True
                )

    def _update_global_committed_fetched(self, fetch_offsets_ev, state: WorkerState):
    ...
    def start_cmd(self, node):
        ...
        cmd += (
            " --reset-policy %s --group-id %s --topic %s --broker-list %s --session-timeout %s"
            % (
                self.reset_policy,
                self.group_id,
                self.topic,
    ...
    def try_parse_json(self, node, string):
        ...
        try:
            ...
            self.logger.debug(
                "%s: Could not parse as json: %s" % (str(node.account), str(string))
            )
            return None
    ...
    def current_position(self, tp):
        with self.lock:
            current_positions = (
                s.current_position(tp) for s in self.global_state.values()
            )
            return max((p for p in current_positions if p is not None), default=None)
    ...
    def last_commit(self, tp):
        with self.lock:
            last_commits = (s.last_commit(tp) for s in self.global_state.values())
            return max((c for c in last_commits if c is not None), default=None)

    def total_consumed(self):
        with self.lock:
            return sum(
                handler.total_consumed for handler in self.event_handlers.values()
            )

    def num_rebalances(self):
        with self.lock:
            return max(
                handler.assigned_count for handler in self.event_handlers.values()
            )

    def num_revokes_for_alive(self, keep_alive=1):
        with self.lock:
            return max(
                handler.revoked_count
                for handler in self.event_handlers.values()
    ...
    def get_committed_offsets(self):
        with self.lock:
            return dict(
                (i, worker.committed) for i, worker in self.global_state.items()
            )
    ...
    def verify_position_offsets_consistency(self):
        msg = []
        with self.lock:
            tps = set().union(s.position.keys for s in self.global_state.values())
            for tp in tps:
                ...
                ranges = [
                    (s.position_first[tp], s.position[tp])
