...
class CloudStorageInventoryTest(PreallocNodesTest):
    ...
    def test_load_inventory_report(self):
        ...
        wait_until(
            lambda: self._hashes_are_found_on_leaders(), timeout_sec=60, backoff_sec=5
        )
    ...
    def _find_ntp_hashes_on_node(self, node):
        ...
        for row in node.account.ssh_capture(
            ...
            assert path.name.isdigit(), f"unexpected non-integer file name {path.name=}"

            found.add(NTP(ns=ns.name, topic=topic.name, partition=int(partition.name)))
        return found
