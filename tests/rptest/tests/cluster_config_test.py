...
class ClusterInvalidConfigPreloadTest(RedpandaTest):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def test_invalid_config(self):
        invalid_config_cache_yaml = """# Auto-generated, do not edit this file unless Redpanda will not start
_version: 1
tombstone_retention_ms: 1200
cloud_storage_enabled: true
        """
        ...
        for node in self.redpanda.nodes:
            ...
            cache_path = f"{self.redpanda.DATA_DIR}/config_cache.yaml"
            assert node.account.exists(cache_path)
            node.account.create_file(cache_path, invalid_config_cache_yaml)
            self.redpanda.start_node(node)

        wait_until(
            lambda: self.redpanda.search_log_all(
                "Misconfigured preload property cloud_storage_enabled") and
            self.redpanda.search_log_all(
                "Misconfigured preload property tombstone_retention_ms"),
            timeout_sec=30,
            backoff_sec=1,
            err_msg="Did not see \"misconfigured preload property \" message")

        # These properties should be reset to their defaults,
        # instead of the being set to the ones provided in invalid_config_cache_yaml.
        cluster_cfg = self.admin.get_cluster_config()
        tombstone_retention_ms = cluster_cfg["tombstone_retention_ms"]
        assert tombstone_retention_ms == None, f"Expected {None} for tombstone_retention_ms, got {tombstone_retention_ms}"

        cloud_storage_enabled = cluster_cfg["cloud_storage_enabled"]
        assert cloud_storage_enabled == False, f"Expected {False} for cloud_storage_enabled, got {cloud_storage_enabled}"
