...
class SchemaRegistrySyncMixin:
    ...
    def _augment_link_request(
        self, req: "shadow_link_pb2.CreateShadowLinkRequest"
    ) -> None:
        # Hook for auth/TLS leaves to adjust the whole link request, e.g. add the
        # Kafka-level source credentials the link needs when the source cluster
        # runs SASL. No-op for the plaintext no-auth suite.
        pass
...
class SchemaRegistrySyncAuthMixin(SchemaRegistrySyncMixin):
    ...
    def _augment_link_request(
        ...
        # stay valid.
        client_options = req.shadow_link.configurations.client_options
