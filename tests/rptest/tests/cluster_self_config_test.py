...
class ClusterSelfConfigTest(EndToEndTest):
    ...
    def str_in_logs(self, node, s):
        return any(s in log.strip() for log in self.log_searcher._capture_log(node, s))

    def self_config_start_in_logs(self, node):
        client_self_configuration_start_string = (
            "Client requires self configuration step"
        )
    ...
    def self_config_result_from_logs(self, node):
        client_self_configuration_complete_string = (
        ...
        for log in self.log_searcher._capture_log(
            ...
            m = re.search(
                client_self_configuration_complete_string + r" (\{.*\})", log.strip()
            )
            if m:
                return m.group(1)
        return None
    ...
    def test_s3_self_config(self, cloud_storage_type):
        ...
        self.log_searcher = LogSearchLocal(
            self.ctx, [], self.redpanda.logger, self.redpanda.STDOUT_STDERR_CAPTURE
        )
        ...
        for node in self.redpanda.nodes:
            ...
            assert (
                self_config_result
                and self_config_result in self_config_expected_results
            )
    ...
    def test_s3_oracle_self_config(self, cloud_storage_type):
        ...
        self.start_redpanda(extra_rp_conf=extra_rp_conf, si_settings=si_settings)
        admin = Admin(self.redpanda)
        self.log_searcher = LogSearchLocal(
            self.ctx, [], self.redpanda.logger, self.redpanda.STDOUT_STDERR_CAPTURE
        )
