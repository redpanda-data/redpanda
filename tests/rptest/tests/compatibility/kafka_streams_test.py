...
# Validate the last N fetched messages
def validate_last_N(ctx, consumer, is_valid_msg, num_msgs):
    def try_cons():
...
class KafkaStreamsAnomalyDetection(RedpandaTest):
    """
    Test KafkaStreams SessionWindow example that counts the user clicks (i.e., usernames)
    within a 1min window and publishes that count to the second topic,
    AnomalousUsers. So, on the consumer side, there is one record per unique user name.
    """
    topics = (
    ...
    def __init__(self, test_context):
        super(KafkaStreamsAnomalyDetection,
              self).__init__(test_context=test_context)
        self._ctx = test_context
        self._num_produces = 10 if self.scale.local else 100

    def is_valid_msg(self, msg):
        username = msg["key"]
        count = int(msg["value"])

        if username != "user-1" and username != "user-2" and username != "user-3":
            return False

        if count != self._num_produces or count != (self._num_produces * 2):
            return False

        return True

    @ok_to_fail  # https://github.com/redpanda-data/redpanda/issues/3032
    @cluster(num_nodes=6)
    def test_kafka_streams_anomaly_detection(self):
        ...
        validate_last_N(self._ctx,
                        consumer,
                        self.is_valid_msg,
                        num_msgs=AnomalyProducer.NUM_UNIQUE_USERS)
...
class KafkaStreamsMapFunction(RedpandaTest):
    ...
    def test_kafka_streams_map_function(self):
        ...
        validate_last_N(self._ctx,
                        consumer,
                        self.is_valid_msg,
                        num_msgs=(len(self._producer.choices) *
                                  self._num_produces))
