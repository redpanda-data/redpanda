...
class TopicPartitionOffset:
    ...
    def prev(self):
        return TopicPartitionOffset(self.topic, self.partition, self.offset - 1)
...
class ConsumerOffsetsRecoveryTest(PreallocNodesTest):
        ...
        super(ConsumerOffsetsRecoveryTest, self).__init__(
            ...
            *args,
            extra_rp_conf={
                ...
                "cloud_storage_cluster_metadata_upload_interval_ms": ConsumerOffsetsRecoveryTest.UPLOAD_INTERVAL_SEC
                * 1000,
            },
            ...
            for p in gd.partitions:
                all_groups[gd.name].add(
                    TopicPartitionOffset(
                        topic=p.topic, partition=p.partition, offset=p.current_offset
                    )
                )
    ...
    def test_consumer_offsets_partition_recovery(
        ...
        wait_until(self.bucket_has_consumer_offsets, timeout_sec=60, backoff_sec=5)
        time.sleep(10)
        ...
        assert groups_post_restore == groups_pre_restore, (
            f"{groups_post_restore} vs {groups_pre_restore}"
        )

        self.run_workload_after_restore(groups_pre_restore, partition_count, topic)
    ...
    def run_workload_after_restore(self, groups, partition_count, topic):
        ...
        for t, part, _ in tp_offsets:
            ...
            produce_result = rpk.produce(
                t, key=f"{part}", msg=f"{part}", partition=part
            )
        ...
        for line in (
            rpk.consume(topic.name, n=partition_count, group=group, format="%t,%p,%o\n")
            .strip()
            .splitlines()
        ):
            tokens = line.split(",")
            ...
            assert (
                TopicPartitionOffset(
                    topic=tokens[0], partition=int(tokens[1]), offset=int(tokens[2])
                )
                in tp_offsets
            ), (
                f"Topic, partition and offset {line} missing from committed offsets: {tp_offsets}"
            )
        ...
        for latest_tp_offset in groups_desc[group]:
            ...
            assert latest_tp_offset.prev() in tp_offsets, (
                f"partition offset did not increment correctly after consume: {latest_tp_offset}"
            )
