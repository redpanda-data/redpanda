...
class ControllerSnapshotPolicyTest(RedpandaTest):
    ...
    def test_snapshotting_policy(self):
        ...
        self.redpanda.start()
        self.redpanda.set_feature_active("controller_snapshots", False, timeout_sec=10)
...
class ControllerState:
    def __init__(self, redpanda, node):
        ...
        self.config_response = admin.get_cluster_config(
            node=node, include_defaults=False
        )
        ...
        versions = list(
            set(broker["version"] for broker in brokers_resp if "version" in broker)
        )
    ...
    def _check_features(self, other, allow_upgrade_changes=False):
        for k, v in self.features_response.items():
            ...
            if allow_upgrade_changes and k in (
                "cluster_version",
                "node_latest_version",
    ...
    def _check_config(self, other):
        ...
        assert len(symdiff) == 0, f"config responses differ, symdiff: {symdiff}"
    ...
    def _check_topics(self, other, allow_upgrade_changes=False):
        ...
        def to_set(configs):
            if allow_upgrade_changes:
                return set(
                    (k, v) for k, v in configs.items() if v[1] != "DEFAULT_CONFIG"
                )
            else:
                return set(configs.items())

        for t in self.topics:
            symdiff = to_set(self.topic_configs[t]) ^ to_set(other.topic_configs[t])
            ...
            if self.version < MIN_VERSION_WITH_ALL_LATER_VERSIONS_FIXED:
                symdiff -= set({("cleanup.policy", ("delete", "DYNAMIC_TOPIC_CONFIG"))})
...
class ControllerSnapshotTest(RedpandaTest):
    ...
    def test_join_restart_catch_up(self):
        ...
        self.redpanda.set_cluster_config({"controller_snapshot_max_age_sec": 10})
        ...
        assert initial_state.features_map["controller_snapshots"]["state"] == "active"
        assert initial_state.config_response["controller_snapshot_max_age_sec"] == 10
        ...
        wait_until(
            lambda: self.redpanda.registered(joiner), timeout_sec=30, backoff_sec=1
        )
        ...
        def check(node, expected):
            ...
            admin.transfer_leadership_to(
                namespace="redpanda", topic="controller", partition=0, target_id=node_id
            )
            admin.await_stable_leader(
                namespace="redpanda", topic="controller", check=lambda id: id == node_id
            )
        ...
        for iter in range(5):
            ...
            controller_max_offset = self._wait_for_everything_snapshotted(seed_nodes)
            ...
            wait_until(
                lambda: admin.get_controller_status(joiner)["last_applied_offset"]
                >= controller_max_offset,
                timeout_sec=30,
                backoff_sec=1,
            )
            ...
            self.logger.info(f"iteration {iter} done (executed {to_execute} ops).")
    ...
    def test_upgrade_compat(self):
        ...
        self.logger.info(f"will test compatibility with redpanda v. {initial_ver}")
        ...
        # could be any non-default value for any property
        self.redpanda.set_cluster_config({"controller_snapshot_max_age_sec": 7})
