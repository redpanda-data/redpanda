...
class DataMigrationsApiTest(RedpandaTest):
    ...
    def test_list_unmounted_topics(self):
        ...
        unmounted_names = [t["topic"] for t in unmounted_response["topics"]]
        assert set(initial_names) == set(unmounted_names), \
            f"Initial topics: {initial_names} should match unmounted topics: {unmounted_names}"

        for t in unmounted_response["topics"]:
        ...
        inbound_topics = [
            ...
            for t in unmounted_response["topics"][:2]
        ] + [
            InboundTopic(
                NamespacedTopic(
                    topic=unmounted_response["topics"][2]["topic_location"],
                    namespace=unmounted_response["topics"][2]["ns"]))
        ]
