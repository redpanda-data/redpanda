...
class TransferLeadersBackgroundThread:
    ...
    def _loop(self):
        while not self.stop_ev.is_set():
            p_id = None
            try:
                partitions = self.admin.get_partitions(
                    namespace="kafka", topic=self.topic
                )
                ...
                self.admin.partition_transfer_leadership(
                    namespace="kafka", topic=self.topic, partition=p_id
                )
...
class DataMigrationsApiTest(RedpandaTest):
    ...
    def wait_for_migration_states(
        ...
        def migration_in_one_of_states_on_node(m):
            ...
            self.logger.debug(f"{assure_completed_after=}, {completed_at=}, {now()=}")
    ...
    def wait_partitions_disappear(self, topics: list[TopicSpec]):
        ...
        wait_until(
            lambda: all(
                self.client().describe_topic(t.name).partitions == [] for t in topics
            ),
            timeout_sec=90,
            backoff_sec=1,
        ...
        with self.tl_thread(tl_topic_name):
            ...
            while not remounted:
                in_migration = InboundDataMigration(
                    topics=[
                        InboundTopic(
                            source_topic_reference=workload_ns_topic, alias=alias
                        )
                    ],
                    consumer_groups=[],
                )
