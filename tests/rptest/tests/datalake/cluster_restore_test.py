...
class DatalakeClusterRestoreTest(RedpandaTest):
    ...
    def test_slow_tiered_storage_dlq(self, cloud_storage_type, catalog_type) -> None:
        ...
        with DatalakeServices(
            ...
            structured_offsets_snap = self.offsets_per_table(dl, structured_tables)

            unstructured_tables, _ = self.start_unstructured_topic_streams(
                dl, create_topics=False
            )
            unstructured_offsets_snap = self.offsets_per_table(dl, unstructured_tables)

            # Key-value topics should continue to write with no issues since
            # they don't depend on restored schemas.
            wait_until(
                lambda: self.tables_translated_more(dl, unstructured_offsets_snap, 50),
                timeout_sec=60,
                backoff_sec=1,
                err_msg="Translation not progressing after restore",
            )
            ...
            for t in structured_tables:
                ...
                assert len(offset_zero_res) == 1, (
                    f"Expected exactly one record with offset zero, got: {offset_zero_res}"
                )
            ...
            wait_until(
                lambda: self.tables_translated_more(dl, structured_offsets_snap, 50),
                timeout_sec=60,
                backoff_sec=1,
                err_msg="Translation not progressing after restore",
            )
    ...
    def test_slow_tiered_storage_dupe_records(
        ...
        with DatalakeServices(
            ...
            for t in all_topics:
                ...
                assert [(0, 2)] == zero_offset_count_res, (
                    f"Expected offset 0 to be duplicated: {zero_offset_count_res}"
                )
            assert dl.num_tables() == len(all_topics), (
                "Expected one table per topic (i.e. no DLQ tables)"
            )
