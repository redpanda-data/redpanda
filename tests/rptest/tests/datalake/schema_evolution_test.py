...
class SchemaEvolutionTest(RedpandaTest):
    ...
    def test_evolving_avro_schemas(self, cloud_storage_type, query_engine):
        ...
        with DatalakeServices(self.test_context,
            ...
            self._create_schema("schema_avro_2", legal_evo_schema_avro)
            self._create_schema("schema_avro_3", illegal_evo_schema_avro)

            connect = RedpandaConnectService(self.test_context, self.redpanda)
            connect.start()

            # create verifier
            verifier = DatalakeVerifier(self.redpanda, topic_name, get_qe())
            # create a stream
            connect.start_stream(name="ducky_stream",
                                 config=self.avro_stream_config(
                                     topic_name, "schema_avro"))

            verifier.start()
            connect.stop_stream("ducky_stream", wait_to_finish=True)
            verifier.wait()

            # do it again with a different schema
            verifier = DatalakeVerifier(self.redpanda, topic_name, get_qe())
            connect.start_stream(name="ducky_stream",
                                 config=self.avro_stream_config(
                                     topic_name,
            ...
            # one more time w/ a non-compatible schema, verifier should time out and fail its assertion
            verifier = DatalakeVerifier(self.redpanda, topic_name, get_qe())
            connect.start_stream(name="ducky_stream",
                                 config=self.avro_stream_config(
                                     topic_name,
