...
class DatalakeTransactionTests(RedpandaTest):
    ...
    def append_last_committed_transaction(self):
        produced_offset = -1

        def delivery_callback(err, msg):
            if err:
                assert False, "failed to deliver message: %s" % err
            nonlocal produced_offset
            produced_offset = msg.offset()

        producer = ck.Producer({
            'bootstrap.servers': self.redpanda.brokers(),
            'transactional.id': 'last_committed_tx',
        })
        ...
        assert produced_offset > 0, f"producer not flushed, offset: {produced_offset}"
        return produced_offset
    ...
    def test_with_transactions(self, cloud_storage_type, compaction):
        ...
        with DatalakeServices(self.test_context,
            ...
            # Issue a last committed transaction so we do not end with an aborted
            # transaction which makes the verification difficult (as we need to keep
            # track of last committed transaction offset)
            offset = self.append_last_committed_transaction()
            tx_verifier.stop()
            ...
            committed_offset_verifier = DatalakeVerifier(
                ...
                self.topic_name,
                dl.trino(),
                max_offset_check_override=offset)
