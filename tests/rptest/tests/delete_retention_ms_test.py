...
class DeleteRetentionMsTest(RedpandaTest):
    ...
    def test_create_topic_configurations_cluster_defaults(self):
        topic_name = "tapioca"
        TopicAndClusterConf = namedtuple(
            "TopicAndClusterConf",
            [
                "delete_retention_ms",
        ...
        for test_case in test_cases:
            self.rpk.cluster_config_set(
                "cloud_storage_enable_remote_read", test_case.cloud_storage_remote_read
            )
            self.rpk.cluster_config_set(
            ...
            assert (
                self.rpk.cluster_config_get("cloud_storage_enable_remote_read")
                == test_case.cloud_storage_remote_read
            )

            assert (
                self.rpk.cluster_config_get("cloud_storage_enable_remote_write")
                == test_case.cloud_storage_remote_write
            )
            ...
            if test_case.delete_retention_ms is not None:
                ...
                expected_delete_retention_ms = str(test_case.delete_retention_ms)
            else:
            ...
            if test_case.valid:
                ...
                topic_desc = self.rpk.describe_topic_configs(topic_name)
                assert (
                    topic_desc["delete.retention.ms"][0] == expected_delete_retention_ms
                )
                ...
                with expect_exception(
                    RpkException,
                    lambda e: "Unsupported delete.retention.ms configuration, cannot be enabled at the same time as redpanda.remote.read or redpanda.remote.write."
                    in str(e),
                ):
                    self.rpk.create_topic(topic_name, partitions=1, config=config)
    ...
    def test_create_topic_configurations(self):
        ...
        TopicConf = namedtuple(
            "TopicConf", ["delete_retention_ms", "remote_read", "remote_write", "valid"]
        )
        # We cannot enable delete.retention.ms if any cloud storage properties are also enabled.
        test_cases = [
            TopicConf(
