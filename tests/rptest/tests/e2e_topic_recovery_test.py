...
class EndToEndTopicRecovery(RedpandaTest):
    ...
    def init_consumer(self, msg_size):
        self._consumer = KgoVerifierSeqConsumer(
            self._ctx,
            self.redpanda,
            self.topic,
            msg_size,
    ...
    def test_restore_with_config_batches(self, num_messages, cloud_storage_type):
        ...
        for topic_spec in self.topics:
            self._restore_topic(topic_spec, {"retention.local.target.bytes": 512})
    ...
    @matrix(
        message_size=[5000],
        num_messages=[100000],
        recovery_overrides=[{}, {"retention.local.target.bytes": 1024}],
        cloud_storage_type=get_cloud_storage_type(),
    )
    def test_restore(
        self, message_size, num_messages, recovery_overrides, cloud_storage_type
    ):
        ...
        assert consume_valid >= produce_acked, (
            f"produced {produce_acked}, consumed {consume_valid}"
        )
    ...
    def test_restore_with_aborted_tx(self, recovery_overrides, cloud_storage_type):
        ...
        rpk.alter_topic_config(self.topic, "retention.local.target.bytes", "1024")
        wait_for_local_storage_truncate(
            self.redpanda,
            self.topic,
            target_bytes=int(msg_size * msg_count * 0.5),
            timeout_sec=30,
        )
        validate()
