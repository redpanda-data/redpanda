...
class IdempotencyTest(RedpandaTest):
    def __init__(self, test_context):
        ...
        super(IdempotencyTest, self).__init__(
...
class IdempotencyWriteCachingTest(PreallocNodesTest):
    ...
    def test_idempotent_producers_write_caching(self):
        ...
        rpk = RpkTool(self.redpanda)
        rpk.alter_topic_config(topic.name, TopicSpec.PROPERTY_WRITE_CACHING, "true")
        # configure topic with write caching to delay state machine apply
        rpk.alter_topic_config(topic.name, TopicSpec.PROPERTY_FLUSH_BYTES, 1024 * 1024)
        rpk.alter_topic_config(topic.name, TopicSpec.PROPERTY_FLUSH_MS, 1024 * 1024)
