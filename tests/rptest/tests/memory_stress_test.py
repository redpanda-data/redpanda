...
class MemoryStressTest(RedpandaTest):
    ...
    @cluster(num_nodes=5)
    @skip_debug_mode
    @parametrize(max_parallelism=1)
    @parametrize(max_parallelism=10)
    @parametrize(max_parallelism=100)
    def test_fetch_with_many_partitions(self, max_parallelism):
        """
        Without max fetch parallelism (or when set too high), memory would have
        been exhausted in this test by consuming from too many partitions in a
        single Fetch API request. Check that with lower values of max fetch
        parallelism that does not happen.
        """
