...
class NodeFolderDeletionTest(PreallocNodesTest):
    ...
    def test_deleting_node_folder(self):
        ...
        wait_until(
            lambda: producer.produce_status.acked > 100, timeout_sec=60, backoff_sec=0.5
        )
        ...
        wait_until(
            lambda: producer.produce_status.acked > 100000,
            timeout_sec=120,
            backoff_sec=0.5,
        )
        ...
        wait_until(
            lambda: producer.produce_status.acked > 200000,
            timeout_sec=120,
            backoff_sec=0.5,
        )
        ...
        wait_until(
            ...
            timeout_sec=20,
            backoff_sec=0.5,
            err_msg=f"Node {id} is expected to be marked as offline as it was replaced by new node",
        )

        # decommission a node that has been cleared
        admin.decommission_broker(id)
        waiter = NodeDecommissionWaiter(
            self.redpanda, id, self.logger, progress_timeout=60
        )

        waiter.wait_for_removal()

        wait_until(
            lambda: producer.produce_status.acked > 300000,
            timeout_sec=120,
            backoff_sec=0.5,
        )
