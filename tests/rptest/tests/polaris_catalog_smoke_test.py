...
class PolarisCatalogSmokeTest(RedpandaTest):
    def __init__(self, test_ctx, *args, **kwargs):
        super(PolarisCatalogSmokeTest, self).__init__(
            test_ctx,
            num_brokers=3,
            extra_rp_conf={},
    ...
    def _start_redpanda(self, catalog_prefix, client_id, client_secret, with_tls):
        self.redpanda._extra_rp_conf.update(
            {
                "iceberg_enabled": "true",
                "iceberg_catalog_type": "rest",
                "iceberg_rest_catalog_endpoint": self.polaris.catalog_url,
                "iceberg_rest_catalog_authentication_mode": "oauth2",
                "iceberg_rest_catalog_client_id": client_id,
                "iceberg_rest_catalog_client_secret": client_secret,
                "iceberg_catalog_commit_interval_ms": 10000,
                "iceberg_rest_catalog_warehouse": catalog_prefix,
            }
        )
        if with_tls == "from_file":
            self.redpanda._extra_rp_conf.update(
                {
                    "iceberg_rest_catalog_trust_file": RedpandaService.TLS_CA_CRT_FILE,
                }
            )
            for n in self.redpanda.nodes:
                n.account.mkdirs(os.path.dirname(RedpandaService.TLS_CA_CRT_FILE))
                n.account.copy_to(
                    self.cert_manager.ca.crt, RedpandaService.TLS_CA_CRT_FILE
                )
        elif with_tls == "direct":
            crt = open(self.cert_manager.ca.crt).read()
            self.redpanda._extra_rp_conf.update(
                {
                    "iceberg_rest_catalog_trust": crt,
                }
            )
    ...
    def _initialize_catalog(self, catalog_name, principal_name):
        ...
        catalog = Catalog(
            ...
            properties=CatalogProperties(
                default_base_location=f"file://{PolarisCatalog.PERSISTENT_ROOT}/catalog_data",
    ...
    def get_client(self, catalog_name, client_id, client_secret):
        self.credentials = cloud_storage.Credentials.from_context(self.test_context)
        conf = dict()
    ...
    def test_connecting_to_catalog(self, cloud_storage_type, with_tls):
        ...
        catalog_properties = {
            ...
            "credential": f"{credentials.client_id}:{credentials.client_secret.get_secret_value()}",
            "warehouse": catalog_name,
        }

        if with_tls != "none":
            catalog_properties.update({"ssl": {"cabundle": self.cert_manager.ca.crt}})
        ...
        client.alter_topic_config(
            topic.name, TopicSpec.PROPERTY_ICEBERG_MODE, "key_value"
        )
