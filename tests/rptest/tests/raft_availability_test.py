...
class RaftAvailabilityTest(RedpandaTest):
    ...
    def _wait_for_leader(self, condition=None):
        t1 = time.time()
        result = {}
    ...
    def _ping_pong(self):
        ...
        payload = str(self._ping_offset)

        rpk.produce(self.topic, "tkey", payload, timeout=5)
        consumed = kc.consume_one(self.topic, 0, self._ping_offset)
