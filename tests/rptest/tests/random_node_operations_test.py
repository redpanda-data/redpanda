...
class RandomNodeOperationsTest(PreallocNodesTest):
    ...
    def _start_redpanda(self, mixed_versions, with_tiered_storage, with_iceberg):
    ...
    def test_node_operations(
        ...
        def enable_fast_partition_movement():
            ...
            supported_by_prev = (initial_version[0] > 23) or (
                initial_version[0] == 23 and initial_version[1] > 2
            )
        ...
        if enable_failures:
                ...
                self.redpanda, self.logger, max_suspend_duration_seconds=4, lock=lock
            )
            fi.start()
