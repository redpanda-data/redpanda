...
class RBACTest(RedpandaTest):
    ...
    def test_superuser_access(self):
        ...
        with expect_http_error(501):
            self.superuser_admin.create_role(role='foo')

        with expect_http_error(501):
            self.superuser_admin.get_role(role='foo')
        ...
        with expect_http_error(501):
            self.superuser_admin.update_role_members(role='bar',
                                                     add=[ALICE.username])

        with expect_http_error(501):
            self.superuser_admin.list_role_members(role='bar')

        with expect_http_error(501):
            self.superuser_admin.list_user_roles()

        with expect_http_error(501):
            self.superuser_admin.delete_role(role='bar')
