...
class ReadDistributionTest(RedpandaTest):
    ...
    def test_read_distribution_metric(self):
        ...
        for timestamp in timestamps_ms:
            kcat.consume_one(self.topic_name, 0, offset=None, first_timestamp=timestamp)

        metrics = self.redpanda.metrics(self.redpanda.nodes[0], MetricsEndpoint.METRICS)
        ...
        cumulative_topic_read_dist = [
            (s.value, s.labels["le"])
            for s in read_dist_metric.samples
        ...
        expected_bucket_list = [
            (first_bucket_lower_bound * 2**i) - 1
        ...
        expected_bucket_order = [2047, 2047, 2047, 1023, 1023, 1023, 511, 511, 255, 3]
        assert len(expected_bucket_order) == len(timestamps_ms)
        expected_read_dist = {bucket: 0 for bucket in topic_read_dist.keys()}
        for expected_bucket, timestamp_ms in zip(expected_bucket_order, timestamps_ms):
            ...
            bucket = bucket_list[bisect.bisect_left(bucket_list, delta_minutes)]
