...
class RedpandaKerberosTest(RedpandaKerberosTestBase):
    ...
    @parametrize(
        req_principal="client", acl=False, topics=["always_visible"], fail=False
    )
    @parametrize(req_principal="invalid", acl=False, topics={}, fail=True)
    def test_init(self, req_principal: str, acl: bool, topics: set[str], fail: bool):
        ...
        for metadata_fn in [self.client.metadata, self.client.metadata_java]:
            try:
                wait_until(
                    ...
                    timeout_sec=5,
                    backoff_sec=0.5,
                    err_msg=f"Did not receive expected set of topics with {metadata_fn.__name__}",
                )
                assert not fail
    ...
    def _have_expected_topics(
        self, metadata_fn, req_principal, expected_broker_count, topics_set
    ):
        metadata = metadata_fn(req_principal)
        self.redpanda.logger.info(f"{metadata_fn.__name__} (GSSAPI): {metadata}")
...
class RedpandaKerberosLicenseTest(RedpandaKerberosTestBase):
    ...
    def __init__(self, test_context, num_nodes=3, **kwargs):
        super(RedpandaKerberosLicenseTest, self).__init__(
            test_context, num_nodes=num_nodes, sasl_mechanisms=["SCRAM"], **kwargs
        )
    ...
    def test_license_nag(self):
        ...
        self.redpanda.set_cluster_config({"sasl_mechanisms": ["GSSAPI", "SCRAM"]})
...
class RedpandaKerberosRulesTesting(RedpandaKerberosTestBase):
    def __init__(self, test_context, **kwargs):
        super(RedpandaKerberosRulesTesting, self).__init__(
            test_context, num_nodes=3, **kwargs
        )
    ...
    @parametrize(
        rules=[
            ...
            "RULE:[1:$1testgood](client.*)s/client(.*)/$1redpanda/U",
            "DEFAULT",
        ],
        kerberos_principal="client",
...
class RedpandaKerberosConfigTest(RedpandaKerberosTestBase):
    ...
    def __init__(self, test_context, **kwargs):
        super(RedpandaKerberosConfigTest, self).__init__(
            test_context,
            num_nodes=3,
    ...
    def test_non_default(self):
        ...
        def keytab_not_found():
            return self.redpanda.search_log_any(f"Key table file '{keytab}' not found")
        ...
        try:
            ...
            assert (
                b"ccselect can't find appropriate cache for server principal redpanda/"
                in err.msg
            )

        wait_until(keytab_not_found, 5)

        self.redpanda.set_cluster_config({"sasl_kerberos_keytab": self.KEYTAB_FILE})
        ...
        self.redpanda.set_cluster_config({"sasl_kerberos_config": self.KRB5_CONF_PATH})
...
class RedpandaKerberosExternalActiveDirectoryTest(RedpandaKerberosTestBase):
    def __init__(self, test_context, **kwargs):
        ...
        ssh_config = RemoteAccountSSHConfig(
            host=ip, hostname=ip, user=ssh_username, password=ssh_password
        )
        wra = WindowsRemoteAccount(ssh_config=ssh_config, externally_routable_ip=ip)
        kdc = ActiveDirectoryKdc(
        ...
        super(RedpandaKerberosExternalActiveDirectoryTest, self).__init__(
            test_context, num_nodes=2, kdc=kdc, realm=realm, **kwargs
        )
...
class GSSAPIReauthTest(RedpandaKerberosTestBase):
    ...
    def __init__(self, test_context, **kwargs):
        super().__init__(test_context, conn_max_reauth_ms=self.MAX_REAUTH_MS, **kwargs)
