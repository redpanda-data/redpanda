...
class RedpandaStartupTest(RedpandaTest):
    ...
    def __init__(self, test_context):
        super(RedpandaStartupTest, self).__init__(
            test_context=test_context, node_ready_timeout_s=10
        )
...
class RedpandaFIPSStartupTest(RedpandaFIPSStartupTestBase):
    ...
    def __init__(self, test_context):
        super(RedpandaFIPSStartupTest, self).__init__(test_context=test_context)
    ...
    def test_startup(self):
        ...
        def check_fips_mode_metric(
            ...
            assert isinstance(metrics, MetricSamples), (
                f"Failed to get metrics {metrics_name}"
            )
    ...
    def test_non_homogenous(self):
        ...
        def get_metrics_value(
            ...
            samples = [
                sample for sample in metrics.samples if sample.node == leader_node
            ]
            assert len(samples) == 1, f"Invalid number of samples: {len(samples)}"
            return int(samples[0].value)

        def target_node_reports_fips_mode(
            admin: Admin, node: ClusterNode, fips_mode: str
        ) -> bool:
            node_id = self.redpanda.node_id(node)
            brokers = admin.get_brokers(node)
            return all(
                [
                    b["in_fips_mode"] == fips_mode
                    for b in brokers
        ...
        self.redpanda.restart_nodes([target_node], override_cfg_params=NON_FIPS_CONFIG)

        wait_until(
            lambda: target_node_reports_fips_mode(admin, target_node, "disabled"),
            timeout_sec=5,
            backoff_sec=0.2,
        ...
        wait_until(
            ...
            timeout_sec=5,
            backoff_sec=0.2,
            err_msg=f"Metrics endpoint never returned 1 after bringing up {target_node} in non-FIPS mode",
        )

        self.logger.debug(f"Restarting node {target_node} in FIPS mode")

        self.redpanda.restart_nodes([target_node], override_cfg_params=FIPS_CONFIG)
        wait_until(
            lambda: target_node_reports_fips_mode(admin, target_node, "permissive"),
            timeout_sec=5,
            backoff_sec=0.2,
        ...
        wait_until(
            ...
            timeout_sec=20,
            backoff_sec=0.2,
            err_msg=f"Metrics endpoint never returned 0 after restarting {target_node} in FIPS mode",
        )
...
class RedpandaFIPSStartupLicenseTest(RedpandaFIPSStartupTestBase):
    ...
    def test_fips_license_nag(self):
        ...
        self.redpanda.restart_nodes(
            self.redpanda.nodes, override_cfg_params=fips_config
        )
