...
class RpkDebugBundleTest(RedpandaTest):
    ...
    def test_debug_bundle(self):
        ...
        if any_errs:
            assert error_lines, (
                f"Found error, but unable to parse error lines: {output}"
            )
        ...
        assert output_file == file_path, (
            f"Expected output file({output_file}) to be in {file_path}"
        )
        ...
        assert f"{root_name}/redpanda.yaml" in files, (
            f"redpanda.yaml not found in the zip files: {files}"
        )
        assert f"{root_name}/redpanda.log" in files, (
            f"redpanda.log not found in the zip files: {files}"
        )

        # At least the first controller log is being saved:
        assert (
            f"{root_name}/controller-logs/redpanda/controller/0_0/0-1-v1.log" in files
        ), f"controller log (0-1-v1.log) not found in the zip files: {files}"

        # Cluster admin API calls:
        assert f"{root_name}/admin/brokers.json" in files, (
            f"admin/brokers.json not found in the zip files: {files}"
        )
        assert f"{root_name}/admin/cluster_config.json" in files, (
            f"admin/cluster_config.json not found in the zip files: {files}"
        )
        assert f"{root_name}/admin/health_overview.json" in files, (
            f"admin/health_overview.json not found in the zip files: {files}"
        )
        ...
        for n in self.redpanda.started_nodes():
            ...
            assert (
                f"{root_name}/metrics/{n.account.hostname}-9644/t0_metrics.txt" in files
            ), (
                f"/metrics/{n.account.hostname}-9644/t0_metrics.txt not found in the zip files: {files}"
            )
            assert (
                f"{root_name}/metrics/{n.account.hostname}-9644/t1_metrics.txt" in files
            ), (
                f"/metrics/{n.account.hostname}-9644/t1_metrics.txt not found in the zip files: {files}"
            )
            assert (
                f"{root_name}/metrics/{n.account.hostname}-9644/t0_public_metrics.txt"
                in files
            ), (
                f"/metrics/{n.account.hostname}-9644/t0_public_metrics.txt not found in the zip files: {files}"
            )
            assert (
                f"{root_name}/metrics/{n.account.hostname}-9644/t1_public_metrics.txt"
                in files
            ), (
                f"/metrics/{n.account.hostname}-9644/t1_public_metrics.txt not found in the zip files: {files}"
            )
            # and 1 cluster_view and node_config per node:
            assert (
                f"{root_name}/admin/cluster_view_{n.account.hostname}-9644.json"
                in files
            ), (
                f"/admin/cluster_view_{n.account.hostname}-9644.json not found in the zip files: {files}"
            )
            assert (
                f"{root_name}/admin/node_config_{n.account.hostname}-9644.json" in files
            ), (
                f"/admin/node_config_{n.account.hostname}-9644.json not found in the zip files: {files}"
            )
            assert (
                f"{root_name}/admin/cpu_profile_{n.account.hostname}-9644.json" in files
            ), (
                f"/admin/cpu_profile_{n.account.hostname}-9644.json not found in the zip files: {files}"
            )
    ...
    def test_remote_debug_bundle_default(self):
        ...
        def _get_job_ID(output):
            ...
            assert len(set(job_ids)) == 1, f"Not all job IDs are the same: {job_ids}"
            return job_ids[0]

        out = self._rpk.remote_bundle_start()
        job_id = _get_job_ID(out)
        assert "The debug bundle collection process has started" in out, (
        ...
        def _validate_bundle_file(output_file):
            ...
            with zipfile.ZipFile(output_file) as zf:
                ...
                files = zf.namelist()
                assert len(files) == 3, (
                    f"Downloaded zip contains less files than the number of brokers: {files}"
                )
                    ...
                    with zf.open(broker_file) as inner_file:
                        with zipfile.ZipFile(inner_file) as bzf:
                            ...
                            assert f"{root_name}/admin/brokers.json" in bfiles, (
                                f"{root_name}/admin/brokers.json not found in zip files: {bfiles}"
                            )
                            # Kafka metadata
                            assert f"{root_name}/kafka.json" in bfiles, (
                                f"{root_name}/kafka.json not found in zip files: {bfiles}"
                            )

        with tempfile.TemporaryDirectory() as td:
            output_file = os.path.join(td, "test.zip")
            out = self._rpk.remote_bundle_download(job_id, output_file=output_file)
