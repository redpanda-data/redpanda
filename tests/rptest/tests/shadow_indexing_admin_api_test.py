...
class SIAdminApiTest(RedpandaTest):
    ...
    def __init__(self, test_context):
        ...
        super(SIAdminApiTest, self).__init__(
    ...
    def test_bucket_validation(self, cloud_storage_type):
        ...
        self.kafka_tools.alter_topic_config(
            self.topic,
            {
                TopicSpec.PROPERTY_RETENTION_LOCAL_TARGET_BYTES: self.local_retention,
            },
        )

        produce_until_segments(
            redpanda=self.redpanda, topic=self.topic, partition_idx=0, count=10, acks=-1
        )

        wait_for_local_storage_truncate(
            self.redpanda, self.topic, target_bytes=self.local_retention
        )
        ...
        for partition in rpk.describe_topic(self.topic):
            ...
            assert partition.start_offset == 0, (
                f"start-offset of the partition is supposed to be 0 instead of {partition.start_offset}"
            )
        ...
        self.cloud_storage_client.delete_object(
            self.s3_bucket_name, segment_to_remove, True
        )
        ...
        for part in self.rpk.describe_topic(self.topic):
            ...
            assert part.start_offset > 0, (
                f"start-offset of the partition is {part.start_offset}, should be greater than 0"
            )
    ...
    def _get_non_controller_node(self):
        self.admin.wait_stable_configuration("controller", namespace="redpanda")
    ...
    def test_topic_recovery_redirects_to_controller_leader(self):
        ...
        assert response.status_code == requests.status_codes.codes["temporary_redirect"]
    ...
    def test_topic_recovery_request_validation(self):
        for payload in (
            ...
            try:
                ...
                ), f"unexpected status code: {e.response} for {payload}"
