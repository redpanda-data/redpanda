...
class ShadowIndexingCompactedTopicTest(EndToEndTest):
    ...
    def __init__(self, test_context):
        ...
        self.si_settings = SISettings(
            test_context, cloud_storage_max_connections=5, fast_uploads=True
        )
        extra_rp_conf = dict(
    ...
    def test_upload(self, cloud_storage_type):
        ...
        self._rpk_client.cluster_config_set(
            "log_compaction_interval_ms", f"{1000 * 60 * 60}"
        )
        ...
        m = MetricCheck(
            self.logger,
            self.redpanda,
            self.redpanda.nodes[original_topic_describe.leader - 1],
            [metric],
        ...
        for node, node_segments in original_snapshot.items():
            assert len(node_segments) >= expected_segment_count, (
                f"Expected at least {expected_segment_count} segments, "
        ...
        self._rpk_client.alter_topic_config(
            self.topic, "retention.local.target.bytes", f"{5 * self.segment_size}"
        )
        ...
        self._rpk_client.cluster_config_set("log_compaction_interval_ms", f"{2000}")
        ...
        def compacted_segments_uploaded():
            manifest = BucketView(self.redpanda, topics=self.topics).manifest_for_ntp(
                self.topic, partition=0
            )
            return any(meta["is_compacted"] for meta in manifest["segments"].values())
...
class TSWithAlreadyCompactedTopic(EndToEndTest):
    ...
    def test_initial_upload(self):
        ...
        self._rpk_client.cluster_config_set("log_compaction_interval_ms", "9999999")
        ...
        self._rpk_client.cluster_config_set("log_compaction_interval_ms", "500")
