...
class ShardPlacementTest(PreallocNodesTest):
    ...
    def stop_client_load(self):
        self.producer.stop()
        self.consumer.wait_total_reads(
            self.producer.produce_status.acked, timeout_sec=60, backoff_sec=1
        )
        self.consumer.stop()
    ...
    def get_replica_shard_map(self, nodes, admin=None):
        ...
        for node in nodes:
            ...
            for p in partitions:
                ...
                topic2partition2shard.setdefault(p["topic"], dict()).setdefault(
                    p["partition_id"], list()
                ).append((self.redpanda.node_id(node), p["core"]))
    ...
    def get_shard_map_from_cluster_partitions(self, partitions):
        ...
        for p in partitions:
            ...
            topic2partition2shard.setdefault(p["topic"], dict())[p["partition_id"]] = (
                replicas
            )
    ...
    def shard_maps_equal(self, left, right):
        ...
        if left_topics != right_topics:
            self.logger.debug(f"topic sets differ: {left_topics} vs. {right_topics}")
            return False
        ...
        for topic, left_partitions in left.items():
            ...
            if left_ids != right_ids:
                self.logger.debug(
                    f"partition sets for topic {topic} differ: "
                ...
                for replica_id, core in replicas:
                    if replica_id == node_id:
                        topic2shard2count.setdefault(
                            topic, list(0 for _ in range(core_count))
                        )[core] += 1
    ...
    def wait_shard_map_stationary(
        self, nodes, admin=None, timeout_sec=10, backoff_sec=2
    ):
        shard_map = None
        ...
        def is_stationary():
            ...
            if shard_map is not None and self.shard_maps_equal(new_map, shard_map):
                return True
            else:
                shard_map = new_map

        wait_until(is_stationary, timeout_sec=timeout_sec, backoff_sec=backoff_sec)
        return shard_map
    ...
    def wait_shard_map_consistent_with_cluster_partitions(
        ...
        def is_consistent():
            ...
            if not self.shard_maps_equal(
                self.get_shard_map_from_cluster_partitions(all_partitions), shard_map
            ):
                return False
            ...
            for topic in user_topics:
                user_partitions.extend(admin.get_cluster_partitions("kafka", topic))
        ...
        wait_until(is_consistent, timeout_sec=timeout_sec, backoff_sec=backoff_sec)

    @cluster(num_nodes=6)
    def test_upgrade(self):
        # Disable partition balancer in this test, as we need partitions
        # to remain stationary.
        self.redpanda.add_extra_rp_conf({"partition_autobalancing_mode": "off"})
        ...
        self.redpanda.await_feature(
            "node_local_core_assignment", "active", timeout_sec=15
        )

        self.logger.info("feature enabled, checking that shard map is stable...")
        ...
        map_after_manual_move = self.wait_shard_map_stationary(seed_nodes, admin)
        self.print_shard_stats(map_after_manual_move)
        foo_shard_counts = self.get_shard_counts_by_topic(
            map_after_manual_move, moved_replica_id
        )["foo"]
        ...
        for joiner in joiner_nodes:
            ...
            shard_counts = self.get_shard_counts_by_topic(map_after_join, joiner_id)[
                "quux"
            ]
        ...
        quux_partitions_on_joiner = [
            p
            for p, rs in map_after_join["quux"].items()
        ...
        # check that they indeed moved
        self.logger.info(
            f"manually moved some replicas on node {joiner_id}, checking shard map..."
        )
        map_after_manual_move2 = self.wait_shard_map_stationary(joiner_nodes, admin)
        self.print_shard_stats(map_after_manual_move2)
        quux_shard_counts = self.get_shard_counts_by_topic(
            map_after_manual_move2, joiner_id
        )["quux"]
        ...
        self.redpanda.set_cluster_config({"core_balancing_on_core_count_change": False})

        map_before_restart = self.wait_shard_map_stationary(self.redpanda.nodes, admin)
        ...
        map_after_restart = self.wait_shard_map_stationary(self.redpanda.nodes, admin)
    ...
    def test_manual_rebalance(self):
        ...
        for p in range(n_partitions):
            admin.set_partition_replica_core(
                topic="foo", partition=p, replica=moved_replica_id, core=0
            )
            admin.set_partition_replica_core(
                topic="bar", partition=p, replica=moved_replica_id, core=core_count - 1
            )
        ...
        counts_by_topic = self.get_shard_counts_by_topic(shard_map, moved_replica_id)
    ...
    def test_core_count_change(self):
        ...
        def restart_node(num_cpus):
            ...
            self.redpanda.set_resource_settings(ResourceSettings(num_cpus=num_cpus))
            ...
            def configuration_updated():
                for n in self.redpanda.nodes:
                    broker = [
                        b for b in admin.get_brokers(node=n) if b["node_id"] == node_id
                    ][0]
        ...
        foo_partitions_on_node = [
            p for p, rs in shard_map["foo"].items() if any(n == node_id for n, _ in rs)
        ]
        for p in foo_partitions_on_node:
            admin.set_partition_replica_core(
                topic="foo", partition=p, replica=node_id, core=0
            )
        shard_map = self.wait_shard_map_stationary([node], admin)
        self.print_shard_stats(shard_map)
    ...
    def test_node_join(self, disable_license):
        ...
        if disable_license:
            ...
            assert not rebalanced, (
                f"continuous core balancing should not be active. Received {rebalanced}"
            )
        else:
