...
class SimpleEndToEndTest(EndToEndTest):
    def __init__(self, test_context, *args, **kwargs):
        super(SimpleEndToEndTest, self).__init__(
    ...
    def test_correctness_while_evicitng_log(self):
        ...
        self.run_validation(
            min_records=100000, producer_timeout_sec=300, consumer_timeout_sec=300
        )
    ...
    def test_consumer_interruption(self):
        ...
        try:
            self.run_validation(
                min_records=30000, producer_timeout_sec=300, consumer_timeout_sec=300
            )
        ...
        assert "Consumed from an unexpected" in str(
            error
        ) or "is behind the current committed offset" in str(error)
    ...
    def test_relaxed_acks(self, write_caching, disable_batch_cache=False):
        ...
        def inject_failures():
            ...
            while not stop_fi_ev.is_set():
                node = random.choice(self.redpanda.nodes)
                    ...
                    FailureSpec(type=FailureSpec.FAILURE_KILL, length=0, node=node)
                )
                time.sleep(10)
        ...
        self.start_redpanda(
            ...
            extra_rp_conf={
                ...
                "retention_bytes": 5242880,
                "default_topic_replications": 3,
                "write_caching_default": wc_conf,
                "raft_replica_max_pending_flush_bytes": 1024 * 1024 * 1024 * 1024,
                "raft_replica_max_flush_delay_ms": 3000000,
        ...
        spec = TopicSpec(
            name="verify-leader-ack", partition_count=16, replication_factor=3
        )
        self.client().create_topic(spec)
