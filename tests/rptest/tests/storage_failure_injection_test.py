...
class StorageFailureInjectionTest(RedpandaTest):
    ...
    def __init__(self, test_context):
        super(StorageFailureInjectionTest, self).__init__(test_context, num_brokers=1)
    ...
    def generate_failure_injection_config(self) -> FailureInjectionConfig:
        ...
        ntp_fcfg = NTPFailureInjectionConfig(ntp=self.ntp, failure_configs=[fcfg])
    ...
    def test_storage_failure_injection(self):
        ...
        Admin(self.redpanda).set_storage_failure_injection(lonely_node, value=True)
        ...
        for _ in range(3):
            try:
                rpk.produce(self.ntp.topic, key="poisoned", msg="chalice", timeout=1)
        ...
        wait_until(
            lambda: next(rpk.describe_topic(self.ntp.topic)).high_watermark == 1,
        ...
        rpk.produce(self.ntp.topic, key="don't", msg="crash")
        wait_until(
            lambda: next(rpk.describe_topic(self.ntp.topic)).high_watermark == 2,
            timeout_sec=3,
            backoff_sec=1,
