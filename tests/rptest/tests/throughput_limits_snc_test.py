...
class ThroughputLimitsSnc(RedpandaTest):
    ...
    def __init__(self, test_ctx: TestContext, *args, **kwargs):
        super(ThroughputLimitsSnc, self).__init__(
            test_ctx, num_brokers=3, *args, **kwargs
        )
    ...
    def binexp_random(self, min: int, max: int):
        ...
        return math.floor(2 ** (self.rnd.random() * (max_exp - min_exp) + min_exp))
    ...
    def current_effective_node_quota(self) -> Tuple[int, int]:
        ...
        node_quota_in = sum(
            int(s.value) for s in metrics.label_filter({"direction": "ingress"}).samples
        )
        node_quota_eg = sum(
            int(s.value) for s in metrics.label_filter({"direction": "egress"}).samples
        )
    ...
    def test_configuration(self):
        ...
        self.logger.info(f"Initial cluster props: {self.redpanda._extra_rp_conf}")
        ...
        for _ in range(12):
            ...
            def check_node_quota_metric(
                self, config_prop: self.ConfigProp, effective_node_quota: int
            ) -> str:
            ...
            errors.append(
                check_node_quota_metric(
                    self, self.ConfigProp.QUOTA_NODE_MAX_IN, effective_node_quota[0]
                )
            )
            errors.append(
                check_node_quota_metric(
                    self, self.ConfigProp.QUOTA_NODE_MAX_EG, effective_node_quota[1]
                )
            )
    ...
    def test_consumers(self):
        ...
        self.redpanda.add_extra_rp_conf(
            {
                self.ConfigProp.QUOTA_NODE_MAX_EG.value: 400 * kiB,
                "kafka_batch_max_bytes": 1 * MiB,
            }
        )
        ...
        wait_until(
            ...
            timeout_sec=10,
            backoff_sec=0.001,
            err_msg="Timeout waiting for the first consumer to receive 10% of messages",
        )
        ...
        wait_until(
            ...
            timeout_sec=30,
            err_msg="Timeout waiting for the second consumer to join the cgroup and consume something",
        )
