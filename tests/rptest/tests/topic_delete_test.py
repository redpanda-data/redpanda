...
class TopicDeleteCloudStorageTest(RedpandaTest):
    ...
    @cluster(num_nodes=4, log_allow_list=["Failed to fetch manifest during finalize()"])
    ...
    @skip_debug_mode  # Rely on timely uploads during leader transfers
    @cluster(num_nodes=5, log_allow_list=["Failed to fetch manifest during finalize()"])
