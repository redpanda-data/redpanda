...
class BaseCase:
    ...
    def _produce_and_verify(self, topic_spec):
        ...
        wait_until(
            ...
            timeout_sec=60,
            backoff_sec=1,
            err_msg=f"failed to get high watermark before produce for {topic_spec}",
        )

        self._kafka_tools.produce(
            topic_spec.name, 10000, 1024, enable_idempotence=False
        )
        ...
        wait_until(
            ...
            timeout_sec=60,
            backoff_sec=1,
            err_msg=f"failed to get high watermark after produce for {topic_spec}",
        )
    ...
    def _get_all_topic_manifests(self):
        ...
        keys = [
            ...
            if key.endswith("topic_manifest.json") or key.endswith("topic_manifest.bin")
        ]
    ...
    def _restore_topic(self, manifest, overrides: dict[str, str] | None = None):
...
class NoDataCase(BaseCase):
    ...
    topics = (TopicSpec(name="panda-topic", partition_count=1, replication_factor=3),)
    ...
    def __init__(
        ...
        super(NoDataCase, self).__init__(
            redpanda,
            s3_client,
            kafka_tools,
            rpk_client,
...
class EmptySegmentsCase(BaseCase):
    ...
    def __init__(
        ...
        super(EmptySegmentsCase, self).__init__(
            redpanda,
            s3_client,
            kafka_tools,
            rpk_client,
    ...
    def validate_cluster(self, baseline, restored):
        ...
        wait_until(verify, timeout_sec=30, backoff_sec=1, err_msg="Invalid topic state")
...
class MissingTopicManifest(BaseCase):
    ...
    def __init__(
        ...
        super(MissingTopicManifest, self).__init__(
            redpanda,
            s3_client,
            kafka_tools,
            rpk_client,
    ...
    def restore_redpanda(self, baseline, controller_checksums):
        ...
        self.expected_recovered_topics = (
            TopicSpec(name="panda-topic", partition_count=1, replication_factor=3),
        )
...
class MissingPartition(BaseCase):
    ...
    topics = (TopicSpec(name=topic_name, partition_count=2, replication_factor=3),)
    ...
    def __init__(
        ...
        super(MissingPartition, self).__init__(
            redpanda,
            s3_client,
            kafka_tools,
            rpk_client,
    ...
    def generate_baseline(self):
        ...
        for topic in self.topics:
            producer = self._rpk_producer_maker(
                topic=topic.name, msg_count=10000, msg_size=1024
            )
            producer.start()
            producer.wait()
            producer.free()
    ...
    def _find_and_remove_partition_manifest(self):
        ...
        self._part1_offset_delta_end = max(
            [
                seg_meta["delta_offset_end"]
                for _, seg_meta in manifest_1["segments"].items()
            ]
        )
    ...
    def restore_redpanda(self, baseline, controller_checksums):
        ...
        super(MissingPartition, self).restore_redpanda(baseline, controller_checksums)
...
class MissingSegment(BaseCase):
    ...
    topics = (TopicSpec(name="panda-topic", partition_count=2, replication_factor=3),)
    ...
    def __init__(
        ...
        super(MissingSegment, self).__init__(
            redpanda,
            s3_client,
            kafka_tools,
            rpk_client,
    ...
    def restore_redpanda(self, baseline, controller_checksums):
        ...
        super(MissingSegment, self).restore_redpanda(baseline, controller_checksums)

    def validate_cluster(self, baseline, restored):
        """Check that the topic is writeable"""
        self.logger.info(f"MissingSegment.validate_cluster - baseline - {baseline}")
        self.logger.info(f"MissingSegment.validate_cluster - restored - {restored}")
        expected_topics = [topic.name for topic in self.expected_recovered_topics]
        verify_file_layout(
            baseline,
            restored,
            expected_topics,
...
class FastCheck(BaseCase):
    ...
    def _collect_non_data_batch_sizes(self, expected_topics):
        ...
        segments = [
            item
            for item in self._s3.list_objects(self._bucket)
...
class SizeBasedRetention(BaseCase):
    ...
    def __init__(
        ...
        super(SizeBasedRetention, self).__init__(
            redpanda,
            s3_client,
            kafka_tools,
            rpk_client,
    ...
    def generate_baseline(self):
        ...
        for topic in self.topics:
            ...
            message_size = 1024
            num_messages = int(
                topic.partition_count * self.max_size_bytes / message_size
            )
        ...
        for topic in self.expected_recovered_topics:
            for _, manifest in topic_manifests:
                if manifest["topic"] == topic.name:
                    self._restore_topic(
                        manifest,
                        {"retention.local.target.bytes": self.restored_size_bytes},
                    )

    def validate_cluster(self, baseline, restored):
        """Check size of every partition."""
        self.logger.info(f"SizeBasedRetention.validate_cluster - baseline - {baseline}")
        self.logger.info(f"SizeBasedRetention.validate_cluster - restored - {restored}")
...
class TimeBasedRetention(BaseCase):
    ...
    def __init__(
        ...
        super(TimeBasedRetention, self).__init__(
            redpanda,
            s3_client,
            kafka_tools,
            rpk_client,
    ...
    def generate_baseline(self):
        ...
        for topic in self.topics:
            ...
            producer = KgoVerifierProducer(
                context=self._redpanda._context,
        ...
        for topic in self.expected_recovered_topics:
            for _, manifest in topic_manifests:
                if manifest["topic"] == topic.name:
                    # retention - 1 hour
                    self._restore_topic(
                        manifest, {"retention.local.target.ms": 3600000}
                    )

    def validate_cluster(self, baseline, restored):
        """Check that the topic is writeable"""
        self.logger.info(f"TimeBasedRetention.validate_cluster - baseline - {baseline}")
        self.logger.info(f"TimeBasedRetention.validate_cluster - restored - {restored}")
...
class VirtualClusterIdPropertyCase(BaseCase):
    def __init__(
        ...
        super(VirtualClusterIdPropertyCase, self).__init__(
            redpanda,
            s3_client,
            kafka_tools,
            rpk_client,
        ...
        for topic in self.topics:
            self._rpk.create_topic(
                ...
                config={
                    "redpanda.remote.write": "true",
                    "redpanda.remote.read": "true",
                    "redpanda.virtual.cluster.id": topic.virtual_cluster_id,
                },
            )
...
class AdminApiBasedRestore(FastCheck):
    ...
    def _assert_retention(self, value):
        ...
        assert topic_config["retention.local.target.bytes"] == value, (
            f"failed: {topic_config}"
        )
    ...
    def _assert_status(self):
        def wait_for_status():
            ...
            assert r.status_code == requests.status_codes.codes["ok"], (
                f"request status code: {r.status_code}"
            )
            response = r.json()
...
class ManyPartitionsCase(BaseCase):
    ...
    def generate_baseline(self):
        ...
        quiesce_uploads(self._redpanda, [topic.name for topic in self.topics], 400)

    def restore_redpanda(self, baseline, controller_checksums):
        # set check mode, start recovery of topics
        self._redpanda.set_cluster_config(
            values={"cloud_storage_recovery_topic_validation_mode": self.check_mode}
        )
        return super().restore_redpanda(baseline, controller_checksums)
...
class PreventRecoveryOfDamagedPartitionsCase(BaseCase):
    ...
    def __init__(
        ...
        self.recoverable_topic = TopicSpec(
            name="panda-topic-recovery-ok", partition_count=1, replication_factor=3
        )
        # this topic will be damaged by removing a segment
        self.unrecoverable_topic = TopicSpec(
            name="panda-topic-recovery-NOT-OK", partition_count=1, replication_factor=1
        )
    ...
    def generate_baseline(self):
        ...
        quiesce_uploads(self._redpanda, [topic.name for topic in self.topics], 300)
    ...
    def _find_and_remove_newest_segments(self, topic: TopicSpec, skip: int):
        ...
        segs_for_topic_partition: dict[int, str] = {
            ...
            if comps.ntpr.topic == topic.name and comps.ntpr.partition == partition
        }

        # sort in descending order by base offset, skip the first #skip elements, pick the first as the victim
        victim = sorted(segs_for_topic_partition.items(), reverse=True)[skip : skip + 1]
        assert len(victim) > 0, (
            f"no segment found for {topic.name}/{partition} at depth {skip}. total size {len(segs_for_topic_partition)}"
        )
    ...
    def validate_cluster(self, baseline, restored):
        ...
        assert (
            len(list(self._rpk.describe_topic(self.unrecoverable_topic.name))) == 0
        ), f"topic {self.unrecoverable_topic.name} is present"
    ...
    def restore_redpanda(self, baseline, controller_checksums):
        ...
        self._redpanda.set_cluster_config(
            values={
                "cloud_storage_recovery_topic_validation_mode": "check_manifest_and_segment_metadata",
                "cloud_storage_recovery_topic_validation_depth": "10",
            }
        )

        # restore recoverable topic. this should not fail
        super().restore_redpanda(
            baseline, controller_checksums, [self.recoverable_topic]
        )
        ...
        try:
            super().restore_redpanda(
                baseline, controller_checksums, [self.unrecoverable_topic]
            )
        except RpkException as e:
            exception = e
        assert exception is not None, (
            f"RpkException expected for unrecoverable {self.unrecoverable_topic.name}"
        )
...
class TopicRecoveryTest(RedpandaTest):
    def __init__(self, test_context: TestContext, *args, **kwargs):
        ...
        assert si_settings.cloud_storage_bucket is not None, (
            "Cloud storage bucket not set"
        )
        ...
        super(TopicRecoveryTest, self).__init__(
            ...
            *args,
            ...
            extra_rp_conf={
                "cloud_storage_recovery_topic_validation_mode": "check_manifest_existence",
    ...
    def rpk_producer_maker(
        self, topic: str, msg_size: int, msg_count: int, acks: Optional[int] = None
    ) -> RpkProducer:
        return RpkProducer(
            self.test_context,
            self.redpanda,
    ...
    def _get_data_log_segment_checksums(self, node: ClusterNode) -> FileToChecksumSize:
        ...
        def included(path):
            ...
            internal_log_prefix = os.path.join(
                RedpandaService.DATA_DIR, "kafka_internal"
            )
            log_segment_extension = ".log"
            return (
                not path.startswith(controller_log_prefix)
    ...
    def _get_log_segment_checksums(
        self, node: ClusterNode, include_condition
    ) -> FileToChecksumSize:
    ...
    def _collect_replaced_segments(
        self, replaced: dict[NTPR, dict[str, dict]], manifest_key: str
    ):
        data = self.cloud_storage_client.get_object_data(self.s3_bucket, manifest_key)
        manifest = None
    ...
    def _wait_for_data_in_s3(
        self, expected_topics, timeout=datetime.timedelta(minutes=1)
    ):
        ...
        def verify():
            ...
            segments = [segment for segment in segments if not is_replaced(segment)]
            ...
            if total_partitions != len(manifests):
                self.logger.info(
                    f"can't find enough manifest objects, "
            ...
            for node, files in self._collect_file_checksums().items():
                ...
                for path, (_, size) in files.items():
                    if (
                        path_matcher.path_matches_any_topic(path)
                        and size > EMPTY_SEGMENT_SIZE
                    ):
                        tmp_size += size
            ...
            is_stable = (
                delta <= tolerance
                and len(cloud_sizes) == cloud_sizes.maxlen
                and cloud_sizes.count(size_in_cloud) == len(cloud_sizes)
            )
            if not is_stable:
                self.logger.info(
    ...
    def _wait_for_topic(self, recovered_topics, timeout=datetime.timedelta(minutes=1)):
        ...
        def all_replicas_in_sync(topic, *, partition, num_replicas):
            partition_state = self.admin.get_partition_state("kafka", topic, partition)
            if len(partition_state["replicas"]) != num_replicas:
                return False
            hwms = [
                replica["high_watermark"] for replica in partition_state["replicas"]
            ]
                ...
                for partition in topic_state:
                    ...
                    if partition.leader in partition.replicas:
                        ...
                        if not all_replicas_in_sync(
                            ...
                            self.logger.debug("partition replicas are not in sync yet")
                            return False
    ...
    def do_run(self, test_case: BaseCase, upload_delay_sec=60):
        ...
        if test_case.verify_s3_content_after_produce:
            ...
            self._wait_for_data_in_s3(
                test_case.topics, timeout=datetime.timedelta(seconds=upload_delay_sec)
            )
    ...
    @cluster(num_nodes=3, log_allow_list=MISSING_DATA_ERRORS + TRANSIENT_ERRORS)
    ...
    def test_missing_topic_manifest(self, cloud_storage_type):
        ...
        in old cluster the empty topic should be created. We should be able
        to produce to the topic."""
        test_case = MissingTopicManifest(
            self.redpanda,
            self.cloud_storage_client,
            self.kafka_tools,
            self.rpk,
    ...
    @cluster(num_nodes=4, log_allow_list=MISSING_DATA_ERRORS + TRANSIENT_ERRORS)
    @matrix(cloud_storage_type=get_cloud_storage_type())
    ...
    @cluster(
        num_nodes=4,
        log_allow_list=MISSING_DATA_ERRORS
        + TRANSIENT_ERRORS
        + ALLOWED_REPLICA_VALIDATOR_ERRORS,
    )
    @matrix(cloud_storage_type=get_cloud_storage_type())
    ...
    def test_fast2(self, cloud_storage_type):
        ...
        of the topic with different set of data."""
        topics = [
            TopicSpec(name="panda-topic-1", partition_count=3, replication_factor=3),
            TopicSpec(name="panda-topic-2", partition_count=3, replication_factor=3),
        ]
        test_case = FastCheck(
            self.redpanda,
            self.cloud_storage_client,
    ...
    def test_fast3(self, cloud_storage_type):
        ...
        of the topic with different set of data."""
        topics = [
            TopicSpec(name="panda-topic-1", partition_count=2, replication_factor=3),
            TopicSpec(name="panda-topic-2", partition_count=2, replication_factor=3),
            TopicSpec(name="panda-topic-3", partition_count=2, replication_factor=3),
        ]
        test_case = FastCheck(
            self.redpanda,
            self.cloud_storage_client,
    ...
    def test_admin_api_recovery(self, cloud_storage_type):
        ...
        test_case = AdminApiBasedRestore(
            self.redpanda,
            self.cloud_storage_client,
            self.kafka_tools,
            self.rpk,
    ...
    def test_vcluster_id(self, cloud_storage_type):
        test_case = VirtualClusterIdPropertyCase(
            self.redpanda,
            self.cloud_storage_client,
            self.kafka_tools,
            self.rpk,
