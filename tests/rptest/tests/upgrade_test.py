...
class UpgradeFromSpecificVersion(RedpandaTest):
    ...
    def __init__(self, test_context):
        super(UpgradeFromSpecificVersion, self).__init__(
...
class UpgradeFromPriorFeatureVersionTest(RedpandaTest):
    ...
    def __init__(self, test_context):
        super(UpgradeFromPriorFeatureVersionTest, self).__init__(
...
class UpgradeBackToBackTest(PreallocNodesTest):
    ...
    def __init__(self, test_context):
        if self.debug_mode:
            ...
            self.topics = (TopicSpec(partition_count=1, replication_factor=3),)
        super(UpgradeBackToBackTest, self).__init__(
            test_context, num_brokers=3, node_prealloc_count=1
        )
        self.installer = self.redpanda._installer
        self.versions = []
    ...
    def test_upgrade_with_all_workloads(self, single_upgrade):
        ...
        for current_version in self.upgrade_through_versions(self.versions):
            if not started:
                ...
                self._producer.wait_for_acks(100, timeout_sec=10, backoff_sec=2)
            ...
            assert (
                self._seq_consumer.consumer_status.validator.valid_reads
                >= wrote_at_least
            )
            assert (
            ...
            assert (
                self._cg_consumer.consumer_status.validator.valid_reads
                >= wrote_at_least
            )
...
class UpgradeWithWorkloadTest(EndToEndTest):
    ...
    def setUp(self):
        ...
        self.start_redpanda(
            num_nodes=3, install_opts=InstallOptions(install_previous_version=True)
        )
        self.installer = self.redpanda._installer
    ...
    def test_rolling_upgrade(self):
        ...
        self.run_validation(
            min_records=post_upgrade_num_msgs + (self.producer_msgs_per_sec * 3)
        )
    ...
    def test_rolling_upgrade_with_rollback(self, upgrade_after_rollback):
        ...
        # Give ample time to restart, given the running workload.
        self.redpanda.rolling_restart_nodes(
            [first_node], start_timeout=90, stop_timeout=90
        )

        def await_progress():
            num_msgs = self.producer.num_acked
            self.await_num_produced(num_msgs + (self.producer_msgs_per_sec * 3))
            self.await_num_consumed(num_msgs + (self.producer_msgs_per_sec * 3))
...
class RedpandaInstallerTest(RedpandaTest):
    ...
    def test_install_by_line(self):
        ...
        head_version, head_version_str = self.redpanda._installer.install(
            self.redpanda.nodes, version=self.redpanda._installer._head_version[0:2]
        )
...
class UpgradeAndCheckRecoveryReads(RedpandaTest):
    ...
    def test_basic_upgrade(self):
        topic = TopicSpec(
            partition_count=3, replication_factor=3, segment_bytes=1024 * 1024
        )
        ...
        time.sleep(5)
        self.redpanda.restart_nodes(self.redpanda.nodes)
        read_before_upgrade = self.redpanda.estimate_total_disk_bytes_read() / len(
            self.redpanda.nodes
        )
        node = self.redpanda.nodes[0]
        initial_version = Version(self.redpanda.get_version(node))
        ...
        time.sleep(5)
        read_after_upgrade = self.redpanda.estimate_total_disk_bytes_read() / len(
            self.redpanda.nodes
        )
        ...
        assert (
            read_after_upgrade
            <= read_before_upgrade * after_upgrade_read_increase_ratio
        ), (
        ...
        # restart once
        self.redpanda.restart_nodes(self.redpanda.nodes)
        read_after_restart = self.redpanda.estimate_total_disk_bytes_read() / len(
            self.redpanda.nodes
        )
        # wait for the cluster to stabilize, and any necessary recovery to finish
        time.sleep(5)
