...
class UsageWindow:
    def __init__(
        self, begin, end, is_open, bytes_sent, bytes_received, bytes_in_cloud_storage
    ):
        self.begin = begin
        self.end = end
...
def assert_usage_consistency(node, a, b):
    ...
    def compare(x, y, fn):
        return fn(x.bytes_sent, y.bytes_sent) and fn(x.bytes_received, y.bytes_received)
    ...
    for x in b:
        ...
        if a_value is not None:
            ...
            if a_value.is_closed and x.is_closed:
                assert a_value.bytes_in_cloud_storage == x.bytes_in_cloud_storage, (
                    f"Failed bytes_in_cs compare node: {node} a: {str(a_value)} b:{str(x)}"
                )
...
class UsageTest(RedpandaTest):
    ...
    def __init__(self, test_context):
        self._settings = self._usage_conf(
            enable_usage=True, num_windows=30, window_interval=3, disk_write_interval=5
        )
        super(UsageTest, self).__init__(
            test_context=test_context, log_level="debug", extra_rp_conf=self._settings
        )
    ...
    def _usage_conf(
        ...
        if disk_write_interval is not None:
            changes["usage_disk_persistance_interval_sec"] = disk_write_interval
    ...
    def _get_usage(self, node, include_open, retries=3):
        ...
        while retries > 0:
            try:
                ...
                self.redpanda.logger.error(f"Error making v1/usage request: {e}")
                retries -= 1
                time.sleep(1)
                exc = e
        raise exc

    def _get_all_usage(self, include_open=True, allow_gaps=False, only_nodes=None):
        """
        ...
        def validate(node, node_response):
            ...
            for e in node_response:
                ...
                assert e.end - e.begin >= self.window_interval, (
                    f"Expected interval: {self.window_interval} observed {e.end - e.begin}"
                )
                assert e.begin < e.end, f"Begin: {e.begin}, End: {e.end}"
                assert e.begin < prev_begin, (
                    f"Begin: {e.begin}, PrevBegin: {prev_begin}"
                )
                assert e.end < prev_end, f"End: {e.end}, PrevEnd: {prev_end}"
                if prev_begin != float("inf"):
                    if not allow_gaps:
                        assert e.end == prev_begin, (
                            f"End: {e.end} PrevEnd: {prev_begin}"
                        )
                    else:
                        assert e.end <= prev_begin, (
                            f"End: {e.end}, PrevBegin: {prev_begin}"
                        )
                prev_begin = e.begin
                prev_end = e.end
        ...
        return flat_map(
            lambda x: validate(x.name, self._get_usage(x, include_open)), nodes
        )
    ...
    def test_usage_metrics_collection(self):
        ...
        while iterations < (self.num_windows + 7):
            ...
            if len(usage) == len(prev_usage):
                ...
                assert total_data >= total_prev, (
                    f"Expected {total_data} >= {total_prev} itr: {iterations}"
                )
    ...
    def test_usage_collection_restart(self):
        ...
        usage_pre_restart = self._calculate_total_usage(
            self._get_all_usage(
                include_open=True, allow_gaps=True, only_nodes=[self.redpanda.nodes[0]]
            )
        )
        ...
        assert usage_post_restart >= usage_pre_restart, (
            f"Usage post restart: {usage_post_restart} Usage pre restart: {usage_pre_restart}"
        )
    ...
    def test_usage_settings_changed(self):
        ...
        self._modify_settings(
            enable_usage=True, num_windows=3, window_interval=2, disk_write_interval=5
        )
...
class UsageTestCloudStorageMetrics(RedpandaTest):
    ...
    def __init__(self, test_context):
        ...
        extra_rp_conf = dict(
            health_monitor_max_metadata_age=2000,
            enable_usage=True,
    ...
    def test_usage_manager_cloud_storage(self):
        ...
        def check_usage():
            ...
            manifest_usage = bucket_view.cloud_log_sizes_sum().total(no_archive=True)
            # Only active leader will have latest value of cs_bytes
            reported_usages = flat_map(
                lambda node: self.admin.get_usage(node), self.redpanda.nodes
            )
            reported_usages = [x["cloud_storage_bytes_gauge"] for x in reported_usages]
            ...
            self.logger.info(f"Metric reported cloud storage usage: {metric_usage}")
