...
class CompactionE2EIdempotencyTest(RedpandaTest):
    def __init__(self, test_context):
        ...
        super(CompactionE2EIdempotencyTest, self).__init__(
    ...
    @cluster(num_nodes=4)
    @matrix(
        initial_cleanup_policy=[TopicSpec.CLEANUP_COMPACT, TopicSpec.CLEANUP_DELETE],
    ...
    def test_basic_compaction(self, initial_cleanup_policy, workload):
        ...
        m = MetricCheck(
            self.logger,
            self.redpanda,
            self.redpanda.partitions(self.topics[0].name)[0].leader,
            [metric],
            labels={
                "namespace": "kafka",
        ...
        rw_verifier = CompactedVerifier(self.test_context, self.redpanda, workload)
        rw_verifier.start()

        rw_verifier.remote_start_producer(
            self.redpanda.brokers(), self.topic, self.partition_count
        )
        ...
        self.logger.info(f"setting log_compaction_interval_ms to {3600 * 1000}")
        ...
        def segment_number_matches(predicate):
            ...
            self.logger.debug(f"Topic {self.topic} {segments_per_partition_per_node=}")
...
class CompactionWithRecoveryTest(RedpandaTest, PartitionMovementMixin):
    ...
    def test_tx_compaction_with_recovery(self):
        ...
        workload = CompactedVerifier(self.test_context, self.redpanda, Workload.TX)
        workload.start()

        workload.remote_start_producer(
            self.redpanda.brokers(), self.topic, 1, abort_probability=1
        )
...
class CompactionE2ERebootTest(RedpandaTest):
    def __init__(self, test_context):
        self.segment_size = 5 * 1024 * 1024
        super(CompactionE2ERebootTest, self).__init__(
            test_context=test_context, extra_rp_conf={}, log_level="trace"
        )
