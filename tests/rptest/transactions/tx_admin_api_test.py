...
class TxAdminTest(RedpandaTest):
    ...
    def __init__(self, test_context):
        super(TxAdminTest, self).__init__(
    ...
    def test_simple_get_transaction(self):
        ...
        for topic in self.topics:
            for partition in range(topic.partition_count):
                ...
                assert len(expected_pids) == len(txs_info["active_transactions"])
    ...
    def test_mark_transaction_expired(self):
        ...
        txs_info = self.admin.get_transactions(topic.name, partition, "kafka")
        expected_pids = list(map(self.extract_pid, txs_info["active_transactions"]))
        ...
        wait_until(
            lambda: mark_expired(random_topic, random_partition),
            timeout_sec=10,
            backoff_sec=1,
            retry_on_exc=True,
        )
            ...
            for topic in self.topics:
                for partition in range(topic.partition_count):
                    txs_info = self.admin.get_transactions(
                        namespace="kafka", topic=topic.name, partition=partition
                    )
                    self.logger.info(
    ...
    def test_all_transactions(self):
        ...
        for partition in tx["partitions"]:
            ...
            expected_partitions[partition["topic"]].add(partition["partition_id"])

        for topic in self.topics:
            assert len(expected_partitions[topic.name]) == topic.partition_count
            for partition in range(topic.partition_count):
    ...
    def test_delete_topic_from_ongoin_tx(self):
        ...
        txs_info = self.admin.get_all_transactions()
        assert len(txs_info) == 1, "Should be only one transaction in current time"
        ...
        tx = txs_info[0]
        assert tx["transactional_id"] == tx_id, (
            f"Expected transactional_id: {tx_id}, but got {tx['transactional_id']}"
        )
    ...
    def test_delete_non_existent_topic(self):
        ...
        try:
            ...
            assert (
                e.response.text
                == '{"message": "Can not find partition({kafka/error_topic/0}) in transaction for delete", "code": 400}'
            )
            ...
            if partition["topic"] == topic_name:
                try:
                    ...
                    e.response.text == '{{"message": "Can not find partition({{{}/{}/{}}}) in transaction for delete", "code": 400}}'.format(
                        partition["ns"], partition["topic"], partition["partition_id"]
                    )
