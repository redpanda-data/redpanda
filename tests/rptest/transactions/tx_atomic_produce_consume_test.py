...
class ExactlyOnceVerifier:
    ...
    def start_src_producer(self):
        ...
        while not self._stop_ev.is_set():
            producer.produce(
                self._src_topic, value=str(i), key=str(i), on_delivery=delivery_report
            )
            i += 1
    ...
    def start_workload(self, workers: int):
        with ThreadPoolExecutor(max_workers=workers + 1) as executor:
            self._tasks.append(executor.submit(lambda: self.start_src_producer()))
        ...
        consumer = ck.DeserializingConsumer(
            {
                ...
                "key.deserializer": deserialize_str,
            }
        )
        try:
            with self._lock:
            ...
            def reached_end(positions):
                try:
                    ...
                    end_for = {
                        p.partition
                        for p in positions
                        if p.partition in high_watermarks
            ...
            consumer.subscribe(
                [self._src_topic], on_assign=on_assign, on_revoke=on_revoke
            )
            ...
            while True:
                ...
                if time.time() - last_progress > self._progress_timeout_sec:
                    ...
                    self._error = TimeoutError("timeout waiting for {tx_id} producer")
                    self._stop_ev.set()
                    break
    ...
    def validate_transform(self):
        ...
        for p, hw in src_high_watermarks.items():
            assert dst_high_watermarks[p] >= hw, (
                "Transformed partitions must the same or larger watermark than source"
            )
        ...
        consumer = ck.DeserializingConsumer(
            {
                "bootstrap.servers": self._redpanda.brokers(),
                "group.id": f"validator-group-{self._group_id}",
                "auto.offset.reset": "earliest",
        ...
        def is_finished():
            ...
            if len(positions) != len(src_high_watermarks) + len(dst_high_watermarks):
                return False
...
class TxAtomicProduceConsumeTest(RedpandaTest):
    def __init__(self, test_context):
        super(TxAtomicProduceConsumeTest, self).__init__(
    ...
    def test_basic_tx_consumer_transform_produce(self, with_failures):
        partition_count = 5
        topic = TopicSpec(
            name="src-test-exactly-once",
            replication_factor=3,
            partition_count=partition_count,
        )

        dst_topic = TopicSpec(
            name="dst-test-exactly-once",
            replication_factor=3,
            partition_count=partition_count,
        )
        self.redpanda.set_cluster_config({"group_new_member_join_timeout": "3000"})
        ...
        transformer = ExactlyOnceVerifier(
            self.redpanda,
            topic.name,
