...
class TxCoordinatorMigrationTest(RedpandaTest):
    def __init__(self, test_context):
        ...
        super(TxCoordinatorMigrationTest, self).__init__(
            test_context=test_context,
            num_brokers=3,
            extra_rp_conf={
                "transaction_coordinator_partitions": self.initial_tx_manager_partitions
            },
        )
    ...
    def _migrate_until_success(self):
        ...
        def _is_finished():
            try:
                ...
                return status["required"] is False and status["in_progress"] is False
            except Exception:
                return False
    ...
    def _assert_tx_lists_are_equal(self, expected_txs, new_txs):
        assert len(expected_txs) == len(new_txs), (
    ...
    def test_migrating_tx_manager_coordinator(self, with_failures, upgrade):
        ...
        try:
            ...
            assert False, (
                "migrate tx manager endpoint is not supposed to be present in normal mode"
            )
        ...
        assert initial_metadata == metadata, (
            "when number of partitions is the same metadata should be identical"
        )
        ...
        assert len(new_tp_metadata) == new_partition_count, (
            "Tx manager topic after migration is expected to have new number of partitions"
        )
