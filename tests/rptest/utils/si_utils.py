...
class S3Snapshot:
        ...
        for o in self.objects:
            if self.path_matcher.is_partition_manifest(o):
                ...
                self.logger.debug(
                    ...
                    f'{pprint.pformat(self.partition_manifests[manifest_path.to_ntp()], indent=2)}'
                )
    ...
    def is_segment_part_of_a_manifest(self, o: ObjectMetadata) -> bool:
        ...
        try:
            ...
            partition_manifest = self.partition_manifests.get(
                segment_path.ntpr.to_ntp())
            if not partition_manifest:
                self.logger.warn(
                    f'no manifest found for {segment_path.ntpr.to_ntp()}')
                return False
...
class BucketView:
    ...
    def __init__(self, redpanda, topics: Optional[Sequence[TopicSpec]] = None):
        ...
        self.bucket = redpanda._si_settings.cloud_storage_bucket
