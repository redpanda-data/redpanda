...
def package_test(name: str, test_dir: Path, args: argparse.Namespace) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        config_ref = f"{tmpl_vars['test_slug']}-config:{content_tag(staging)}"
    ...
    aliases = tag_images(local_refs, args.tag)
    ...
    if pushed:
        ...
        lines.append("Push skipped (no --push); to push manually:")
        for ref in [*local_refs, *aliases]:
