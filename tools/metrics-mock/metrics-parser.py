...
sample_regex = re.compile(r"(?P<name>[^ ]+){(?P<labels>[^}]*)} (?P<value>[-0-9.]+)")
# namespace="kafka"
...
KNOWN_STANDALONE_LABELS = {
    Endpoint.INTERNAL: _KNOWN_STANDALONE_LABELS,
    # PUBLIC is a mix of redpanda_ prefixed and not label names, so just include
    # all in both states
    Endpoint.PUBLIC: _KNOWN_STANDALONE_LABELS
    + list(map(add_prefix, _KNOWN_STANDALONE_LABELS)),
}
...
def parse_metrics(path: Path, endpoint: Endpoint) -> list[str]:
    ...
    metrics: list[Metric] = []
    name_to_meta: defaultdict[str, MetricMeta] = defaultdict(lambda: MetricMeta())
    ...
    for line in file:
        if m := help_regex.match(line):
            ...
            if (
                bname != name
                and bname in name_to_meta
                and name_to_meta[bname].type == "histogram"
            ):
                pass
            else:
                bname = name
            ...
            meta.count += 1
            metrics.append(Metric(name, parse_labels(labels), float(value), meta))
        else:
    ...
    for m in metrics:
        ...
        if existing is not None:
            ...
            for d in diff:
                assert d in MISMATCHED_LABELS_ALLOW, (
                    f"metric {m.name} had different label sets:\n{existing}\n{label_set}"
                )
        else:
    ...
    for mname, labels in name_to_labels.items():
        gens: list[str] = []
        special = sorted(map(strip_prefix, set(labels.keys()).difference(known_labels)))
        ...
        if not special:
            ...
            if special == ["namespace", "topic"]:
                ...
                if endpoint == Endpoint.INTERNAL:
                    assert labels.keys() == {"topic", "partition", "group", "shard"}, (
                ...
                raise RuntimeError(f"Unsupported special label set: {mname}{special}")
