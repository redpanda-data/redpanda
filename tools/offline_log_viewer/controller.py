...
def read_incremental_topic_update_serde(rdr: Reader):
    ...
    return rdr.read_envelope(incr_topic_upd, max_compat_version=7)
...
def decode_feature_command_serde(k_rdr: Reader, rdr: Reader):
    ...
    if cmd['type'] == 0:
        ...
        cmd |= k_rdr.read_envelope(
            lambda k_rdr, _: {
                'redpanda_license':
                k_rdr.read_envelope(decode_license_t, max_compat_version=1)
            })
    return cmd
...
def read_topic_metadata_fields(rdr: Reader, v: int):
    v = {}
    v |= {
        "configuration": rdr.read_envelope(read_topic_config,
                                           max_compat_version=2)
    }
...
class ControllerSnapshot():
    ...
    def read_snapshot(self, rdr: Reader):
        ...
        data['bootstrap'] = rdr.read_envelope(
            type_read=lambda r, v: self.read_bootstrap(r, v),
            max_compat_version=0)
        data['features'] = rdr.read_envelope(
            type_read=lambda r, v: self.read_features(r, v),
            max_compat_version=0)
        data['members'] = rdr.read_envelope(
            type_read=lambda r, v: self.read_members(r, v),
            max_compat_version=1)
        data['config'] = rdr.read_envelope(
            type_read=lambda r, v: self.read_config(r, v),
            max_compat_version=0)
        data['topics'] = rdr.read_envelope(
            type_read=lambda r, v: self.read_topics(r, v),
            max_compat_version=1)
        data['security'] = rdr.read_envelope(
            type_read=lambda r, v: self.read_security(r, v),
            max_compat_version=1)
        data['metrics_reporter'] = rdr.read_envelope(
            type_read=lambda r, v: self.read_metrics_reporter(r, v),
            max_compat_version=0)
        data['plugins'] = rdr.read_envelope(
            type_read=lambda r, v: self.read_plugins(r, v),
            max_compat_version=0)
    ...
    def parse_snapshot(self, snapshot_file):
        ...
        data = reader.read_checksum_envelope(
            type_read=lambda r, _: self.read_snapshot(r), max_compat_version=2)
