...
class Reader:
    def __init__(
        self, stream, endianness=Endianness.LITTLE_ENDIAN, include_envelope=False
    ):
    ...
    def read_envelope_inner(self, envelope, type_read, reader_version):
        ...
        if type_read is not None:
            ...
            if envelope.compat_version == 0:
                ...
                bytes_left_in_envelope -= self.stream.tell() - envelope_start_pos
                ...
                return {
                    "error": {"reader_version": reader_version, "envelope": envelope}
                }
        return envelope
