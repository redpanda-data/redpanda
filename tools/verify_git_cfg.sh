#!/bin/bash

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
verified=true
source "${script_dir}/base_script.sh"

git config format.signOff yes
git config diff.orderfile "${script_dir}/../.gitorderfile"

if [[ "$(git config user.name)" == "" ]]; then
    error "please set user name with 'git config user.name <user_name>'"
    verified=false
fi

if [[ "$(git config user.email)" != *"@vectorized.io" ]]; then
    error "Please set user email with 'git config user.email <user>@vectorized.io'"
    verified=false
fi

if [[ "$(git config sendemail.smtpuser)" != *"@vectorized.io" ]]; then
    error "Please set sendemail with 'git config sendemail.smtpuser <user>@vectorized.io'"
    verified=false
fi

if [[ "$(git config sendemail.from)" != "" ]]; then
    error "Please unset sendemail.from with 'git config --unset sendemail.from'"
    verified=false
fi

if [[ "$verified" == "false" ]]; then
    fatal "Please fix git config settings"
fi

log "Git config successfully verified"
